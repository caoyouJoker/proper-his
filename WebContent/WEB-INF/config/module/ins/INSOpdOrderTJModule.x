...
selectOpdOrder.SQL=SELECT  A.REGION_CODE,A.CASE_NO, A.SEQ_NO, A.CONFIRM_NO,A.CHARGE_DATE,A.NHI_ORDER_CODE,A.ORDER_CODE,H.XMMC AS ORDER_DESC,A.OWN_RATE,&
            D.DOSE_CHN_DESC AS DOSE_CODE, A.STANDARD, A.PRICE, A.QTY, A.TOTAL_AMT, A.TOTAL_NHI_AMT,A.OWN_AMT,A.ADDPAY_AMT,A.REFUSE_AMT,&
            ...
            B.MEDI_QTY||G.UNIT_CHN_DESC AS TAKE_QTY,  E.ROUTE_CHN_DESC||F.FREQ_CHN_DESC AS  ROUTE ,A.HYGIENE_TRADE_CODE,A.INS_CROWD_TYPE,A.INS_PAT_TYPE,A.ORIGSEQ_NO,A.TAKE_DAYS,A.INV_NO,A.RECP_TYPE,&
            D.INS_DOSE_CODE,E.INS_ROUTE_CODE||F.INS_FREQ_CODE AS INS_ROUTE &
            FROM INS_OPD_ORDER A , OPD_ORDER  B, PHA_BASE C, PHA_DOSE  D, SYS_PHAROUTE E, SYS_PHAFREQ F, SYS_UNIT G,INS_RULE H &
            WHERE A.CASE_NO = B.CASE_NO(+) &
            AND A.RX_SEQNO = B.RX_NO(+) &
            AND A.ORDER_CODE  = B.ORDER_CODE(+) &
            AND A.OPB_SEQNO  = B.SEQ_NO(+) &
            AND A.ORDER_CODE = C.ORDER_CODE(+) &
            AND A.NHI_ORDER_CODE =H.SFXMBM(+) &
            AND A.CHARGE_DATE BETWEEN H.KSSJ AND H.JSSJ &
            AND C.DOSE_CODE = D.DOSE_CODE(+) &
            AND C.ROUTE_CODE  = E.ROUTE_CODE(+) &
            AND B.FREQ_CODE = F.FREQ_CODE(+) &
            AND B.MEDI_UNIT  = G.UNIT_CODE(+) &
            AND A.CASE_NO=<CASE_NO> AND  A.CONFIRM_NO =<CONFIRM_NO> &
            UNION ALL &
            SELECT  A.REGION_CODE,A.CASE_NO, A.SEQ_NO, A.CONFIRM_NO,A.CHARGE_DATE,A.NHI_ORDER_CODE,A.ORDER_CODE,A.ORDER_DESC,A.OWN_RATE,&
            D.DOSE_CHN_DESC AS DOSE_CODE, A.STANDARD, A.PRICE, A.QTY, A.TOTAL_AMT, A.TOTAL_NHI_AMT,A.OWN_AMT,A.ADDPAY_AMT,A.REFUSE_AMT,&
            A.REFUSE_REASON_CODE,A.REFUSE_REASON_NOTE, A.OP_FLG, A.CARRY_FLG, A.PHAADD_FLG,A.ADDPAY_FLG,A.NHI_ORD_CLASS_CODE,A.INSAMT_FLG,&
            A.OPT_USER, A.OPT_DATE, A.OPT_TERM, A.RX_SEQNO, A.OPB_SEQNO,A.SPECIAL_CASE_DESC,&
            B.MEDI_QTY||G.UNIT_CHN_DESC AS TAKE_QTY,  E.ROUTE_CHN_DESC||F.FREQ_CHN_DESC AS  ROUTE ,A.HYGIENE_TRADE_CODE,A.INS_CROWD_TYPE,A.INS_PAT_TYPE,A.ORIGSEQ_NO,A.TAKE_DAYS,A.INV_NO,A.RECP_TYPE,&
            D.INS_DOSE_CODE,E.INS_ROUTE_CODE||F.INS_FREQ_CODE AS INS_ROUTE &
            FROM INS_OPD_ORDER A , OPD_ORDER  B, PHA_BASE C, PHA_DOSE  D, SYS_PHAROUTE E, SYS_PHAFREQ F, SYS_UNIT G &
            WHERE A.CASE_NO = B.CASE_NO(+) &
            AND A.RX_SEQNO = B.RX_NO(+) &
            AND A.ORDER_CODE  = B.ORDER_CODE(+) &
            AND A.OPB_SEQNO  = B.SEQ_NO(+) &
