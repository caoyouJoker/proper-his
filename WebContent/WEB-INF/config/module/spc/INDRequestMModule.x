...
Module.item=queryRequestM;createNewRequestM;updateRequestM;deleteRequestM;queryOutReqNo;queryOtherInNo;queryIBSDeptExmM;queryIBSDeptExmD;queryOPDDeptExmM;queryOPDDeptExmD;onQueryExm;queryIBSDeptExmDFinish;queryOdiDspnmExmEd;queryOdiDsnpmExmM;queryOdiDspnmExmD;&
            deleteSPCRequestM;querySPCRequestM;updateSPCRequestM;createNewSPCRequestM;queryOutReqNoAcnt;queryOtherInNoAcnt
...
//查询申请主档 wanglong add 20150202
querySPCRequestM.Type=TSQL
querySPCRequestM.SQL=SELECT REQUEST_NO, REQTYPE_CODE, APP_ORG_CODE, TO_ORG_CODE, REQUEST_DATE, &
             REQUEST_USER, REASON_CHN_DESC, DESCRIPTION, UNIT_TYPE, URGENT_FLG, &
             OPT_USER, OPT_DATE, OPT_TERM &
             FROM SPC_REQUESTM   &
                     ORDER BY REQUEST_DATE ASC
querySPCRequestM.ITEM=APP_ORG_CODE;REQUEST_NO;START_DATE;END_DATE;REQTYPE_CODE;TO_ORG_CODE;TYPE_DEPT_TEC;TYPE_WAS_THO;REGION_CODE;DRUG_CATEGORY;APPLY_TYPE
querySPCRequestM.APP_ORG_CODE=APP_ORG_CODE=<APP_ORG_CODE>
querySPCRequestM.REQUEST_NO=REQUEST_NO=<REQUEST_NO>
querySPCRequestM.REQTYPE_CODE=REQTYPE_CODE=<REQTYPE_CODE>
querySPCRequestM.START_DATE=REQUEST_DATE>=<START_DATE>
querySPCRequestM.END_DATE=REQUEST_DATE<=<END_DATE>
querySPCRequestM.TO_ORG_CODE=TO_ORG_CODE=<TO_ORG_CODE>
querySPCRequestM.TYPE_DEPT_TEC=REQTYPE_CODE IN ('DEP', 'TEC','ATO','TOX')
querySPCRequestM.TYPE_WAS_THO=REQTYPE_CODE IN ('WAS', 'THO')
querySPCRequestM.REGION_CODE=REGION_CODE=<REGION_CODE>
querySPCRequestM.APPLY_TYPE=APPLY_TYPE=<APPLY_TYPE>
querySPCRequestM.DRUG_CATEGORY=DRUG_CATEGORY=<DRUG_CATEGORY>
querySPCRequestM.Debug=N
...
//新建药库申请主档 wanglong add 20150202
createNewSPCRequestM.Type=TSQL
createNewSPCRequestM.SQL=INSERT INTO SPC_REQUESTM( &
            REQUEST_NO, REQTYPE_CODE, APP_ORG_CODE, TO_ORG_CODE, REQUEST_DATE, &
            REQUEST_USER, REASON_CHN_DESC, DESCRIPTION, UNIT_TYPE, URGENT_FLG, &
            OPT_USER, OPT_DATE, OPT_TERM, REGION_CODE,APPLY_TYPE,DRUG_CATEGORY) &
               VALUES( &
...
updateSPCRequestM.Type=TSQL
updateSPCRequestM.SQL=UPDATE SPC_REQUESTM SET &
              REQTYPE_CODE=<REQTYPE_CODE>, APP_ORG_CODE=<APP_ORG_CODE>, TO_ORG_CODE=<TO_ORG_CODE>, REQUEST_DATE=<REQUEST_DATE>, REQUEST_USER=<REQUEST_USER>, &
              REASON_CHN_DESC=<REASON_CHN_DESC>, DESCRIPTION=<DESCRIPTION>, UNIT_TYPE=<UNIT_TYPE>, URGENT_FLG=<URGENT_FLG>,OPT_USER=<OPT_USER>, &
              OPT_DATE=<OPT_DATE>, OPT_TERM=<OPT_TERM> &
            WHERE REQUEST_NO=<REQUEST_NO>
updateSPCRequestM.Debug=N
...
deleteSPCRequestM.Type=TSQL
deleteSPCRequestM.SQL=DELETE FROM SPC_REQUESTM WHERE REQUEST_NO=<REQUEST_NO>
deleteSPCRequestM.Debug=N
...
//查询所需出库作业的单据 wanglong add 20150202
queryOutReqNoAcnt.Type=TSQL
queryOutReqNoAcnt.SQL=SELECT DISTINCT A.REQUEST_NO, A.REQTYPE_CODE, A.APP_ORG_CODE, A.TO_ORG_CODE, A.REQUEST_DATE, &
             A.REQUEST_USER, A.REASON_CHN_DESC, A.DESCRIPTION, A.UNIT_TYPE, A.URGENT_FLG &
            FROM SPC_REQUESTM A, SPC_REQUESTD B &
           WHERE A.REQUEST_NO = B.REQUEST_NO &
             AND B.UPDATE_FLG IN ('0', '1') &
             AND A.REQTYPE_CODE <> 'THI' &
             AND B.QTY > B.ACTUAL_QTY &
                     ORDER BY A.REQUEST_NO DESC
queryOutReqNoAcnt.ITEM=APP_ORG_CODE;REQUEST_NO;START_DATE;END_DATE;REQTYPE_CODE;REGION_CODE
queryOutReqNoAcnt.APP_ORG_CODE=A.APP_ORG_CODE=<APP_ORG_CODE>
queryOutReqNoAcnt.REQUEST_NO=A.REQUEST_NO=<REQUEST_NO>
queryOutReqNoAcnt.REQTYPE_CODE=A.REQTYPE_CODE=<REQTYPE_CODE>
queryOutReqNoAcnt.START_DATE=A.REQUEST_DATE>=<START_DATE>
queryOutReqNoAcnt.END_DATE=A.REQUEST_DATE<=<END_DATE>
queryOutReqNoAcnt.REGION_CODE=A.REGION_CODE<=<REGION_CODE>
queryOutReqNoAcnt.Debug=N
...
//查询其他入库方式的单据 wanglong add 20150202
queryOtherInNoAcnt.Type=TSQL
queryOtherInNoAcnt.SQL=SELECT DISTINCT A.REQUEST_NO, A.REQTYPE_CODE, A.APP_ORG_CODE, A.TO_ORG_CODE, A.REQUEST_DATE, &
             A.REQUEST_USER, A.REASON_CHN_DESC, A.DESCRIPTION, A.UNIT_TYPE, A.URGENT_FLG &
            FROM IND_REQUESTM A, IND_REQUESTD B &
           WHERE A.REQUEST_NO = B.REQUEST_NO &
             AND B.UPDATE_FLG IN ('0', '1') &
             AND A.REQTYPE_CODE = 'THI' &
                     ORDER BY A.REQUEST_NO DESC
queryOtherInNoAcnt.ITEM=APP_ORG_CODE;REQUEST_NO;START_DATE;END_DATE;REQTYPE_CODE
queryOtherInNoAcnt.APP_ORG_CODE=A.APP_ORG_CODE=<APP_ORG_CODE>
queryOtherInNoAcnt.REQUEST_NO=A.REQUEST_NO=<REQUEST_NO>
queryOtherInNoAcnt.REQTYPE_CODE=A.REQTYPE_CODE=<REQTYPE_CODE>
queryOtherInNoAcnt.START_DATE=A.REQUEST_DATE>=<START_DATE>
queryOtherInNoAcnt.END_DATE=A.REQUEST_DATE<=<END_DATE>
queryOtherInNoAcnt.Debug=N
...
queryIBSDeptExmM.REQUEST_FLG_B=(A.REQUEST_FLG <> 'Y' OR A.REQUEST_FLG IS NULL)  
...
queryIBSDeptExmD.REQUEST_FLG_B=(A.REQUEST_FLG <> 'Y' OR A.REQUEST_FLG IS NULL)
//=======lirui 20120605 对药品管制（普通，毒麻，全部）
