...
//liuyalin  2017/02/17 从字典表取字段判断麻精和普药分类
...
queryOdiDsnpmExmM.SQL=SELECT 'N' AS SELECT_FLG, A.REQUEST_NO, C.ORDER_CODE, C.ORDER_DESC, &
	                 ...
	                 B.STOCK_PRICE, B.STOCK_PRICE * SUM (A.DOSAGE_QTY) AS STOCK_AMT, &
	                 A.OWN_PRICE, SUM(A.OWN_AMT) AS OWN_AMT, SUM(A.OWN_AMT) - B.STOCK_PRICE * SUM (A.DOSAGE_QTY) AS DIFF_AMT, &
					 (SELECT SUM(STOCK_QTY) FROM IND_STOCK WHERE ORG_CODE = <APP_ORG_CODE> AND ORDER_CODE = A.ORDER_CODE) AS QTY, &
					 H.SAFE_QTY,H.SAFE_QTY-(SELECT SUM(STOCK_QTY) FROM IND_STOCK WHERE ORG_CODE = <APP_ORG_CODE> AND ORDER_CODE = A.ORDER_CODE) AS SUP_QTY &
	            FROM ODI_DSPNM A, PHA_BASE B, SYS_FEE C, SYS_UNIT D ,IND_STOCKM H,IND_CABDSPN I &
	           WHERE A.ORDER_CODE = B.ORDER_CODE &
	             AND A.ORDER_CODE = C.ORDER_CODE &
	             AND B.ORDER_CODE = C.ORDER_CODE &
	             AND A.DOSAGE_UNIT = D.UNIT_CODE &
				 AND A.STATION_CODE = H.ORG_CODE &
...
queryOdiDsnpmExmM.CTRLDRUGCLASS_CODE_A=(A.CTRLDRUGCLASS_CODE IS NULL OR A.CTRLDRUGCLASS_CODE NOT IN (SELECT CTRLDRUGCLASS_CODE FROM SYS_CTRLDRUGCLASS WHERE CTRL_FLG='Y'))
queryOdiDsnpmExmM.CTRLDRUGCLASS_CODE_B=(A.CTRLDRUGCLASS_CODE IS NOT NULL AND A.CTRLDRUGCLASS_CODE IN (SELECT CTRLDRUGCLASS_CODE FROM SYS_CTRLDRUGCLASS WHERE CTRL_FLG='Y'))
...
//======== wukai modify 20170117
queryOdiDspnmExmD.ITEM=REQUEST_NO;REGION_CODE;CTRLDRUGCLASS_CODE_A;CTRLDRUGCLASS_CODE_B;ORDER_CODE;ORDER_DESC;SPECIFICATION;UNIT_CHN_DESC;OWN_PRICE;STOCK_PRICE
...
queryOdiDspnmExmD.CTRLDRUGCLASS_CODE_B=(A.CTRLDRUGCLASS_CODE IS NOT NULL AND A.CTRLDRUGCLASS_CODE IN (SELECT CTRLDRUGCLASS_CODE FROM SYS_CTRLDRUGCLASS WHERE CTRL_FLG='Y'))
//======== wukai modify 20170117
queryOdiDspnmExmD.ORDER_CODE=D.ORDER_CODE=<ORDER_CODE>
queryOdiDspnmExmD.ORDER_DESC=D.ORDER_DESC=<ORDER_DESC>
queryOdiDspnmExmD.SPECIFICATION=D.SPECIFICATION=<SPECIFICATION>
queryOdiDspnmExmD.UNIT_CHN_DESC=F.UNIT_CHN_DESC=<UNIT_CHN_DESC>
queryOdiDspnmExmD.OWN_PRICE=A.OWN_PRICE=<OWN_PRICE>
queryOdiDspnmExmD.STOCK_PRICE=C.STOCK_PRICE=<STOCK_PRICE>
...
queryOdiDspnmExmEd.CTRLDRUGCLASS_CODE_B=(A.CTRLDRUGCLASS_CODE IS NOT NULL AND A.CTRLDRUGCLASS_CODE IN (SELECT CTRLDRUGCLASS_CODE FROM SYS_CTRLDRUGCLASS WHERE CTRL_FLG='Y'))
...
onQueryExm.SQL=SELECT 'Y' AS SELECT_FLG, A.REQUEST_NO, A.ORDER_CODE, C.ORDER_DESC, &
			...
			GROUP BY 'Y',A.REQUEST_NO, A.ORDER_CODE, C.ORDER_DESC,C.SPECIFICATION, A.QTY, &
					 D.UNIT_CHN_DESC, A.STOCK_PRICE,A.RETAIL_PRICE,H.SAFE_QTY, A.SEQ_NO &
