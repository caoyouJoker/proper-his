...
getQueryQtyCheckMaster.SQL=SELECT B.TYPE_CODE, &
			    CASE WHEN (B.GOODS_DESC IS NULL) THEN B.ORDER_DESC &
			    ELSE B.ORDER_DESC || ' (' || B.GOODS_DESC || ')' &
			    END AS ORDER_DESC, B.SPECIFICATION, C.UNIT_CHN_DESC, &
			    SUM (A.ACTUAL_CHECK_QTY - A.STOCK_QTY) AS CHECK_QTY, &
			    A.RETAIL_PRICE AS OWN_PRICE, SUM (A.ACTUAL_CHECK_QTY - A.STOCK_QTY) * A.RETAIL_PRICE AS OWN_AMT &
		      ...
		      WHERE A.ORDER_CODE = B.ORDER_CODE &
		        ...
		        GROUP BY B.TYPE_CODE, B.GOODS_DESC, B.ORDER_DESC, B.SPECIFICATION, &
        		         C.UNIT_CHN_DESC, A.RETAIL_PRICE, A.ORDER_CODE &
...
getQueryQtyCheckMaster.OWN_PRICE_A=B.RETAIL_PRICE>=<OWN_PRICE_A>
getQueryQtyCheckMaster.OWN_PRICE_B=B.RETAIL_PRICE<=<OWN_PRICE_B>
getQueryQtyCheckMaster.CHECK_QTY_A=(A.ACTUAL_CHECK_QTY - A.STOCK_QTY)>=<CHECK_QTY_A>
getQueryQtyCheckMaster.CHECK_QTY_B=(A.ACTUAL_CHECK_QTY - A.STOCK_QTY)<=<CHECK_QTY_B>
getQueryQtyCheckMaster.CHECK_AMT_A=(A.ACTUAL_CHECK_QTY - A.STOCK_QTY) * A.RETAIL_PRICE>=<CHECK_AMT_A>
getQueryQtyCheckMaster.CHECK_AMT_B=(A.ACTUAL_CHECK_QTY - A.STOCK_QTY) * A.RETAIL_PRICE<=<CHECK_AMT_B>
getQueryQtyCheckMaster.Debug=N


//盘盈亏统计(明细)
getQueryQtyCheckDetail.Type=TSQL
getQueryQtyCheckDetail.SQL=SELECT A.UNFREEZE_DATE, B.TYPE_CODE, &
			    CASE WHEN (B.GOODS_DESC IS NULL) THEN B.ORDER_DESC &
			    ELSE B.ORDER_DESC || ' (' || B.GOODS_DESC || ')' &
			    END AS ORDER_DESC, B.SPECIFICATION, C.UNIT_CHN_DESC, A.BATCH_NO, A.VALID_DATE, &
			    A.STOCK_QTY, A.ACTUAL_CHECK_QTY, A.ACTUAL_CHECK_QTY - A.STOCK_QTY AS CHECK_QTY, &
			    A.RETAIL_PRICE AS OWN_PRICE, (A.ACTUAL_CHECK_QTY - A.STOCK_QTY) * A.RETAIL_PRICE AS OWN_AMT &
		      FROM IND_QTYCHECK A, PHA_BASE B, SYS_UNIT C &
		     WHERE A.ORDER_CODE = B.ORDER_CODE &
		       AND A.DOSAGE_UNIT = C.UNIT_CODE &
		       AND A.UNFREEZE_DATE IS NOT NULL &
...
getQueryQtyCheckDetail.OWN_PRICE_A=B.RETAIL_PRICE>=<OWN_PRICE_A>
getQueryQtyCheckDetail.OWN_PRICE_B=B.RETAIL_PRICE<=<OWN_PRICE_B>
getQueryQtyCheckDetail.CHECK_QTY_A=(A.ACTUAL_CHECK_QTY - A.STOCK_QTY)>=<CHECK_QTY_A>
getQueryQtyCheckDetail.CHECK_QTY_B=(A.ACTUAL_CHECK_QTY - A.STOCK_QTY)<=<CHECK_QTY_B>
getQueryQtyCheckDetail.CHECK_AMT_A=(A.ACTUAL_CHECK_QTY - A.STOCK_QTY) * A.RETAIL_PRICE>=<CHECK_AMT_A>
getQueryQtyCheckDetail.CHECK_AMT_B=(A.ACTUAL_CHECK_QTY - A.STOCK_QTY) * A.RETAIL_PRICE<=<CHECK_AMT_B>
