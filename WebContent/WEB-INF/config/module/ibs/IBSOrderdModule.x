...
#  version 1.0
Module.item=insertDdata;deleteDdata;selectdata;selectdata2;selOrderDAll;selectdataAll;selFeeDetail;selBillDetail;selSingleOrder;selMergeFee;selFeeByDept;&
	    ...
	    selAuditFeeData;selBillReturnD;seldelOrderDPatch;selMaxSeq;selDifferentSeq;selClpClncpathCode;selClpSchdCode
...
selectdata.SQL=SELECT D.CASE_NO,D.REXP_CODE,D.HEXP_CODE,SUM(D.OWN_AMT) AS OWN_AMT,SUM(D.NHI_PRICE) AS NHI_AMT,&
		...
		WHERE M.CASE_NO = D.CASE_NO & 
		  ...
		  AND M.BILL_NO IS NULL &
		  AND M.BILL_DATE <= <BILL_DATE> &		  
...
selectdata.STATION_CODE=M.STATION_CODE=<STATION_CODE>
selectdata.MR_NO=M.MR_NO=<MR_NO>
selectdata.IPD_NO=M.IPD_NO=<IPD_NO>
selectdata.BED_NO=M.BED_NO=<BED_NO>
selectdata.CASE_NO=D.CASE_NO=<CASE_NO>
selectdata.Debug=N

//查询未产生账单的费用明细档数据
selectdata2.Type=TSQL
selectdata2.SQL=SELECT D.CASE_NO,D.REXP_CODE,SUM(D.OWN_AMT) AS OWN_AMT,SUM(D.NHI_PRICE) AS NHI_AMT,&
		      SUM (D.TOT_AMT) AS AR_AMT &
		 FROM IBS_ORDM M,IBS_ORDD D &
		WHERE M.CASE_NO = D.CASE_NO & 
		  AND M.CASE_NO_SEQ =D.CASE_NO_SEQ &
...
selectdata2.item=STATION_CODE;MR_NO;IPD_NO;BED_NO;CASE_NO
selectdata2.STATION_CODE=M.STATION_CODE=<STATION_CODE>
selectdata2.MR_NO=M.MR_NO=<MR_NO>
selectdata2.IPD_NO=M.IPD_NO=<IPD_NO>
selectdata2.BED_NO=M.BED_NO=<BED_NO>
selectdata2.CASE_NO=D.CASE_NO=<CASE_NO>
selectdata2.Debug=Y
...
selBillDetail.SQL=SELECT D.ORDER_CODE,D.DOSAGE_UNIT,TO_CHAR(D.BILL_DATE,'HH24:MI') AS BILL_DATE,&
			 F.ORDER_DESC||CASE WHEN F.SPECIFICATION IS NOT NULL  THEN '('||F.SPECIFICATION||')' ELSE '' END AS ORDER_DESC,&
		         D.OWN_FLG,D.DOSAGE_QTY,D.OWN_PRICE,D.TOT_AMT,O.USER_NAME,D.CASE_NO,D.CASE_NO_SEQ,D.SEQ_NO,C.COST_CENTER_ABS_DESC &
...
selSingleOrder.SQL=SELECT D.BILL_DATE,D.OWN_FLG,D.DOSAGE_QTY,D.OWN_PRICE,&
			  D.TOT_AMT,O.USER_NAME,D.DR_CODE,D.EXE_DR_CODE,D.CASE_NO_SEQ,D.CASE_NO,D.SEQ_NO &
...
selMergeFee.SQL=SELECT D.REXP_CODE,D.ORDER_CODE,F.ORDER_DESC,F.SPECIFICATION DESCRIPTION,D.DOSAGE_QTY,D.DOSAGE_UNIT,&
		       D.OWN_PRICE,D.TOT_AMT,D.EXE_DEPT_CODE &
...
selFeeByDept.SQL=SELECT D.BILL_DATE,&
			...
			D.OPT_DATE,U.USER_NAME,D.INDV_FLG,D.ORDER_CODE,D.DOSAGE_UNIT,D.ORDERSET_CODE,D.CASE_NO_SEQ &
