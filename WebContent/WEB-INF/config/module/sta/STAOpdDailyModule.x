...
delete_STA_OPD_DAILY;select_STA_OPD_DAILY;FIRST_NUM;FURTHER_NUM;APPT_NUM;ZR_DR_NUM;ZZ_DR_NUM;ZY_DR_NUM;ZX_DR_NUM

//获取各个部门门诊人次  modify by huangtt 20141218
//SELECT COUNT(CASE_NO)  AS O_NUM,DEPT_CODE &
//		       FROM REG_PATADM &
//		       WHERE ADM_TYPE='O' &
...
//		       GROUP BY DEPT_CODE 
OUTP_NUM.Type=TSQL
OUTP_NUM.SQL=SELECT COUNT(A.CASE_NO)  AS O_NUM ,A.DEPT_CODE &
               FROM REG_PATADM A,BIL_REG_RECP B &
               WHERE A.ADM_TYPE='O' &
               AND A.CASE_NO = B.CASE_NO &
               AND A.REGCAN_USER IS NULL &
               AND B.BILL_DATE BETWEEN TO_DATE(<ADMDATE>,'YYYYMMDD') AND TO_DATE(<ADMDATE>||' 23:59:59','YYYYMMDD HH24:MI:SS') & 
...
//		GROUP BY DEPT_CODE
ERD_NUM.Type=TSQL
ERD_NUM.SQL=SELECT COUNT(A.CASE_NO)  AS O_NUM ,A.DEPT_CODE &
               FROM REG_PATADM A,BIL_REG_RECP B &
               WHERE A.ADM_TYPE='E' &
               AND A.CASE_NO = B.CASE_NO &
               AND A.REGCAN_USER IS NULL &
               ...
               FROM (SELECT CASE_NO,MR_NO,DEPT_CODE &
                      ...
                      WHERE VISIT_CODE = '0' &
                        ...
                        AND ((ADM_TYPE = 'O' &
                          AND ADM_DATE BETWEEN TO_DATE(<ADMDATE>, 'YYYYMMDD') AND TO_DATE(<ADMDATE> || ' 23:59:59', 'YYYYMMDD HH24:MI:SS')) &
                          OR (ADM_TYPE = 'E' &
                          AND REG_DATE BETWEEN TO_DATE(<ADMDATE>, 'YYYYMMDD') AND TO_DATE(<ADMDATE> || ' 23:59:59', 'YYYYMMDD HH24:MI:SS')))) &
           GROUP BY DEPT_CODE
FIRST_NUM.Debug=N
