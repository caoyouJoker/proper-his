#
# TBuilder Config File 
#
# Title:日结/月结报表
#
# Company:JavaHis
#
# Author:zhangy 2009.09.03
#
# version 1.0
#

<Type=TFrame>
UI.Title=日结/月结报表
UI.MenuConfig=%ROOT%\config\ind\INDMonthMenu.x
UI.Width=1024
UI.Height=748
UI.toolbar=Y
UI.controlclassname=com.javahis.ui.ind.INDMonthControl
UI.item=tPanel_1
UI.layout=null
UI.Text=日结/月结报表
UI.Tip=日结/月结报表
UI.TopMenu=Y
UI.TopToolBar=Y
tPanel_1.Type=TPanel
tPanel_1.X=5
tPanel_1.Y=5
tPanel_1.Width=1014
tPanel_1.Height=738
tPanel_1.AutoX=Y
tPanel_1.AutoY=Y
tPanel_1.AutoWidth=Y
tPanel_1.AutoHeight=Y
tPanel_1.Border=组
tPanel_1.Item=tLabel_6;DAY_TYPE;MONTH_TYPE;tLabel_7;TYPE_A;TYPE_B;tLabel_15;ORG_CODE;tLabel_16;tLabel_17;START_DATE;tTextField_0;tLabel_18;END_DATE;tTextField_1;MONTH_AREA
MONTH_AREA.Type=TTextFormat
MONTH_AREA.X=98
MONTH_AREA.Y=126
MONTH_AREA.Width=77
MONTH_AREA.Height=20
MONTH_AREA.Text=
MONTH_AREA.FormatType=date
MONTH_AREA.Format=yyyy/MM
MONTH_AREA.Enabled=N
tTextField_1.Type=TTextField
tTextField_1.X=384
tTextField_1.Y=163
tTextField_1.Width=62
tTextField_1.Height=20
tTextField_1.Text=23:59:59
tTextField_1.Enabled=N
END_DATE.Type=TTextFormat
END_DATE.X=290
END_DATE.Y=163
END_DATE.Width=90
END_DATE.Height=20
END_DATE.Text=
END_DATE.showDownButton=Y
END_DATE.FormatType=date
END_DATE.Format=yyyy/MM/dd
END_DATE.HorizontalAlignment=2
tLabel_18.Type=TLabel
tLabel_18.X=267
tLabel_18.Y=166
tLabel_18.Width=20
tLabel_18.Height=15
tLabel_18.Text=至
tTextField_0.Type=TTextField
tTextField_0.X=193
tTextField_0.Y=163
tTextField_0.Width=62
tTextField_0.Height=20
tTextField_0.Text=00:00:00
tTextField_0.Enabled=N
START_DATE.Type=TTextFormat
START_DATE.X=98
START_DATE.Y=163
START_DATE.Width=90
START_DATE.Height=20
START_DATE.Text=
START_DATE.showDownButton=Y
START_DATE.FormatType=date
START_DATE.Format=yyyy/MM/dd
START_DATE.HorizontalAlignment=2
tLabel_17.Type=TLabel
tLabel_17.X=20
tLabel_17.Y=166
tLabel_17.Width=72
tLabel_17.Height=15
tLabel_17.Text=统计日期
tLabel_16.Type=TLabel
tLabel_16.X=20
tLabel_16.Y=129
tLabel_16.Width=72
tLabel_16.Height=15
tLabel_16.Text=查询区间
ORG_CODE.Type=药房下拉列表
ORG_CODE.X=98
ORG_CODE.Y=89
ORG_CODE.Width=123
ORG_CODE.Height=23
ORG_CODE.Text=TButton
ORG_CODE.showID=Y
ORG_CODE.showName=Y
ORG_CODE.showText=N
ORG_CODE.showValue=N
ORG_CODE.showPy1=N
ORG_CODE.showPy2=N
ORG_CODE.Editable=Y
ORG_CODE.Tip=药房
ORG_CODE.TableShowList=name
ORG_CODE.ModuleParmTag=
ORG_CODE.ExpandWidth=30
ORG_CODE.OrgType=A
tLabel_15.Type=TLabel
tLabel_15.X=20
tLabel_15.Y=92
tLabel_15.Width=72
tLabel_15.Height=15
tLabel_15.Text=统计部门
TYPE_B.Type=TRadioButton
TYPE_B.X=168
TYPE_B.Y=51
TYPE_B.Width=81
TYPE_B.Height=23
TYPE_B.Text=中库
TYPE_B.Group=2
TYPE_B.Action=onChangeOrgType
TYPE_A.Type=TRadioButton
TYPE_A.X=94
TYPE_A.Y=52
TYPE_A.Width=65
TYPE_A.Height=23
TYPE_A.Text=主库
TYPE_A.Group=2
TYPE_A.Selected=Y
TYPE_A.Action=onChangeOrgType
tLabel_7.Type=TLabel
tLabel_7.X=20
tLabel_7.Y=55
tLabel_7.Width=72
tLabel_7.Height=15
tLabel_7.Text=统计单位
MONTH_TYPE.Type=TRadioButton
MONTH_TYPE.X=168
MONTH_TYPE.Y=13
MONTH_TYPE.Width=81
MONTH_TYPE.Height=23
MONTH_TYPE.Text=月结
MONTH_TYPE.Group=1
MONTH_TYPE.Action=onChangeCountType
DAY_TYPE.Type=TRadioButton
DAY_TYPE.X=94
DAY_TYPE.Y=13
DAY_TYPE.Width=66
DAY_TYPE.Height=23
DAY_TYPE.Text=日结
DAY_TYPE.Group=1
DAY_TYPE.Selected=Y
DAY_TYPE.Action=onChangeCountType
tLabel_6.Type=TLabel
tLabel_6.X=20
tLabel_6.Y=18
tLabel_6.Width=72
tLabel_6.Height=15
tLabel_6.Text=统计类别