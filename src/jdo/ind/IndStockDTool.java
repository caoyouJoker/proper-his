...
public class IndStockDTool
    ...
    /**
     * 根据药库编号及药品代码(还有批号)查询药库库存量
     *
     * @param parm
     * @return
     */
    public TParm onQueryStockQTYBatch(TParm parm) {  
        TParm result = this.query("queryStockQTYBatch", parm);
        if (result.getErrCode() < 0) {  
            err("ERR:" + result.getErrCode() + result.getErrText()
                + result.getErrName());
            return result;
    ...
    public TParm onQueryBatch(TParm parm) {  
        TParm result = this.query("queryBatch", parm);  
        if (result.getErrCode() < 0) {  
            err("ERR:" + result.getErrCode() + result.getErrText()
                + result.getErrName());
            return result;
     ...
     * @return TParm
     */
    public TParm onQueryStockQty(String org_code, String order_code,String batchSeq,
                                 String sort) {
        TParm result = new TParm(TJDODBTool.getInstance().select(INDSQL.
     ...
     * @return
     */
    public TParm onQueryStockMaxBatchSeqAcnt(String org_code, String order_code) {//wanglong add 20150202
        TParm result = new TParm(TJDODBTool.getInstance().select(
            INDSQL.getIndStockMaxBatchSeqAcnt(org_code, order_code)));
        if (result.getErrCode() < 0) {
            err("ERR:" + result.getErrCode() + result.getErrText()
                + result.getErrName());
            return result;
     ...
     * @return
     */
    public TParm onQueryStockBatchSeqAcnt(String org_code, String order_code,
                                      String batch_no, String valid_date) {
        TParm result = new TParm(TJDODBTool.getInstance().select(
            INDSQL.getIndStockBatchSeqAcnt(org_code, order_code, batch_no,
                                       valid_date)));
        if (result.getErrCode() < 0) {
            err("ERR:" + result.getErrCode() + result.getErrText()
                + result.getErrName());
...
//    					valid_date,verifyInPrice));
    ...
    */
   public TParm onQueryStockBatchSeqAcnt(String org_code, String order_code,
           String batch_no, String valid_date,String verifyInPrice) {//wanglong add 20150202
       TParm result = new TParm(TJDODBTool.getInstance().select(
               INDSQL.getIndStockBatchSeqAcnt(org_code, order_code, batch_no,
                       valid_date,verifyInPrice)));
//     System.out.println("sql:"+INDSQL.getIndStockBatchSeq(org_code, order_code, batch_no,
