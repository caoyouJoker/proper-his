 ...
 * @version 1.0
 */
public class InsManagerTool extends TJDOTool {
	public static boolean DEBUG = true;
	/**
	 * 实例
	 */
	public static InsManagerTool instanceObject;
	 ...
	 * @return SchWeekTool
	 */
	public static InsManagerTool getInstance() {
		if (instanceObject == null)
			instanceObject = new InsManagerTool();
	...
	public TParm getNhiFlg(Object parm) {
		...
		// 检核参数是否为空
		if (parm == null) {
			result.setErr(-1, "action.ins.InsInterface.getNhiFlg->Err:参数为空");
			return result;
		}
		// 检核参数类型是否正确
		if (!(parm instanceof TParm)) {
			result.setErr(-1, "action.ins.InsInterface.getNhiFlg->Err:参数类型错误");
			return result;
		}
	 ...
	 * 
	 * @param parm
	 *            Object TParm 传入参数 HOSP_AREA,PIPELINE,PLOT_TYPE,和相关数据<BR>
	...
	public TParm safe(Object parm) {
		//System.out.println("pangben 2012-01-29");
		TParm result = new TParm();
		// 检核参数是否为空
		if (parm == null) {
			result.setErr(-1, "action.ins.InsInterface.safe->Err:参数为空");
			return result;
		}
		// 检核参数类型是否正确
		if (!(parm instanceof TParm)) {
			result.setErr(-1, "action.ins.InsInterface.safe->Err:参数类型错误");
			return result;
		}
		TParm acParm = (TParm) parm;
		return safe(acParm);
	 ...
	 * 
	 * @param parm
	 *            TParm 传入参数 HOSP_AREA,PIPELINE,PLOT_TYPE,和相关数据<BR>
	 *            具体内容参见数据库字典INS_IO表<BR>
	 *            范例<BR>
	 *            parm.setCommitData("HOSP_AREA","HIS");<BR>
	 *            parm.setCommitData("PIPELINE","DataDown_rs");<BR>
	...
	public TParm safe(TParm parm) {
		...
		// if (!IsinsIodata(parm.getValue("PIPELINE"),
		...
		// return result;
		// }
		//System.out.println("parm出出出出出出出" + parm + "       dddd" + result);
		// 解析传出数据
	...
	private boolean parseIn(TParm parm, TParm result) {
		...
		for (int row = 0; row < rowCount; row++) {
			...
			if (rowCount > 1){ 
				if(((parm.getValue("PIPELINE").equals("DataDown_zjks")&&
					parm.getValue("PLOT_TYPE").equals("H"))||
					(parm.getValue("PIPELINE").equals("DataDown_cjks")&&
					parm.getValue("PLOT_TYPE").equals("H"))||
					(parm.getValue("PIPELINE").equals("DataUpload")&&
					parm.getValue("PLOT_TYPE").equals("H")))&&(row+1)!=rowCount){
					sb.append(newline);
				}
				else if((!parm.getValue("PIPELINE").equals("DataDown_zjks")&&
					     !parm.getValue("PLOT_TYPE").equals("H"))||
				        (!parm.getValue("PIPELINE").equals("DataDown_cjks")&&
					     !parm.getValue("PLOT_TYPE").equals("H"))||
					     (!parm.getValue("PIPELINE").equals("DataUpload")&&
		...
		// subValue,false);
		// // FileWriter file =new FileWriter("D:\\log\\INS_IN\\" + fileName +
		// ".txt");
		// // file.write(subValue);
		// }
		// catch (IOException e) {
		...
		//System.out.println("ioData出出出出出出出出《《》《》《》《》《》《》" + ioData);
		if (ioData.getErrCode() < 0) {
			result.setErr(-1, ioData.getErrText(), ioData.getErrName());
			return false;
		}
	...
	private TParm getIOData(TParm parm) {
		...
		if (ioData.getErrCode() < 0)
			return ioData;
		if (ioData.getErrCode() > 0) {
			ioData.setErr(-1, "INS_IO 表中没有找到 [CITY=" + parm.getValue("CITY")
					+ " PIPELINE=" + parm.getValue("PIPELINE") + " PLOT_TYPE="
	...
	public TParm linkFunction(TParm parm) {
		...
		parmm.setData("ioOutData", ((TParm) parm.getData("ioOutData"))
				.getData());
	...
	private TParm doSocket(TParm parm) {
		...
		//socket = new TSocket("192.168.1.100", TSocket.FILE_SERVER_PORT);
//		System.out.println("测试接口方法:::::::测试222222");
//		socket=new TSocket("192.168.1.100",8103);
//		System.out.println("测试接口方法:::::::测试333333");
//       // Object obj = socket.doSocket("aaa");
//        TParm parmD = new TParm();
//        parmD.setData("ACTION",0xFF);
//        Object obj = socket.doSocket(parmD.getData());
//        parmD = new TParm((Map)obj);
//        System.out.println("parm==="+parmD);
//
//		// socket = new TSocket("192.168.1.100",8103);
//		System.out.println("测试接口方法:::::::测试");
...
//	}
	...
	// ========================================//
	/*
	 * public Properties test_Data() { Properties prop = new Properties();
	 * prop.setProperty("HOSP_AREA","HIS"); return prop; } public boolean
	 * test_getNhiFlg() { ActionParm parm = new ActionParm();
	 * parm.setCommitData("HOSP_AREA",test_Data().getProperty("HOSP_AREA"));
	 * System.out.println("========>test_getNhiFlg()");
	 * System.out.println("inPut:" + parm); ActionParm result = getNhiFlg(parm);
	 * System.out.println("outPut:" + result); boolean state =
	 * result.getErrCode() == 0; System.out.println("state:" + state); return
	 * state; } public boolean test_NhiFlg1() { System.out.println("isDebug()="
	 * + isDebug(test_Data().getProperty("HOSP_AREA")));
	 * System.out.println("isRegOnline()=" +
	 * isRegOnline(test_Data().getProperty("HOSP_AREA")));
	 * System.out.println("isOpbOnline()=" +
	 * isOpbOnline(test_Data().getProperty("HOSP_AREA"))); return true; }
	 */
	public static void main(String args[]) {
		InsManagerTool driver = new InsManagerTool();
