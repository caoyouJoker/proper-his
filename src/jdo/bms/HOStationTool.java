...
/**
 * <p>Title: 保健办公室tool类</p>
 *
 * <p>Description: 保健办公室tool</p>
 *
 ...
 * @version 5.0
 */
public class HOStationTool extends TJDOTool{

    /**
     * 构造器
     */
    public HOStationTool() {
        onInit();
    }
     ...
     * 实例
     */
    private static HOStationTool instanceObject;

    /**
     * 得到实例
     * @return INFCaseTool
     */
    public static HOStationTool getInstance() {
        if (instanceObject == null) instanceObject = new HOStationTool();
	...
	public TParm onNew(TParm parm, TConnection connection){
		TParm result = new TParm();
		for(int i=1;i<=parm.getInt("i");i++){

			 String sql = " INSERT INTO REG_AUTH_CODE (AUTH_SYS_NO,CREATE_USER,OPT_USER,OPT_TERM,OPT_DATE,CREATE_DATE,SEQ_NO,AUTH_CODE,STATUS) "
			 		+ "VALUES('" + parm.getValue("AUTH_SYS_NO") +"','"  + parm.getValue("CREATE_USER") +"','" +
					  parm.getValue("OPT_USER") + "','" + parm.getValue("OPT_TERM") + "',to_date('" +
					  parm.getValue("OPT_DATE")+"','yyyy-mm-dd hh24:mi:ss'),to_date('"+parm.getValue("CREATE_DATE")+"','yyyy-mm-dd hh24:mi:ss'),"
			 				+ "" + i + ",'" + parm.getValue("TEXT"+i) + "','" +  parm.getValue("STATUS") +  "')";//授权码
			 
//			 System.out.println("sql" + i + sql + "111111HOStationTool" + parm);
