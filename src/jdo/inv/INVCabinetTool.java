...
public class INVCabinetTool extends TJDOTool {

	/**
	 * 实例
	 */
	private static INVCabinetTool instanceObject;
	 ...
	 * @return PatAdmTool
	 */
	public static INVCabinetTool getInstance() {
		if (instanceObject == null)
			instanceObject = new INVCabinetTool();
	 ...
	 * 构造器
	 */
	public INVCabinetTool() {
		setModuleName("inv\\INVCabinetModule.x");
		onInit();
	}
	...
	public TParm queryLog(TParm parm) {
		...
		String sql ="SELECT CABINET_ID,(SELECT A.CABINET_DESC FROM IND_CABINET A WHERE A.CABINET_ID=CABINET_ID ) AS " +
				"CABINET_DESC, LOG_TIME, (CASE WHEN TASK_TYPE=1 THEN '补货申请-病区普药入库' WHEN TASK_TYPE=2 " +
				"THEN '临时医嘱/首日量-病区普药出库' WHEN TASK_TYPE=3 THEN '紧急抢救-病区普药出库' WHEN TASK_TYPE=4 THEN " +
				"'补货申请-麻精入库' WHEN TASK_TYPE=5 THEN '补货申请-麻精出库' WHEN TASK_TYPE=6 THEN '临时医嘱/首日量-病区麻精出库' " +
				"WHEN TASK_TYPE=7 THEN '紧急抢救-病区麻精出库' WHEN TASK_TYPE=8 THEN '临时医嘱/首日量-住院药房麻精出库' ELSE '' END) " +
				"AS TASK_TYPE,TASK_NO,(CASE WHEN EVENT_TYPE=1 THEN '开门' WHEN EVENT_TYPE=2 THEN '关门' " +
				"ELSE '' END) AS EVENT_TYPE, GUARD_ID,OPT_USER,OPT_DATE,OPT_TERM FROM IND_CABINET_LOG  WHERE 1=1 ";
			if(TASK_TYPE!=null) {
				sql = sql+" AND TASK_TYPE='"+TASK_TYPE+"'";
			}	
			if(EVENT_TYPE!=null) {
				sql = sql+" AND EVENT_TYPE='"+TASK_TYPE+"'";
			}	
			if((START_DATE!=null)&&(END_DATE!=null)) {
				sql = sql + " AND LOG_TIME BETWEEN TO_DATE('"+START_DATE.toString()+"','YYYY-MM-DD HH24:MI:SS')"+
