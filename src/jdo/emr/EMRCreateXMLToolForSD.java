...
/**
 * <p>Title: EMR转XML工具类(for 单病种)</p>
 *
 * <p>Description:此类和EMRCreateXMLTool基本一致，只修改和加入若干行代码，
 *                专为单病种准备，不能通用，请勿用在别处 </p>
 *
 ...
 * @version 1.0
 */
public class EMRCreateXMLToolForSD {//add by wanglong 20121128
     ...
     * 实例
     */
    private static EMRCreateXMLToolForSD instanceObject;


    /**
     * 构造器
     */
    public EMRCreateXMLToolForSD() {
    }
     ...
     */
    public static EMRCreateXMLToolForSD getInstance() {
        if (instanceObject == null)
            instanceObject = new EMRCreateXMLToolForSD();
...
//                System.out.println("--------block-----------------"+block.getBlockValue());
   ...
   private void setTableRowColumn(int pageIndex,int panelIndex,
    ...
    private TParm getStyle(EText text){
        ...
        //style还有些问题
        parm.setData("style",((DStyle)text.getStyle()).getFont().getStyle());
...
//        System.out.println("======XMLElementCDA2=====");
    ...
    private Node XMLElement(String name,String value,TParm attributes,Node parent){
        try{
        	if(!name.equals("")){//add by wanglong 20121128
                Element element = XMLDoc.createElement(name);
        		...
        		return null;//add by wanglong 20121128
        	}
        }catch(Exception ex){
            ex.printStackTrace();
