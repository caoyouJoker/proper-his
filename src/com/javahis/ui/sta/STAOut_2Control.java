...
public class STAOut_2Control
    ...
    private TParm Day_Data;//记录日期段查询结果 用于打印
    ...
    private void generate_MonthData() {
        ...
        //生成的结果集
        TParm result = STAOut_2Tool.getInstance().selectDiseaseSum(parm);
        ...
        TParm re = TIOM_AppServer.executeAction(
            "action.sta.STAOut_2Action",
            "insertSTA_OUT_02", result);
        if (re.getErrCode() < 0) {
    ...
    private void gridBind(String STADATE){
        TTable table = (TTable)this.getComponent("Table");
        //=======pangben modify 20110520 添加区域
        String sql = STASQLTool.getInstance().getSTA_OUT_02(STADATE,Operator.getRegion());
        table.setSQL(sql);
        table.retrieve();
        table.setDSValue();
...
//        }
        ...
        if("month".equals(S_TYPE)){//如果是月统计 查询数据库 取得该月统计信息
            print = STAOut_2Tool.getInstance().selectPrint(DATA_StaDate,Operator.getRegion());//====pangben modfiy 20110520
        ...
        for(int i=0;i<page_f1;i++){
            ...
            printDate.addData("ICD_DESC_2",print.getValue("ICD_DESC",i+page_f1));
        ...
        for(int i=page1;i<(page1+page_f2);i++){
            ...
            printDate.addData("ICD_DESC_2",print.getValue("ICD_DESC",i+page_f2));
        ...
        for(int i=page2;i<(page2+page_f3);i++){
            ...
            printDate.addData("ICD_DESC_2",print.getValue("ICD_DESC",i+page_f3));
        ...
        for(int i=page3;i<(page3+page_f4);i++){
            ...
            printDate.addData("ICD_DESC_2",print.getValue("ICD_DESC",i+page_f4));
        ...
        printDate.addData("SYSTEM", "COLUMNS", "DATA_07_1");
        ...
        printDate.addData("SYSTEM", "COLUMNS", "DATA_07_2");
        //打印参数
        String dataDate = "";
        ...
        this.openPrintWindow("%ROOT%\\config\\prt\\sta\\STA_OUT_02.jhw", parm);
    }
    /**
     * 保存
     */
    ...
    private boolean canGeneration(String STADATE){
        boolean can=false;
        //检核数据状态
        int reFlg = STATool.getInstance().checkCONFIRM_FLG("STA_OUT_02",
            STADATE,Operator.getRegion());//======pangben modify 20110520 添加区域
        //数据已经提交
        if (reFlg == 2) {
    ...
    private void setDateVisble(String type){
        if("month".equals(type)){//月统计
            ...
            ((TLabel)this.getComponent("tLabel_3")).setVisible(false);
            ...
            ((TLabel)this.getComponent("tLabel_3")).setVisible(true);
