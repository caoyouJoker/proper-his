...
public class STAIn_04Control
   ...
   public void showFormat() {
    ...
    private void generate_MonthData() {
        ...
        TParm data = STAIn_04Tool.getInstance().selectData(parm);
        ...
        TParm re = TIOM_AppServer.executeAction(
            "action.sta.STAIn_04Action",
            "insertSTA_IN_04", data);
        if (re.getErrCode() < 0) {
   ...
   private void generate_DayData(){
       ...
       if(Dept.trim().length()>0){
           ...
           TParm deptInfo = STADeptListTool.getInstance().selectNewIPDDeptCode(this.
               getValueString("DEPT_CODE"),Operator.getRegion());//===========pangben modify 20110524
       ...
       parm.setData("REGION_CODE",Operator.getRegion());//===========pangben modify 20110524
       TParm result = STAIn_04Tool.getInstance().selectData(parm);
...
//		System.out.println("------------parm----------"+parm);
		TParm re = TIOM_AppServer.executeAction("action.sta.STAIn_04Action",
				"updateSTA_IN_04", parm);
		if (re.getErrCode() < 0) {
...
//        System.out.println("data:"+data);
    ...
    private TParm getPrintData(String STA_DATE){
        ...
        if ("day".equals(S_TYPE) && DAY_DEPT.length() > 0) {
            DeptList = STADeptListTool.getInstance().selectNewDeptByCode(DAY_DEPT,Operator.getRegion());//===========pangben modify 20110524
        }
        else {
        ...
        if ("month".equals(S_TYPE)) { //如果是月统计 查询数据库 取得该月统计信息
            data = STAIn_04Tool.getInstance().selectSTA_IN_04(STA_DATE);
        }
        ...
        boolean isOE = false;//记录科室是否包含住院科室信息
...
//                d_DESC = "  " + d_DESC; //加入前方空格
            ...
            for(int j=0;j<data.getCount("STA_DATE");j++){
                ...
                if(subDept.equals(d_CODE)){
                    //判断该子科室是否是门急住院诊科室如果不是 那么继续下一次循环
            ...
            if((DATA_01-DATA_04)!=0){
                DATA_05 = (double)DATA_02/(double)(DATA_01-DATA_04)*100;//计算门诊与住院符合率
                DATA_06 = (double)DATA_03/(double)(DATA_01-DATA_04)*100;//计算门诊与住院不符合率
            }
            if((DATA_07-DATA_10)!=0){
                DATA_11 =  (double)DATA_08/(double)(DATA_07-DATA_10)*100;//计算“入院与出院”符合率
                DATA_12 = (double)DATA_09/(double)(DATA_07-DATA_10)*100;//计算“入院与出院”不符合率
            }
            if((DATA_13-DATA_16)!=0){
                DATA_17 =  (double)DATA_14/(double)(DATA_13-DATA_16)*100;//计算临床与病理符合率
                DATA_18 = (double)DATA_15/(double)(DATA_13-DATA_16)*100;//计算临床与病理不符合率
            }
            if((DATA_19-DATA_22)!=0){
                DATA_23 =  (double)DATA_20/(double)(DATA_19-DATA_22)*100;//计算“手术前与手术后”符合率
                DATA_24 = (double)DATA_21/(double)(DATA_19-DATA_22)*100;//计算“手术前与手术后”不符合率
            }
            if(isOE){
                double a = 0;
    ...
    private void gridBind(String STADATE) {
        ...
        table.setLockColumns("0");//锁定第一列不可修改
        String sql = STASQLTool.getInstance().getSTA_IN_04(STADATE,Operator.getRegion(),deptcode);//===========pangben modify 20110524
		...
		for (int i = 0; i < result.getCount("DEPT_CODE"); i++) {
			...
			DATA_21 += result.getDouble("DATA_21", i);
      ...
      //计算符合率 分母不可为零
		...
		result.addData("DATA_25", DATA_25 == 0? 0 : DATA_25);
    ...
    private void gridBind(TParm result) {
        ...
        table.setLockColumns("all");//锁定所有列不可修改
        int DATA_01 = 0;
        int DATA_02 = 0;
        int DATA_03 = 0;
        int DATA_04 = 0;
    ...
    private boolean canGeneration(String STADATE) {
        boolean can = false;
        //检核数据状态
        int reFlg = STATool.getInstance().checkCONFIRM_FLG("STA_IN_04",
