 ...
 * @version 1.0
 */
public class STAIn_06Control extends TControl {
	...
	private void generate_MonthData() {
		...
		parm.setData("REGION_CODE", Operator.getRegion()); // ==============pangben
															// modify 20110525
		TParm data = STAIn_06Tool.getInstance().selectData(parm);
		...
		for (int i = 0; i < data.getCount("STA_DATE"); i++) {
			...
			data.setData("REGION_CODE", i, Operator.getRegion()); // ==========pangben
																	// modify
																	// 20110525
		}
		TParm re = TIOM_AppServer.executeAction("action.sta.STAIn_06Action",
				"insertSTA_IN_06", data);
		if (re.getErrCode() < 0) {
	...
	private void generate_DayData() {
		...
		if (Dept.trim().length() > 0) {
			// 根据选中的科室CODE查询科室详细信息
			TParm deptInfo = STADeptListTool.getInstance().selectNewIPDDeptCode(
					this.getValueString("DEPT_CODE"), Operator.getRegion()); // =========pangben
																				// modify
																				// 20110525
...
//		System.out.println("parm============"+parm);
		TParm result = STAIn_06Tool.getInstance().selectData(parm);
	...
	private TParm getPrintData(String STA_DATE) {
		...
		if ("day".equals(S_TYPE) && DAY_DEPT.length() > 0) {
			...
			if (printType.equals("3")) { // 按照三级科室打印
				...
				DeptList = STATool.getInstance().getDeptByLevel(
						new String[] { "1", "2", "3", "4" },
						Operator.getRegion()); // ===========pangben modify
												// 20110525
			}
		}
		...
		if ("month".equals(S_TYPE)) { // 如果是月统计 查询数据库 取得该月统计信息
			data = STAIn_06Tool.getInstance().selectSTA_IN_06(STA_DATE,
					Operator.getRegion()); // ===========pangben modify 20110525
			...
			ConditionParm.setData("DATE_S", DATE_Start.replace("/", ""));
			ConditionParm.setData("DATE_E", DATE_End.replace("/", ""));
		}
		if (data.getErrCode() < 0) {
			return data;
		}
		ConditionParm.setData("REGION_CODE", Operator.getRegion()); // ===========pangben
																	// modify
																	// 20110525
		...
		for (int i = 0; i < DeptList.getCount(); i++) {
			...
			for (int j = 0; j < data.getCount("STA_DATE"); j++) {
				...
				if (subDept.equals(d_CODE)) {
					...
					if (deptIPD.get(data.getValue("DEPT_CODE", j)).toString()
							.length() > 0) {
						...
						// 住院科室CODE和住院病区CODE
						String[] ipd = (String[]) deptIPD.get(data.getValue(
								"DEPT_CODE", j));
						// 计算 病危实际住院日数
			...
			if (isOE) {
				...
				printData.addData("DATA_02",
						DATA_02 == 0 ? "" : df.format(DATA_02));
				printData.addData("DATA_03",
						DATA_03 == 0 ? "" : df.format(DATA_03));
				printData.addData("DATA_04",
						DATA_04 == 0 ? "" : df.format(DATA_04));
				...
				printData.addData("DATA_12",
						DATA_12 == 0 ? "" : df.format(DATA_12));
				printData.addData("DATA_13",
						DATA_13 == 0 ? "" : df.format(DATA_13));
				printData.addData("DATA_14",
						DATA_14 == 0 ? "" : df.format(DATA_14));
	...
	private void gridBind(String STADATE) {
		...
		String sql = STASQLTool.getInstance().getSTA_IN_06(STADATE,
				Operator.getRegion(),deptcode); // ===========pangben modify 20110525
		...
		for (int j = 0; j < result.getCount("DEPT_CODE"); j++) {
			DATA_01 += result.getInt("DATA_01", j);
			DATA_05 += result.getInt("DATA_05", j);
			DATA_06 += result.getInt("DATA_06", j);
			DATA_07 += result.getInt("DATA_07", j);
			DATA_08 += result.getInt("DATA_08", j);
			DATA_09 += result.getInt("DATA_09", j);
			DATA_15 += result.getInt("DATA_15", j);
			DATA_16 += result.getInt("DATA_16", j);
			DATA_17 += result.getInt("DATA_17", j);
			DATA_18 += result.getInt("DATA_18", j);
			DATA_19 += result.getInt("DATA_19", j);
			out_num+= result.getInt("DATA_25", j);
			inDays1+=result.getInt("DATA_26", j);
			inDays2+=result.getInt("DATA_27", j);
		}
		if (DATA_05 != 0) {
			DATA_10 = (double) DATA_06 / (double) DATA_05 * 100; // 治愈率
			DATA_11 = (double) DATA_07 / (double) DATA_05 * 100; // 好转率
		...
		if (out_num != 0) {
			DATA_02 = (double) (DATA_05 + DATA_15) / out_num * 100;// 危急病人占百分比
			DATA_03 = (double) DATA_05 / out_num * 100;// 病危病人占出院病人百分比
			DATA_04 = (double) DATA_15 / out_num * 100;// 病急病人占出院病人百分比
		}
	...
	private void gridBind(TParm result) {
		...
		for (int i = 0; i < result.getCount("DEPT_CODE"); i++) {
			...
			inDays1 += result.getInt("DATA_26", i);
			inDays2 += result.getInt("DATA_27", i);
			out_num += result.getInt("DATA_25", i);
		}
		if (DATA_05 != 0) {
			DATA_10 = (double) DATA_06 / (double) DATA_05 * 100; // 治愈率
			DATA_11 = (double) DATA_07 / (double) DATA_05 * 100; // 好转率
	...
	private boolean canGeneration(String STADATE) {
		boolean can = false;
		// 检核数据状态
		int reFlg = STATool.getInstance().checkCONFIRM_FLG("STA_IN_06",
				STADATE, Operator.getStation()); // =============pangben modify
													// 20110525
		// 数据已经提交
		if (reFlg == 2) {
