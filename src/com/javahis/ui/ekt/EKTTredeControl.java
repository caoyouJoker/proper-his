...
import com.tiis.ui.dataChangedEventEvent;
...
//        setValue("USER_ID", Operator.getID());
    ...
    public void onQuery() {
        ...
        for (int i = 0; i < parm.getCount(); i++) {
        	parm.setData("LATEST_AMT", i, df.format(parm.getDouble("LATEST_AMT", i)));
        	parm.setData("BUSSINESS_AMT", i, df.format(parm.getDouble("BUSSINESS_AMT", i)));
        	parm.setData("CURRENT_AMT", i, df.format(parm.getDouble("CURRENT_AMT", i)));
...
//            where2 += " AND A.CASE_NO = '" + getValueString("CASE_NO") + "'";
        ...
        if (!"".equals(this.getValueString("BUSINESS_NO"))) {
            where1 += " AND A.HISTORY_NO = '" + getValueString("BUSINESS_NO") +
                "'";
//            where2 += " AND A.HISTORY_NO = '" + getValueString("BUSINESS_NO") +
//            "'";
        }
...
//        	getValueString("PAT_NAME") + "'";
        ...
        return 
        " SELECT   A.MR_NO, A.CARD_NO, A.CASE_NO, A.HISTORY_NO, B.PAT_NAME," +
        " A.LATEST_AMT, A.BUSSINESS_AMT, " +
        " A.CURRENT_AMT, A.OPT_TYPE BUSINESS_TYPE, A.OPT_USER, A.OPT_DATE," +
        " A.OPT_TERM" +
        " FROM EKT_MASTER_HISTORY A, SYS_PATINFO B" +
...
//        where2 +
    ...
    public void onClear() {
        String clear =
            "START_DATE;END_DATE;USER_ID;TREDE_NO;CARD_NO;MR_NO;CASE_NO;BUSINESS_NO;BUSINESS_TYPE;PAT_NAME";
...
//        TParm parm = EKTIO.getInstance().getPat();
    ...
    public void onOld(){
        ...
        parm.setData("END_DATE",  this.getValueString("END_DATE"));
        ...
        parm.setData("CARD_NO",  this.getValueString("CARD_NO"));
        
        parm.setData("PAT_NAME",  this.getValueString("PAT_NAME"));
        
        parm.setData("BUSINESS_NO",  this.getValueString("BUSINESS_NO"));
        
    	this.openDialog("%ROOT%\\config\\ekt\\EKTTredeOldUI.x",parm);
    }
