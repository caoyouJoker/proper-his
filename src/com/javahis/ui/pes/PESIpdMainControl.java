 ...
 * </p>
 * 
 * @author caoyong 20140214
 * @version 1.0
 */
public class PESIpdMainControl extends TControl {
	...
	public void onQuery() {
		...
		TParm result = PESTool.getInstance().selectIPDOPDM(parm);
	...
	public void onClickTableM() {
		...
		if("1".equals(this.getValue("PHA_TYPE"))){
			parm.setData("PHA_TYPE1",getValue("PHA_TYPE"));
		}
		if("2".equals(this.getValue("PHA_TYPE"))){
			parm.setData("PHA_TYPE2", getValue("PHA_TYPE"));
		}
		//if("3".equals(this.getValue("PHA_TYPE"))&&this.getValue("PHA_TYPE")!=null){
			//parm.setData("PHA_TYPE3", getValueString("PHA_TYPE"));
		//}
		if("1".equals(this.getValue("TYPE_CODE"))){
			parm.setData("TYPE_CODE1", getValueString("TYPE_CODE"));
		}
		if("2".equals(this.getValue("TYPE_CODE"))){
			parm.setData("TYPE_CODE2", getValueString("TYPE_CODE"));
		}
		parm.setData("PES_NO", getValueString("PES_NO"));
		parm.setData("CASE_NO", tableM.getParmValue().getValue("CASE_NO", row));
		//parm.setData("TYPE_CODE", tableM.getParmValue().getValue("TYPE_CODE",
				//row));
		TParm result = PESTool.getInstance().selectIPDOPDD(parm);
		
		
		for(int i=1;i<result.getCount()+1;i++){
			result.addData("SEQ_NO",i);
			result.setData("ORDER_STATE", i-1, this.getName(result.getValue("ORDER_STATE",i-1)));
		}
	...
	public void onSavePESResult() {
		...
		parm.setData("TYPE_CODE", getValueString("PES_TYPE"));
		parm.setData("PES_NO", getValueString("PES_NO"));
		TParm tableParm = PESTool.getInstance().selectIPDOPDM(parm);
