...
public class MROOpeAvgOfBloodControl extends TControl {
	 ...
	 */
	public void onQuery(){
		String sql = "SELECT M.OUT_DEPT,M.OP_CODE,COUNT(M.OP_CODE) QTY,ROUND(AVG(M.RBC),2) RBC,ROUND(AVG(M.PLASMA),2) PLASMA," +
				"ROUND(AVG(M.PLATE),2) PLATE,ROUND(AVG(WHOLE_BLOOD),2) WHOLE_BLOOD,ROUND(AVG(OTH_BLOOD),2) OTH_BLOOD FROM ";
		String groupSql = "(SELECT A.OUT_DEPT, E.OPT_CHN_DESC OP_CODE, " +
				"A.RBC, A.PLASMA, A.PLATE, A.WHOLE_BLOOD, A.OTH_BLOOD " +
				"FROM MRO_RECORD A, SYS_DEPT C, SYS_OPERATIONICD E " +
				"WHERE 1=1  AND A.OUT_DEPT = C.DEPT_CODE " +
...
//		System.out.println("sql====:"+sql);
	...
	public void onExport() {
		if (table.getRowCount() <= 0){
			...
			ExportExcelUtil.getInstance().exportExcel(table, "各外科按病种术式平均用血统计表");
		}
	}
