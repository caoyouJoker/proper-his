...
//        System.out.println("申报的rstParm:"+rstParm);
        //数据库是否有记录
...
//        System.out.println("申报的caseNo:"+caseNo);
...
//        System.out.println("flgParm===" + flgParm.getValue("CASE_NO"));
        ...
        if (insPayType.equals("1")||
           (insPayType.equals("2")&&localFlg.equals("N"))) {//本地城职 、异地城职、异地城居
        ...
        } else if(insPayType.equals("2")&&localFlg.equals("Y")){ //本地城居
            if (singleFlg.equals("2")) { //单病种
...
//                           parm.getValue("YEAR_MON", 0).substring(0, 6));
    ...
    public TParm DataDown_zjks_A1(TParm parm,String type) {
    	 ...
    	 if(type.equals("CZ"))
    		ParmData.addData("INS_TYPE", "310");//险种
          else if(type.equals("CJ"))
        	ParmData.addData("INS_TYPE", "390");//险种
    	 ParmData.addData("HOSP_NHI_NO", this.nhi_hosp_code);//医院编码
    	 ParmData.addData("ADM_SEQ", parm.getData("ADM_SEQ", 0));//就医顺序号
    	 ParmData.addData("CONFIRM_NO", parm.getData("CONFIRM_NO", 0));//资格确认书编号
    	 ParmData.addData("OWN_NO", parm.getData("PERSONAL_NO", 0));//个人编码
    	 ParmData.addData("ID_NO", parm.getData("IDNO", 0));//身份证号码
    	 ParmData.addData("PAT_NAME", parm.getData("PAT_NAME", 0));//姓名
    	 ParmData.addData("SEX_CODE", parm.getData("SEX_CODE", 0));//性别
         ParmData.addData("AGE", parm.getData("PAT_AGE", 0));//年龄
    	 ParmData.addData("BIRTH_DATE", parm.getData("BIRTH_DATE", 0));//出生日期
    	 ParmData.addData("ICD_CODE", parm.getData("ICD_CODE", 0));//致死的主要疾病诊断
    	 ParmData.addData("ICD_DESC", parm.getData("ICD_DESC", 0));//疾病名称
    	 ParmData.addData("PARM_COUNT", 12);//入参数量
...
//        System.out.println("age3====:"+age3);
         ...
         mroParm.addData("OE_DIAG_CODE", oediagcode);//门（急）诊疾病编码
...
//        System.out.println("result" + result);
    ...
    public TParm DataUpload_G1(TParm parm,String type) {
    	TParm result = new TParm();
         ...
         mroParm.addData("ADM_SEQ", parm.getData("ADM_SEQ", 0));//就医顺序号
         mroParm.addData("HOSP_NHI_NO", this.nhi_hosp_code);//医院代码
