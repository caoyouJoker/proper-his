 ...
 * </p>
 * 
 * @author liling 20140825
 * @version 1.0
 */
public class CLPBscInfoControl extends TControl {
	/**
	 * 构造方法
	 */
	public CLPBscInfoControl() {
	}
	...
	private String clpStatus = "";// 路径使用状态liling 20140822 add
	private TParm clpPack;// 当前使用的临床路径医嘱liling 20140822 add
	private String clpStatusSQL = "SELECT CLP_STATUS FROM CLP_BSCINFO WHERE CLNCPATH_CODE='@' ";// 路径使用状态查询sql
																								// liling
																								// 20140827add
	// 选项卡的页签控件
	...
	// 时程金额
	private TNumberTextField SCHD_AMT;
	// 治疗时程 CLP_THRPYSCHDM
	...
	// 是否启用
	private TComboBox ACTIVE;
	private TTable CLP_PACKNEW;
	/**
	 * 第四个页签的控件 关键诊疗套餐
	 */
	// 路径项目
	...
	/** CLP_THRPYSCHDM */
	public static final String CLP_THRPYSCHDM_PARMMAP = "SCHD_DESC;SUSTAINED_DAYS;SCHD_DAY;SCHD_CODE;SCHD_AMT";
	/** 斩断table名称 */
	...
	TRadioButton DEPT_TYPE_N;//内科
	TRadioButton DEPT_TYPE_W;//外科
	/**
	 * 初始化方法 初始化查询全部
	 */
	public void onInit() {
		...
		// System.out.println("slelectParm+++"+slelectParm);
		TParm allBscInfos = BscInfoTool.getInstance().getAllBscInfos(
				slelectParm);
		// System.out.println("allBscInfos+++"+allBscInfos);
		// 卢海修改 end
		// 显示在前台界面
		// System.out.println("CLP_BSCINFO+++"+CLP_BSCINFO.getParmValue());
		CLP_BSCINFO.setParmValue(allBscInfos);
		// 设置默认修改日期
		// System.out.println("CLP_BSCINFO++11111+"+CLP_BSCINFO.getParmValue());
		this.setValue("LASTVRSN_DATE", getDateStr("yyyy/MM/dd"));
		// 诊断和手术诊断的Table初始化和注册监听事件
		initDiagTables();
		if (!this.getPopedem("CLPLEAD")) {// 组长权限，yanjing 20140924
			callFunction("UI|revise|setEnabled", false);
			callFunction("UI|release|setEnabled", false);
		}
		onSelPack();
	}
	...
	public void getAllComponent() {
		...
		tTabbedPane_0.setEnabledAt(3, false);// 第四页签 关键诊疗 置灰
		tTabbedPane_0.setEnabledAt(4, false);// 第五页签 护理计划 置灰
		// 选项卡的页签控件
		...
		this.SCHD_AMT = (TNumberTextField) this.getComponent("SCHD_AMT");
		...
		this.ACTIVE = (TComboBox) this.getComponent("ACTIVE");
		this.CLP_PACKNEW = (TTable) this.getComponent("CLP_PACKNEW");// yanjing
																		// 导出医嘱细项用
		/**
		...
		DEPT_TYPE_N=(TRadioButton)this.getComponent("DEPT_TYPE_N");
		DEPT_TYPE_W=(TRadioButton)this.getComponent("DEPT_TYPE_W");
	}
	...
	public void initControler() {
		...
		callFunction("UI|CLP_PACK03|addEventListener", "CLP_PACK03->"
		 ...
		 ( (TTable) getComponent("CLP_PACK01")).addEventListener(TTableEvent.CHECK_BOX_CLICKED, this,
			"onCheckBox");
		// 诊断ICD
		...
		// ==liling 20140910 add 医嘱页签查询条件ACTIVE 是否启用的监听事件
		addEventListener("ACTIVE->" + TComboBoxEvent.SELECTED, this, "onQuery");
	}
	...
	public void onTableClickedForBscInfo(int row) {
		...
		this.setValue("CLP_TYPE", CLP_BSCINFO.getParmValue().getValue("CLP_TYPE",row));//路径类型，溢出校验使用
		if (CLP_BSCINFO.getParmValue().getValue("DEPT_TYPE_W",row).equals("Y")) {//科室赋值，溢出校验使用
			DEPT_TYPE_W.setSelected(true);
		}else{
			DEPT_TYPE_N.setSelected(true);
		}
		// 诊断
	...
	public void onTableClickedForThrpyschdm(int row) {
		...
		SCHD_AMT.setValue(nullToString(String.valueOf(CLP_THRPYSCHDM
				.getItemData(row, "SCHD_AMT"))));
	}
	...
	public void onTableClickedForPack01(int row) {
		...
		if (version == null || "".equals(version.trim())) {
			...
			if (column == 0) {
				...
				// 套餐名称 医嘱类别 临床路径类别 医嘱名称 核查类别可编辑
				if (column == 2 || column == 3 || column == 4 || column == 5 || column == 19) {
				...
				// 卢海删除
				// column == 1 ||
				if (column == 6 || column == 13 || column == 14 ||column == 15 || column == 25|| column == 26
			...
			if (column == 0) {
				...
				if (column == 4 || column == 5 || column == 6
						|| column == 13 || column == 14|| column == 15|| column == 25|| column == 26
						|| column == 27|| column == 28|| column == 29|| column == 30) {
	...
	public void onTableClicked() {
		if (!"1".equals(clpStatus)) {
			this.messageBox("请先操作修订");
			return;
		}
	...
	public void onCreatePACK01(Component com, int row, int column) {
		...
		if (column == 5) {
			...
			textFilter.setPopupMenuParameter("ORDER", getConfigParm()
	...
	public void setCLP_PACK01(TParm parm) {
		...
		// System.out.println("OWN_PRICE+++  :"+parm.getDouble("OWN_PRICE"));
		// 完整信息
		...
		result.setData("SCHD_CODE", SCHD_CODE.getValue());
		// 医嘱类别
		...
		if ((checkInputString(ordTypeCode) && checkInputString(typeChnDesc))
			...
			// String dose = phaResult.getValue("MEDI_QTY", 0);//
			// old:DEFAULT_TOTQTY
			double dose = phaResult.getDouble("MEDI_QTY", 0);// liling
			...
			if ("PHA".equals(cat1Type)) {
				...
				/** 非药品类医嘱（其它） */
				// 用量result.setData("DOSE", "1");
				result.setData("DOSE", 1);// liling
				// 单位
			...
			result.setData("OWN_PRICE", parm.getDouble("OWN_PRICE"));
			// 总金额（当前记录）
		...
		// 接收到医嘱代码，没有接受到临床路径项目代码（即选择医嘱名称，反查询临床路径项目）
			...
			// 临床路径项目
			result.setData("TYPE_CHN_DESC", typeChnDesc);// CHKTYPE_CODE
															// 核查类别有问题，关键诊疗
			// 医嘱名称
			String orderDesc = parm.getValue("ORDER_CHN_DESC");
			if (orderDesc.length() <= 0) {
				orderDesc = parm.getValue("ORDER_DESC");
			}
			result.setData("ORDER_CHN_DESC", orderDesc);
			// 用量 频次 单位 途径（用法）
			TParm phaResult = PackTool.getInstance()
			...
			result.setData("OWN_PRICE", parm.getDouble("OWN_PRICE"));
			// 总金额（当前记录）
			result.setData("FEES", parm.getDouble("OWN_PRICE"));
			// 医嘱备注 手动输入
			result.setData("NOTE", "");
			// 执行人员
			result.setData("CHKUSER_CODE", "001");
	...
	private void addTotalAndTotalPrice(TParm result) {
		...
		result.setData("FEES", totParm.getDouble("QTY")
				* result.getDouble("OWN_PRICE"));
	}
	...
	public void setCLP_PACK02(TParm parm) {
		...
		if ((checkInputString(ordTypeCode) && checkInputString(typeChnDesc))
			...
			result.setData("DOSE", basicResult.getDouble("CLP_QTY", 0));// liling
			// 单位
	...
	public void setCLP_PACK03(TParm parm) {
		...
		if ((checkInputString(ordTypeCode) && checkInputString(typeChnDesc))
			...
			result.setData("ORDER_CHN_DESC", orderDesc);
			// 用量
			result.setData("DOSE", parm.getDouble("AMOUNT"));// liling
			// 单位
	...
	public void onTableChangeValueForPack01(Object obj) {
		...
		// 用量 频次 日份 列触发
		if (column == 6 || column == 8 || column == 12) {
			double dose = 0.0;
			TParm tableParm = CLP_PACK01.getParmValue();
			...
			if (column == 6) {
				...
				// CLP_PACK01.setItem(row, "DOSAGE_QTY", node.getValue());//
				// 发药单位
	...
	public TParm getTotalAndFees(TParm parm, int row) {
		...
		parm.setData("FEES", row, StringTool.round(dose
				* parm.getRow(row).getDouble("OWN_PRICE"), 2));// modify by
																// wanglong
																// 20120924
		parm.setData("FEES_REAL", row, StringTool.round(dose
				* parm.getRow(row).getDouble("OWN_PRICE_REAL"), 2));// modify by
																	// wanglong
																	// 20120924
	...
	public void onQuery() {
		...
		if (selectedIndex == 0) {
			...
			// 显示在前台界面
			// System.out.println("bscInfoList+++++"+bscInfoList);
			...
			parm.setData("CLNCPATH_CODE", CLNCPATH_CODE.getValue());
			// 模糊查询治疗时程
			// System.out.println("parm+++治疗时程"+parm.getData());
			TParm thrpyschdmList = ThrpyschdmTool.getInstance()
					.getThrpyschdmList(parm);
			// System.out.println("parm+++治疗时程qiantai"+thrpyschdmList.getData());
			// 显示在前台界面
			CLP_THRPYSCHDM.setParmValue(thrpyschdmList);// 治疗时程
			...
			parm.setData("ORDER_FLG", "Y");
			if (getActiveFlg(CLNCPATH_CODE.getValue())) {// 路径已经停用
				parm.setData("ACTIVE_FLG", "N");
			} else {
				parm.setData("ACTIVE_FLG", "Y");// 启用
			}
			// 模糊查询医嘱套餐
			TParm pack01List = new TParm();
			if ("1".equals(clpStatus)) {// 修订状态下 第三个页签医嘱界面显示历史表的数据
				parm.setData("VERSION", "0");
				pack01List = PackTool.getInstance().getPackHistoryList(parm);// 启用时修订状态（单价取sys_fee表）
			} else {// 非修订状态下 第三个页签医嘱界面显示正在使用的临床路径项目医嘱
				String active = ACTIVE.getValue();
				if (active.equals("ALL")) {
					active = "N','Y";
				} else {
					active = ACTIVE.getValue();
				}
				parm.setData("ACTIVE", active);
				// System.out.println("active+++: "+active);
				pack01List = PackTool.getInstance().getPack01List(parm);// 启用时非修订状态（单价取sys_fee表）
			}
			for (int i = 0; i < pack01List.getCount(); i++) {
				// ============pangben 2012-6-25 集合医嘱细项显示金额 ORDERSET_FLG=Y
				// 为集合医嘱主项
				TParm result = pack01List.getRow(i);
				if (null != result.getValue("CAT1_TYPE")
						&& "Y".equals(result.getValue("ORDERSET_FLG"))) {
					double allOwnAmt = 0.00;
					double allOnwAmtReal = 0.00;
					...
					for (int j = 0; j < parmDetail.getCount(); j++) {
						allOwnAmt = allOwnAmt
								...
								* parmDetail.getDouble("TOTQTY", j);
						allOnwAmtReal = allOnwAmtReal
								+ parmDetail.getDouble("OWN_PRICE", j)
								* parmDetail.getDouble("TOTQTY", j);
					}
					pack01List.setData("OWN_PRICE", i, allOwnAmt);
					pack01List.setData("OWN_PRICE_REAL", i, allOwnAmt);
					// result.setData("OWN_PRICE",allOwnAmt);
				}
				getTotalAndFees(pack01List, i);
			}
			// 显示在前台界面
			// System.out.println("========pack01List pack01List is ::"+pack01List);
			CLP_PACK01.setParmValue(pack01List);
			insertNewRow(CLP_PACK01, CLP_PACK01.getParmValue().getCount() - 1);
			CLP_PACKNEW.setParmValue(pack01List);
	...
	public void onChange() {
		...
		if (selectedIndex == 2) {
			...
			// ==liling 20140822 modify start====
			String sql = clpStatusSQL.replaceAll("@", CLNCPATH_CODE.getValue());
			TParm result = new TParm(TJDODBTool.getInstance().select(sql));
			clpStatus = result.getValue("CLP_STATUS", 0);
			if ("1".equals(clpStatus)) {// 修订中
	...
	public TParm parmFormatForBscInfo() {
		...
		parm.setData("CLP_TYPE", this.getValue("CLP_TYPE"));//==pangben 2015-8-12 添加路径类别，医生站溢出校验使用
		parm.setData("DEPT_TYPE_N",this.getValue("DEPT_TYPE_N"));//==pangben 2015-8-12 添加科室类别，医生站溢出校验使用
		parm.setData("DEPT_TYPE_W",this.getValue("DEPT_TYPE_W"));//==pangben 2015-8-12 添加科室类别，医生站溢出校验使用
		return parm;
	}
	...
	public TParm parmFormatForThrpyschdm() {
		...
		parm.setData("SCHD_AMT", SCHD_AMT.getValue());
		parm.setData("OPT_USER", Operator.getID());
		parm.setData("OPT_DATE", dateFormat.format(SystemTool.getInstance()
				.getDate()));
		parm.setData("OPT_TERM", Operator.getIP());
	...
	public boolean saveBscInfo(TParm parm) {
		...
		if (count <= 0) {
			...
			// parm.setData("ACTIVE_FLG",ACTIVE_FLG.getValue());
			parm.setData("ACTIVE_FLG", "Y");// 创建即启用
			parm.setData("VERSION", "1");
			...
			// System.out.println("saveParm22222"+saveParm);
			TParm result = TIOM_AppServer.executeAction(
					"action.clp.CLPBscInfoAction", "insertBSCInfo", saveParm);
			// System.out.println("saveParm333333"+parm);
			// TParm result1 = TIOM_AppServer.executeAction(
			// "action.clp.CLPBscInfoAction", "insertBSCInfoHis", parm);
			...
			TParm saveParm = new TParm();
			saveParm.setData("bscinfoParm", parm.getData());
			// System.out.println("bscinfoParm++"+ parm.getData());
			saveParm.setData("icdParm", this.diagnoseTable.getParmValue()
					.getData());
			saveParm.setData("optParm", this.operatorDiagnoseTable
					.getParmValue().getData());
			saveParm.setData("basicInfo", this.getBasicOperatorMap());
			// System.out.println("saveParm444444"+saveParm);
			TParm result = TIOM_AppServer.executeAction(
	...
	public boolean checkThrpyschdm() {
		...
		double schdAMt = Double.parseDouble(SCHD_AMT.getValue().toString());
		if (SCHD_AMT.getValue() == null || schdAMt == 0) {
			this.messageBox("请输入时程金额！");
			return false;
		}
	...
	public boolean checkPack01(TParm detail) {
		...
		TParm parm = new TParm();
		parm.setData("FREQ_CODE", detail.getValue("FREQ_CODE"));
		parm.setData("STAT_FLG", orderType.equalsIgnoreCase("ST") ? "Y"
				: "N");
		TParm result = BscInfoTool.getInstance().checkFreq(parm);
		if (result.getCount() <= 0) {
	...
	public TParm parmFormatPackDetail(TTable table, int row, TParm parm) {
		...
		detail.setData("DOSE", data.getDouble("DOSE", row));// liling
		...
		detail.setData("PACK_CODE", data.getValue("PACK_CODE", row));//===pangben 2015-8-12 套餐名
		detail.setData("SEL_FLG", data.getValue("SEL_FLG", row));
		return detail;
	}
	...
	public void onSave() {
		...
		if (selectedIndex == 0) {
			...
			// System.out.println("saveParm111111"+parm);
		...
		if (selectedIndex == 2) {
			...
			parm.setData("CLP_BSCINFO", getUseParm(ACTIVE_FLG.getValue())
					.getData());
			...
			if (num <= 0) {
				this.messageBox("请勾选要保存的医嘱");
				return;
			} else {
				Map tableMap = (Map) parm.getData("CLP_PACK");
				TParm tableParm = new TParm(tableMap);
				// System.out.println("tableParm++ :"+tableParm.getCount());
				// System.out.println("parm.getData(CLP_PACK)++"+parm.getData("CLP_PACK"));
				TParm result = new TParm();
				if (this.messageBox("询问", "是否保存？", 2) == 0) {
					result = TIOM_AppServer.executeAction(
							"action.clp.CLPBscInfoAction", "savePackHis", parm);
					// 判断错误值
					if (result.getErrCode() < 0) {
	...
	private String getEndDate() {
		String endDate = SystemTool.getInstance().getDate().toString()
		...
		if (selectedIndex == 0) {
			...
			if (CLP_BSCINFO.getRowCount() > 0 && selectedRow >= 0) {
				if (this.messageBox("询问", "是否删除？", 2) == 0) {
					String sql = "SELECT START_DATE FROM CLP_BSCINFO_HISTORY WHERE CLNCPATH_CODE='"
							+ CLNCPATH_CODE.getValue()
							+ "' ORDER BY START_DATE DESC ";
					// System.out.println("sql++888888888++"+sql);
					TParm result = new TParm(TJDODBTool.getInstance().select(
							sql));
					String startDate = result.getValue("START_DATE", 0);
					TParm parm = new TParm();
					parm.setData("REGION_CODE", REGION_CODE.getValue());
					parm.setData("CLNCPATH_CODE", CLNCPATH_CODE.getValue());
					parm.setData("VERSION", VERSION.getValue());// ==liling
					parm.setData("START_DATE", startDate);// ==liling
					endDate = getEndDate();
			...
			if (parm.getCount("REGION_CODE") <= 0) {
				this.messageBox("请勾选要删除的医嘱");
				return;
			} else {
				if (this.messageBox("询问", "是否删除？", 2) == 0) {
					// for (int i = 0; i < parm.getCount("REGION_CODE"); i++) {
					// System.out.println("delete control++  :"+i+"==="+parm.getRow(i));
					// }
					// System.out.println("parm.getCount()++  :"+parm.getCount("REGION_CODE"));
					TParm result = TIOM_AppServer.executeAction(
							"action.clp.CLPBscInfoAction", "deletePackHis",
							parm);
					// 判断错误值
					if (result.getErrCode() < 0) {
						err("ERR:" + result.getErrCode() + result.getErrText()
	...
	public boolean checkStayHospDays(String clncPathCode) {
		...
		int stayHospDays = Integer.parseInt(STAYHOSP_DAYS.getValue());
		if (schdDay != stayHospDays) {
			// this.messageBox("时程总天数和标准住院天数不一致,是否继续?");
			if (this.messageBox("提示信息 Tips", "时程总天数和标准住院天数不一致,是否继续? ",
					this.YES_NO_OPTION) != 0) {// 否
				this
						.setValue("ACTIVE_FLG", !this
								.getValueBoolean("ACTIVE_FLG"));
				return false;
			}
			this.setValue("ACTIVE_FLG", this.getValueBoolean("ACTIVE_FLG"));
			return true;
		}
		return true;
	}
	...
	/**
	 * 修订 liling 20140822 add 查询当前使用的临床路径医嘱 SELECT * FROM CLP_PACK WHERE XXX
	 * 修改历史表中当前使用的临床路径医嘱结束时间 UPDATE CLP_ PACK_HISTORY SET END_DATE =当前时间
	 * WHERE版本号=XXX AND XXX 将医嘱添加到历史表,开始时间为当前时间，结束时间为99991231235959 ， INSERT
	 * CLP_ PACK_HISTORY（XXX... 版本号码，VALUES(XXX,'0',..)
	 */
	public void onRevise() {
		// 校验已经停用的临床路径不可操作修订
		if (getActiveFlg(CLNCPATH_CODE.getValue())) {
			this.messageBox("已停用的路径不可操作修订。");
			return;
		}
		TParm parm = new TParm();
		TParm result = new TParm();
		if (!"".equals(CLNCPATH_CODE.getValue().trim())
				&& null != CLNCPATH_CODE.getValue()) {
			// 查询是否在修订
			String sql = clpStatusSQL.replaceAll("@", CLNCPATH_CODE.getValue());
			result = new TParm(TJDODBTool.getInstance().select(sql));
			clpStatus = result.getValue("CLP_STATUS", 0);
			if ("1".equals(clpStatus)) {// 修订中
				this.messageBox("已修订未发布");
				return;
			}
			// 2查询当前使用的临床路径医嘱 "'  AND SCHD_CODE='"+SCHD_CODE01.getValue()+
			sql = "SELECT * FROM CLP_PACK WHERE  CLNCPATH_CODE='"
					+ CLNCPATH_CODE.getValue() + "' AND VERSION='"
					+ VERSION.getValue() + "' ";
			clpPack = new TParm(TJDODBTool.getInstance().select(sql));
			if (clpPack.getCount() <= 0) {
				this.messageBox("没有获得需要修订的医嘱,请确认版本号码是否正确");
				return;
			}
			// 将医嘱添加到历史表,开始时间为当前时间，结束时间为99991231235959
			parm.setData("CLP_PACK", clpPack.getData());
			String now = getEndDate();
			parm.setData("END_DATE", now);
			parm.setData("CLNCPATH_CODE", CLNCPATH_CODE.getValue());
			parm.setData("VERSION", VERSION.getValue());
			if (this.messageBox("询问", "是否修订？", 2) == 0) {
				result = TIOM_AppServer.executeAction(
						"action.clp.CLPBscInfoAction", "onRevise", parm);
				// 判断错误值
				if (result.getErrCode() < 0) {
					err("ERR:" + result.getErrCode() + result.getErrText()
							+ result.getErrName());
					this.messageBox("修订失败！");
					return;
				} else {
					this.messageBox("修订成功！");
					onChange();
					return;
				}
			}
			/*
			 * //1更新路径使用状态为修订中
			 * sql="UPDATE CLP_BSCINFO SET CLP_STATUS ='1'  WHERE CLNCPATH_CODE='"
			 * +CLNCPATH_CODE.getValue()+"' ";
			 * TJDODBTool.getInstance().update(sql);
			 * System.out.println("sql+11111111++"+sql); //2查询当前使用的临床路径医嘱
			 * "'  AND SCHD_CODE='"+SCHD_CODE01.getValue()+
			 * sql="SELECT * FROM CLP_PACK WHERE  CLNCPATH_CODE='"
			 * +CLNCPATH_CODE.
			 * getValue()+"' AND VERSION='"+VERSION.getValue()+"' "; clpPack =
			 * new TParm(TJDODBTool.getInstance().select(sql));
			 * System.out.println("sql+22222222++"+sql);
			 * System.out.println("clpPack++"+clpPack);
			 * //将医嘱添加到历史表,开始时间为当前时间，结束时间为99991231235959 parm=clpPack; for(int
			 * i=0;i<parm.getCount();i++){ insertpackHis(parm.getRow(i)); }
			 * String now=getEndDate(); //3修改历史表中当前使用的临床路径医嘱结束时间
			 * sql="UPDATE CLP_PACK_HISTORY  SET END_DATE ='"
			 * +now+"'  WHERE VERSION='"+VERSION.getValue()
			 * +"' AND CLNCPATH_CODE='"+CLNCPATH_CODE.getValue()+"' "; result =
			 * new TParm(TJDODBTool.getInstance().update(sql));
			 * System.out.println("sql+3333333333++"+sql);
			 */
		}
	}

	private boolean getActiveFlg(String clncPathCode) {
		boolean flg = false;
		String sql = "SELECT ACTIVE_FLG FROM CLP_BSCINFO WHERE CLNCPATH_CODE = '"
				+ clncPathCode + "' ";
	...
	// private void insertpackHis(TParm parm){
	// BscInfoTool.getInstance().insertPackHistory(parm);
	// }

	/**
	 * 发布 删除当前正在使用的医嘱数据 DELETE FROM CLP_ PACK WHERE 版本号=XXX AND XXX
	 * 将历史表中版本号码=0的医嘱添加到CLP_ PACK，同时同步当前版本号码(CLP_PACK当前版本号码+1) UPDATE
	 * CLP_PACK_HISTORY SET 当前版本号码=XXX WHERE 版本号码='0'
	 */
	public void onRelease() {
		if (getActiveFlg(CLNCPATH_CODE.getValue())) {
			this.messageBox("已停用的路径不可操作发布。");
			return;
		}
		TParm parm = new TParm();
		if (CLNCPATH_CODE.getValue() == null
				|| "".equals(CLNCPATH_CODE.getValue().trim())) {
			this.messageBox("请选择要发布的临床路径！");
			return;
		}
		// 查询是否在修订
		String sql = clpStatusSQL.replaceAll("@", CLNCPATH_CODE.getValue());
		TParm result = new TParm(TJDODBTool.getInstance().select(sql));
		clpStatus = result.getValue("CLP_STATUS", 0);
		// System.out.println("sql+44444444++"+sql);
		if (!"1".equals(clpStatus)) {// 未修订过
			this.messageBox("此临床路径没有修订！");
			return;
		}
		sql = "SELECT * FROM CLP_PACK_HISTORY WHERE CLNCPATH_CODE='"
				+ CLNCPATH_CODE.getValue() + "' AND VERSION='0' ";
		TParm parm1 = new TParm(TJDODBTool.getInstance().select(sql));
		parm.setData("CLP_PACK", parm1.getData());
		String version = (Integer.parseInt(VERSION.getValue()) + 1) + "";
		parm.setData("CLNCPATH_CODE", CLNCPATH_CODE.getValue());
		parm.setData("VERSION", VERSION.getValue());
		parm.setData("VERSION_NEW", version);
		if (this.messageBox("询问", "是否发布？", 2) == 0) {
			result = TIOM_AppServer.executeAction(
					"action.clp.CLPBscInfoAction", "onRelease", parm);
			// 判断错误值
			if (result.getErrCode() < 0) {
				err("ERR:" + result.getErrCode() + result.getErrText()
						+ result.getErrName());
				...
				this.messageBox("发布成功！");
				VERSION01.setValue(version);
				VERSION.setValue(version);
				onChange();
				return;
			}
	...
	/**
	 * 启用复选框 启用操作：启用勾选去掉，历史表中根据版本号查询数据，结束时间修改为当前时间，
	 * 再次启用以后，创建一条历史数据，开始时间为当前时间，结束时间为99991231235959 CLP_BSCINFO_HISTORY
	 * 表将当前使用的临床路径的结束时间修改为当前时间，再创建一条新的数据，版本号码不变 UPDATE CLP_BSCINFO_HISTORY SET
	 * END_DATE =XXX WHERE 版本号=XXX AND XXX
	 */
	public void onCheckBox() {
		// System.out.println("oncheckbox++++++");
		// 获取页签索引
		int selectedIndex = tTabbedPane_0.getSelectedIndex();
		String sql = "";
		TParm result = new TParm();
		if (selectedIndex == 0) {
			...
			if (CLP_BSCINFO.getRowCount() > 0 && selectedRow >= 0) {
				// 此临床路径已经启用
				if (!"Y".equals(ACTIVE_FLG.getValue())) {
					if (clpStatus.equals("1")) {// 1:路径为修订状态
						this.messageBox("该路径为修订状态,不可以停用。");
						this.setValue("ACTIVE_FLG", !this
								.getValueBoolean("ACTIVE_FLG"));
						return;
					}
					// System.out.println("oncheckbox+++是否停用+++");
					if (this.messageBox("询问", "是否停用？", 2) == 0) {
						sql = "SELECT START_DATE FROM CLP_BSCINFO_HISTORY WHERE CLNCPATH_CODE='"
								+ CLNCPATH_CODE.getValue()
								+ "' ORDER BY START_DATE DESC ";
						// System.out.println("sql++888888888++"+sql);
						result = new TParm(TJDODBTool.getInstance().select(sql));
						startDate = result.getValue("START_DATE", 0);
						TParm parm = new TParm();
						parm.setData("USE_PARM", getUseParm("N").getData());
						parm.setData("END_DATE", now);
						parm.setData("CLNCPATH_CODE", CLNCPATH_CODE.getValue());
						parm.setData("VERSION", VERSION.getValue());
						parm.setData("START_DATE", startDate);
						result = TIOM_AppServer.executeAction(
								"action.clp.CLPBscInfoAction", "onStop", parm);
						/*
						 * sql="UPDATE CLP_BSCINFO_HISTORY  SET END_DATE ='"+now+
						 * "'  WHERE VERSION='"+VERSION.getValue()
						 * +"' AND CLNCPATH_CODE='"
						 * +CLNCPATH_CODE.getValue()+"' AND START_DATE='"
						 * +startDate+"' ";
						 * System.out.println("sql++9999999999++"+sql);
						 * TJDODBTool.getInstance().update(sql); result =
						 * BscInfoTool.getInstance().use( getUseParm("N"));
						 */
						if (result.getErrCode() >= 0) {
							// 清空以前信息
							onClear();
					...
					// this.messageBox("NNN");
					// System.out.println("oncheckbox+++是否启用+++");
					TParm parm = parmFormatForBscInfo();
					// 处理最近修订日期和修订次数 luhai Modify 20110630
					if (!this.checkNullAndEmpty(parm.getValue("LASTVRSN_DATE"))) {
						parm.setData("LASTVRSN_DATE", parm
								.getData("FRSTVRSN_DATE"));
					}
					...
					parm.setData("SEQ", seqParm.getInt("SEQ", 0));
					parm.setData("VERSION", VERSION.getValue());
					parm.setData("USE_PARM", getUseParm("Y").getData());
					// System.out.println("parm+++"+parm);
					if (this.messageBox("询问", "是否启用？", 2) == 0) {
						result = TIOM_AppServer.executeAction(
								"action.clp.CLPBscInfoAction", "onUse", parm);
						/*
						 * TParm result1 = TIOM_AppServer.executeAction(
						 * "action.clp.CLPBscInfoAction", "insertBSCInfoHis",
						 * parm); result = BscInfoTool.getInstance().use(
						 * getUseParm("Y"));
						 */
						if (result.getErrCode() >= 0) {
							// 清空以前信息
	...
	public void onExport() {
		...
		case 0:
			ExportExcelUtil.getInstance().exportExcel(CLP_BSCINFO, "临床路径类别");
			break;
		case 1:
			ExportExcelUtil.getInstance().exportExcel(CLP_THRPYSCHDM, "治疗时程");
			break;
		case 2:
			CLP_PACKNEW.setParmValue(CLP_PACK01.getParmValue());
			getOrderDetial();
			// ExportExcelUtil.getInstance().exportExcel(CLP_PACK01,
			// "医嘱套餐-类别:"+this.getText("CLNCPATH_CODE02")+" 时程:"+this.getText("SCHD_CODE01"));
			ExportExcelUtil.getInstance().exportExcel(CLP_PACKNEW, "医嘱套餐");
			// ExportExcelUtil.getInstance().exportExcel(CLP_PACK01, "ll");
			break;
		}
	}

	/**
	 * 取得集合医嘱的细项并整合到table中 yanjing 20140926
	 */
	private void getOrderDetial() {
		int m = 0;
		TParm tableParmNew = new TParm();
		TParm tableParm = CLP_PACKNEW.getParmValue();
		CLP_PACKNEW.setParmValue(tableParm);
		// System.out.println("表格数据内容  tableParm is：："+tableParm);
		// System.out.println("www表格数据内容  tableParm is：："+tableParm.getCount());
		if (tableParm.getCount() > 0) {// 循环校验是否是集合医嘱
			for (int i = 0; i < tableParm.getCount(); i++) {
				tableParmNew.setRowData(m, tableParm.getRow(i));
				m++;
				// System.out.println("00000orderCode orderCode is ::"+tableParm.getValue("ORDERSET_FLG",
				// i));
				if (tableParm.getValue("ORDERSET_FLG", i).equals("Y")) {// 集合医嘱将细项插入相应的位置
					String orderCode = tableParm.getValue("ORDER_CODE", i);// 得到集合医嘱主项的医嘱代码
					// System.out.println("1111orderCode orderCode is ::"+orderCode);
					TParm parmDetail = SYSOrderSetDetailTool.getInstance()
							.selectByOrderSetCode(orderCode);
					// System.out.println("2222parmDetail parmDetail is ::"+parmDetail);
					for (int j = 0; j < parmDetail.getCount(); j++) {
						tableParmNew.setData("SEL_FLG", m, tableParm.getValue(
								"SEL_FLG", i));// 选
						tableParmNew.setData("SCHD_CODE", m, tableParm
								.getValue("SCHD_CODE", i));// 时程
						tableParmNew.setData("SEQ", m, tableParm.getValue(
								"SEQ", i));// 序号
						tableParmNew.setData("PACK_CODE", m, tableParm
								.getValue("PACK_CODE", i));// 套餐
						tableParmNew.setData("ORDER_TYPE", m, tableParm
								.getValue("ORDER_TYPE", i));// 医嘱类别
						tableParmNew.setData("TYPE_CHN_DESC", m, tableParm
								.getValue("TYPE_CHN_DESC", i));// 临床路径项目
						tableParmNew.setData("ORDER_CHN_DESC", m, parmDetail
								.getValue("ORDER_DESC", j));// 医嘱名称
						tableParmNew.setData("ACTIVE_FLG", m, tableParm
								.getValue("ACTIVE_FLG", i));// 启
						tableParmNew.setData("DOSE", m, parmDetail.getDouble(
								"TOTQTY", j)*tableParm.getDouble("DOSE", i));// 用量  modify by kangy 20170103
						tableParmNew.setData("DOSE_UNIT", m, tableParm
								.getValue("DOSE_UNIT", i));// 单位
						tableParmNew.setData("FREQ_CODE", m, tableParm
								.getValue("FREQ_CODE", i));// 频次
						tableParmNew.setData("ROUT_CODE", m, tableParm
								.getValue("ROUT_CODE", i));// 途径
						tableParmNew.setData("START_DAY", m, tableParm
								.getValue("START_DAY", i));// 第几天
						tableParmNew.setData("DOSE_DAYS", m, tableParm
								.getValue("DOSE_DAYS", i));// 日份
						tableParmNew.setData("TOTAL", m, parmDetail.getDouble(
								"TOTQTY", j)*tableParm.getDouble("TOTAL", i));// 总量modify by kangy 20170103
						tableParmNew.setData("OWN_PRICE", m, parmDetail
								.getValue("OWN_PRICE", j));// 单价
						// tableParmNew.setData("FEES", m,
						// tableParm.getValue("FEES", i));//总金额
						tableParmNew.setData("FEES", m, parmDetail.getDouble(
								"OWN_PRICE", j)* parmDetail.getDouble("TOTQTY", j)*tableParm.getDouble("TOTAL", i));// 总金额 modify by kangy 20170103
						tableParmNew.setData("NOTE", m, tableParm.getValue(
								"NOTE", i));// 医嘱备注
						tableParmNew.setData("CHKUSER_CODE", m, tableParm
								.getValue("CHKUSER_CODE", i));// 执行人员
						tableParmNew.setData("RBORDER_DEPT_CODE", m, tableParm
								.getValue("RBORDER_DEPT_CODE", i));// 执行科室
						tableParmNew.setData("CHKTYPE_CODE", m, tableParm
								.getValue("CHKTYPE_CODE", i));// 核查类别
						tableParmNew.setData("URGENT_FLG", m, tableParm
								.getValue("URGENT_FLG", i));// 急作
						tableParmNew.setData("EXEC_FLG", m, tableParm.getValue(
								"EXEC_FLG", i));// 必执
						tableParmNew.setData("STANDARD", m, tableParm.getValue(
								"STANDARD", i));// 变异标准差(%)
						tableParmNew.setData("VERSION", m, tableParm.getValue(
								"VERSION", i));// 版本
						tableParmNew.setData("ORDER_SEQ_NO", m, tableParm
								.getValue("ORDER_SEQ_NO", i));// 顺序号
						tableParmNew.setData("ORDER_CODE", m, parmDetail
								.getValue("ORDER_CODE", j));// 医嘱代码
						tableParmNew.setData("ORDTYPE_CODE", m, tableParm
								.getValue("ORDTYPE_CODE", i));// 类型代码
						tableParmNew.setData("CAT1_TYPE", m, tableParm
								.getValue("CAT1_TYPE", i));// 收费类型
						tableParmNew.setData("MEDI_UNIT", m, tableParm
								.getValue("MEDI_UNIT", i));// 用量单位
						tableParmNew.setData("DISPENSE_UNIT", m, tableParm
								.getValue("DISPENSE_UNIT", i));// 配药单位
						m++;
					}
				}
			}
		}
		CLP_PACKNEW.setParmValue(tableParmNew);
		// System.out.println("输出新的parm数据看看 is：："+tableParmNew);
	}

	/**
	 * 全部导出EXCEL
	 */
	public void onAllExport() {
		TParm parm = new TParm();
		// 获取查询条件，组装参数对象
		parm.setData("REGION_CODE", this.regionCode);
		parm.setData("CLNCPATH_CODE", CLNCPATH_CODE.getValue());
		...
		// 模糊查询医嘱套餐
		TParm pack01List = new TParm();
		// System.out.println("clpStatus clpStatus is ::"+clpStatus);
		if ("1".equals(clpStatus)) {// 修订状态下 第三个页签医嘱界面显示历史表的数据
			parm.setData("VERSION", "0");
			pack01List = PackTool.getInstance().getPackHistoryList(parm);// 启用时修订状态（单价取sys_fee表）
		} else {// 非修订状态下 第三个页签医嘱界面显示正在使用的临床路径项目医嘱
		...
		// System.out.println("111222333pack01List pack01List is ::"+pack01List);
		for (int i = 0; i < pack01List.getCount(); i++) {
			pack01List = getTotalAndFees(pack01List, i);
		}

		CLP_PACKNEW.setParmValue(pack01List);
		getOrderDetial();
		ExportExcelUtil.getInstance().exportExcel(CLP_PACKNEW, "全部时程医嘱套餐");
	}

	/**
	 * 启用 启用操作：启用勾选去掉，历史表中根据版本号查询数据，结束时间修改为当前时间，
	 * 再次启用以后，创建一条历史数据，开始时间为当前时间，结束时间为99991231235959 CLP_BSCINFO_HISTORY
	 * 表将当前使用的临床路径的结束时间修改为当前时间，再创建一条新的数据，版本号码不变 UPDATE CLP_BSCINFO_HISTORY SET
	 * END_DATE =XXX WHERE 版本号=XXX AND XXX
	 */
		...
		if (selectedIndex == 0) {
			...
			if (CLP_BSCINFO.getRowCount() > 0 && selectedRow >= 0) {
				...
				if ("Y".equals(ACTIVE_FLG.getValue())) {
					...
					// System.out.println("sql++9999999999++"+sql);
					TJDODBTool.getInstance().update(sql);
					result = BscInfoTool.getInstance().use(getUseParm("N"));
					if (result.getErrCode() >= 0) {
						// 清空以前信息
						onClear();
						// 模糊查询时程
					...
					TParm result1 = TIOM_AppServer.executeAction(
							...
							parm);
					if (this.messageBox("询问", "是否启用？", 2) == 0) {
						result = BscInfoTool.getInstance().use(getUseParm("Y"));
						if (result.getErrCode() >= 0
								&& result1.getErrCode() >= 0) {
							// 清空以前信息
							onClear();
							// 模糊查询时程
							onQuery();
	 ...
	 * 
	* @Title: onSelPack
	* @Description: TODO(套餐更新复选框操作)
	* @author pangben
	* @throws
	 */
	public void onSelPack(){
		TCheckBox chk=(TCheckBox)this.getComponent("CHK_PACK_FLG");
		if (chk.isSelected()) {
			callFunction("UI|PACK_CODE|setEnabled", true);
		}else{
			this.setValue("PACK_CODE", "");
			callFunction("UI|PACK_CODE|setEnabled", false);
		}
	}
	 /**
	 * 医嘱页签选中后套餐名称赋值
    ...
    public boolean onCheckBox(Object obj) {
		...
		if ("SEL_FLG".equals(columnName)) {
			if (table.getParmValue().getBoolean("SEL_FLG",row)) {
				if (chk.isSelected()) {
					table.getParmValue().setData("PACK_CODE",row,this.getValue("PACK_CODE"));
