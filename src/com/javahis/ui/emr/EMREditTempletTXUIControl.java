 ...
 * @version 1.0
 */
public class EMREditTempletTXUIControl
    extends TControl {

    private static final String OP_USER_ID = "OPERATOR_USERID";
    private static final String OP_IP = "OPERATOR_IP";
    ...
    public void onInit() {
        ...
        medttisParm = new TParm();
        //this.messageBox("aaa"+p.getParameter());
        if (p.getParameter() != null && !p.getParameter().equals("")) {
            Vector VParm = (Vector) p.getParameter();
            //操作ID Operator
            medttisParm.addData(OP_USER_ID, VParm.get(0));
            //IP
            medttisParm.addData(OP_IP, VParm.get(1));
            //院区
            medttisParm.addData(REGION_CODE, VParm.get(2));

        }
        else {
            //操作ID Operator
            medttisParm.addData(OP_USER_ID, "000211");
            //IP
            medttisParm.addData(OP_IP, "127.0.0.1");
            //院区
            medttisParm.addData(REGION_CODE, "HIS");
    ...
    public boolean onSave() {
        ...
        if (onlyEditfileName.length() != 0) {
            ...
            //设置文件作者
            //this.messageBox(medttisParm.getValue(OP_USER_ID));
            word.setFileAuthor(medttisParm.getValue(OP_USER_ID));
            word.setFileLastEditUser(medttisParm.getValue(OP_USER_ID));
            //日期
            ...
            if (word.onSaveAs(this.saveParm.getValue("TEMPLET_PATH"),
                ...
                if (this.getOnlyType().equals("NEW")) {
                    ...
                    data.setItem(rowNumber, "OPT_USER",
                                 medttisParm.getValue(OP_USER_ID));
                    data.setItem(rowNumber, "OPT_DATE",
                                 StringTool.getTimestamp(new Date()));
                    data.setItem(rowNumber, "OPT_TERM",
                                 medttisParm.getValue(OP_IP));
    ...
    public void onNewTemplet() {
        ...
        Object obj = this.openDialog("%ROOT%\\config\\emr\\EMRTempInfoUITX.x",
                                     inParm);
        if (obj == null) {
            return;
        }
...
//        this.messageBox_(onlyParm);
        for (int i = 0; i < rowNumber; i++) {
            if (data.getRowParm(i).getValue("SUBCLASS_CODE").equals(
                subclassCode) && data.getRowParm(i).getValue("SEQ").equals(seq)) {
                data.setItem(i, "OPT_USER", medttisParm.getValue(OP_USER_ID));
                data.setItem(i, "OPT_DATE", StringTool.getTimestamp(new Date()));
                data.setItem(i, "OPT_TERM", medttisParm.getValue(OP_IP));
...
//        System.out.println(""+data.getErrText());
    ...
    public TParm getChildTreeType(String classCode) {
        TParm result = new TParm(this.getDBTool().select("SELECT SUBCLASS_CODE,EMT_FILENAME,SUBCLASS_DESC,CLASS_CODE,TEMPLET_PATH,DEPT_CODE,IPD_FLG,HRM_FLG,OPD_FLG,EMG_FLG,OIDR_FLG,NSS_FLG,RUN_PROGARM,SUBTEMPLET_CODE,CLASS_STYLE,PY1,SEQ,SYSTEM_CODE,DOCUMENT_TYPE FROM EMR_TEMPLET WHERE CLASS_CODE='" +
