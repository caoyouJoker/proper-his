 ...
 * <p>Title: 门诊收费日结</p>
 *
 * <p>Description: 门诊收费日结</p>
 *
 * <p>Copyright: Copyright (c) 2008</p>
 *
 * <p>Company: javahis</p>
 *
 * @author fudw 20090723
 * @version 1.0
 */
public class OPBAccountControl
    extends TControl {
    TTable table;
...
//                System.out.println("payaramt========"+payParm.getDouble("AR_AMT"));
                ...
                payParm.setData("ALIPAY",
                        payParm.getDouble("ALIPAY") +
                        parm.getDouble("ALIPAY", i)); //add by huangtt 20160607 支付宝支付方式
                //支付方式总计
                ...
                payTot.setData("PAY_INS",
                               ...
                               parm.getDouble("PAY_INS", i));
                payTot.setData("ALIPAY",
                        payTot.getDouble("ALIPAY") +
                        parm.getDouble("ALIPAY", i));  //add by huangtt 20160607 支付宝支付方式
            }
            else { //退费总计
                //支付方式
...
//                                            parm.getDouble("PAY_INS", i)));
                ...
                payParmBack.setData("ALIPAY",
                		(payParmBack.getDouble("ALIPAY") +
                                parm.getDouble("ALIPAY", i))); //add by huangtt 20160607 支付宝支付方式
...
//        charge2非抗生素
        ...
        payParmBack.setData("ALIPAY",
        		Math.abs(payParmBack.getDouble("ALIPAY")));
        //===zhangp 2012419 end
	...
	public void toBalacnce(String accountSeq) {
		String sql = " UPDATE BIL_OPB_RECP "
				+ " SET PAY_MEDICAL_CARD = AR_AMT"
				+ " WHERE PAY_MEDICAL_CARD + PAY_INS_CARD +PAY_OTHER1 +PAY_OTHER2+ALIPAY<> AR_AMT "
				+ " AND PAY_INS_CARD = 0 " 
				+ " AND ADM_TYPE IN ('O','E')" 
				+ " AND ACCOUNT_SEQ IN (" + accountSeq + ")";
    ...
    public void onDetailPrint() {
        ...
        TParm printData = new TParm();
            //获得医保明细
            printData = getInsDetailData(endParm);
            //表头
            printData.setData("TITLE", "TEXT","门诊收费医保明细表");
     ...
     * @return TParm
     */
    public TParm getInsDetailData(TParm tableParm) {
        TParm parm = new TParm();
        String accountSeq = getAccountSeq(tableParm);
        toBalacnce(accountSeq);
        parm = getInsDetailPrint(accountSeq);
        if (parm == null)
            return null;
        return parm;
    }
...
//         System.out.println("退==="+sql1);
        ...
        returnParm.setData("FLG_AGENT_AMT", "TEXT", StringTool.round(flgagentamtS,2));
 		...
 		String sqlBil = " SELECT PRINT_NO,BILL_DATE,ACCOUNT_SEQ " +
                        " FROM BIL_OPB_RECP WHERE ADM_TYPE LIKE '%"+admType+"%' " +
                        " AND ACCOUNT_SEQ in ("+accountSeq+") ORDER BY BILL_DATE ";
        TParm bilParm = new TParm(TJDODBTool.getInstance().select(sqlBil));
        String stardate = bilParm.getData("BILL_DATE", 0).toString();
        String enddate = bilParm.getData("BILL_DATE", bilParm.getCount()-1).toString();
        stardate = stardate.substring(0, 19);
       ...
       // System.out.println("tableParm"+tableParm);
        ...
        if ("N".equals(getValue("TOGEDER_FLG"))) {
            for (int i = 0; i < rowCount; i++) {
                ...
                singleParm.addData("ACCOUNT_SEQ",endParm.getData("ACCOUNT_SEQ",i));
                singleParm.setCount(1);
                printData = getThirdpayData(singleParm);
                printData.setData("USER","TEXT",Operator.getName());//修改 添加收费员签章 20130722 caoyong
                this.openPrintWindow(
                    "%ROOT%\\config\\prt\\opb\\OPBThirdpartPrint.jhw", printData);

            }

        }
        else {
            printData = getThirdpayData(endParm);
            printData.setData("USER","TEXT",Operator.getName());//修改 添加收费员签章 20130722 caoyong
            if (printData == null)
                return;
            this.openPrintWindow(
     ...
     * 第三方打印数据
     */
    public TParm getThirdpayData(TParm dateParm){
    	TParm printData = new TParm();
    	String accountSeq = getAccountSeq(dateParm);
    	String admType = getValueString("ADM_TYPE");
    	BIL bil = new BIL();
        ...
        TParm payParmBack = bil.getPayParm();       
       //微信支付
        TParm payParm1 = bil.getPayParm();
        //微信退费
        TParm payParmBack1 = bil.getPayParm();
        
    	String sql = " SELECT ALIPAY,AR_AMT,QE_PAY_TYPE FROM BIL_OPB_RECP WHERE ACCOUNT_SEQ IN ("+accountSeq+")";
        TParm receiptParm = new TParm(TJDODBTool.getInstance().select(sql));        
        if (receiptParm.getErrCode() < 0) {
            return null;
        }
        int count = receiptParm.getCount();
        for (int i = 0; i < count; i++) {
        	if(receiptParm.getValue("QE_PAY_TYPE", i).equals("2")) {//支付宝
        		if (receiptParm.getDouble("AR_AMT", i) >= 0) {//支付宝收入
        			payParm.setData("ALIPAY",payParm.getDouble("ALIPAY") + receiptParm.getDouble("ALIPAY", i));
        		}
        		else { //支付宝退费
        			payParmBack.setData("ALIPAY",(payParmBack.getDouble("ALIPAY") + receiptParm.getDouble("ALIPAY", i)));
        		}
        	}
        	if(receiptParm.getValue("QE_PAY_TYPE", i).equals("6")) {//微信
        		if (receiptParm.getDouble("AR_AMT", i) >= 0) {//微信收入
        			payParm1.setData("ALIPAY",payParm1.getDouble("ALIPAY") + receiptParm.getDouble("ALIPAY", i));
        		}
        		else { //微信退费
        			payParmBack1.setData("ALIPAY",(payParmBack1.getDouble("ALIPAY") + receiptParm.getDouble("ALIPAY", i)));
        		}
        	}       	
        }
        
        payParm.setData("ALIPAY",Math.abs(payParm.getDouble("ALIPAY")));//支付宝收入
        payParmBack.setData("ALIPAY",Math.abs(payParmBack.getDouble("ALIPAY")));//支付宝退费
        payParm1.setData("ALIPAY",Math.abs(payParm1.getDouble("ALIPAY")));//微信收入
        payParmBack1.setData("ALIPAY",Math.abs(payParmBack1.getDouble("ALIPAY")));//微信退费
        double zfb_c = 0.00;
        double wx_c = 0.00;
        if(payParmBack.getDouble("ALIPAY") > 0){
        	zfb_c = -payParmBack.getDouble("ALIPAY");
        }
        else{
        	zfb_c = 0;
        }
        if(payParmBack1.getDouble("ALIPAY") > 0){
        	wx_c = -payParmBack1.getDouble("ALIPAY");
        }
        else{
        	wx_c = 0;
        }
        double pay_all = payParm.getDouble("ALIPAY") + payParm1.getDouble("ALIPAY");//收入合计
        double pay_all_c = zfb_c + wx_c;//退费合计
    	
        //支付宝收入
        printData.setData("ZFB", "TEXT",StringTool.round(payParm.getDouble("ALIPAY"),2));
        //支付宝退费
        printData.setData("ZFB_C","TEXT", StringTool.round(zfb_c,2));
        //微信收入
        printData.setData("WX","TEXT", StringTool.round(payParm1.getDouble("ALIPAY"),2));
        //微信退费
        printData.setData("WX_C","TEXT", StringTool.round(wx_c,2));
        //收入合计
        printData.setData("PAY_ALL","TEXT",StringTool.round(pay_all,2));
        //退费合计
        printData.setData("PAY_ALL_C","TEXT", StringTool.round(pay_all_c,2));
        
    	//打印时间
        String apDate = StringTool.getString(SystemTool.getInstance().getDate(),"yyyyMMddHHmmss");
        apDate = dealDate(apDate);
        printData.setData("PRINTDATE", "TEXT",apDate);
        
    	//结算时间
    	String sqlBil = "SELECT PRINT_NO,BILL_DATE,ACCOUNT_SEQ FROM BIL_OPB_RECP WHERE ADM_TYPE LIKE '%"+admType+"%' AND ACCOUNT_SEQ in ("+
        accountSeq+") ORDER BY BILL_DATE ";
        TParm bilParm = new TParm(TJDODBTool.getInstance().select(sqlBil));    	
    	String stardate = bilParm.getData("BILL_DATE", 0).toString();
        String enddate = bilParm.getData("BILL_DATE", bilParm.getCount()-1).toString();
