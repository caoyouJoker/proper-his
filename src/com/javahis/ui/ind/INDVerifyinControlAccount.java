...
public class INDVerifyinControlAccount
    extends TControl {
    ...
    public INDVerifyinControlAccount() {
        super();
    }
    ...
    public void onQuery() {
        ...
        result = TIOM_AppServer.executeAction("action.ind.INDVerifyinAction",
                                              "onQueryM", parm);
//        System.out.println("------------query: "+result);
    ...
    public void onClear() {
        ...
        String clearString =
            "START_DATE;END_DATE;VERIFYIN_DATE;ORG_CODE;VERIFYIN_NO;"
...
/*            for (int i = 0; i < updateSql.length; i++) {
            ...
            result = TIOM_AppServer.executeAction(
                "action.ind.INDVerifyinAction", "onInsert", parm);
...
//                        org_code, order_code, batch_no, valid_date);
                    ...
                    double verpriceD = dataStore.getItemDouble(i, "VERIFYIN_PRICE") /d_qty / s_qty;
                    ...
                    //add by liyh 20120614 在IND_STOCK 里查询药品验收价格时，应该是片/支的价格（最小单位的价格） end
                     //出入库效期批号者
                    getSEQ = IndStockDTool.getInstance().onQueryStockBatchSeq(
                        org_code, order_code, batch_no, valid_date,verpriceDStr);//
                    //*************************************************************
...
/*                    result = IndPurorderMTool.getInstance().onQueryDone(inparm);
                    ...
                    else if ("Y".equals(check_flg) ) {//物联网-导致HIS没有订购直接验收入库，所以update_flg=1 这样才能在验收汇总统计可以查询
                        update_flg = "1";
                    }
            ...
            // 执行数据更新
            result = TIOM_AppServer.executeAction(
                "action.ind.INDVerifyinAction", "onUpdate", parm);
            // 保存判断
            if (result == null || result.getErrCode() < 0) {
                this.messageBox("E0001");
                return;
...
/*			if ( "Y".equals(getValueString("CHECK_FLG"))) {// 如果是审核  且是则自动生成请领单和出库单
				String dispenseNo = "";
				...
				// 执行数据更新
				result = TIOM_AppServer.executeAction("action.ind.INDVerifyinAction", "onSaveReqeustAndDispense", parm);
			}*/
...
/*		if (getRadioButton("GEN_DRUG").isSelected()) {// 非麻精
		...
		Object result = openDialog("%ROOT%\\config\\ind\\INDVerifyinImpXML.x", parm);
...
//		System.out.println("------------SPCSQL.getErpIdInfo(orderCode, boxCode, billNo, erpId):"+SPCSQL.getErpIdInfo(orderCode, boxCode, billNo, erpId));
		TParm parm = new TParm(TJDODBTool.getInstance().select(INDSQL.getErpIdInfo(orderCode, boxCode, billNo, erpId)));
...
//		System.out.println("-------------flag: "+flag);
    ...
    public void onImpXML(){
		...
		if (result != null) {
            ...
            for (int i = 0; i < addParm.getCount("ORDER_CODE"); i++) {
                int row = table.addRow();
                String supOrderCode = addParm.getValue("ORDER_CODE",
                        i);
                TParm supParm = new TParm(TJDODBTool.getInstance().select(INDSQL.getOrderCodeBySupRe(supCode, supOrderCode)));
                ...
                // 验收单价
                puroder_price = addParm.getDouble("PURORDER_PRICE", i);
                table.setItem(row, "VERIFYIN_QTY",  purorder_qty * conversionTraio);
                // 赠送量
                table.setItem(row, "GIFT_QTY", 0);
                // 进货单位
//                System.out.println("BILL_UNIT:"+phaParm.getValue("PURCH_UNIT", 0));
                table.setItem(row, "BILL_UNIT", phaParm.getValue("PURCH_UNIT", 0));
                table.setItem(row, "VERIFYIN_PRICE",  StringTool.round(puroder_price/conversionTraio, 4));
                // 进货金额
                table.setItem(row, "INVOICE_AMT",StringTool.round(puroder_price * purorder_qty, 2));
                // 零售价
                retail_price = phaParm.getDouble("RETAIL_PRICE", 0);
                TParm phaDosageParm = new TParm(TJDODBTool.getInstance().select(INDSQL.getTransunitByCode(orderCode)));
                int dosageQty = 1;
                if (null != phaDosageParm ) {
                	  dosageQty = phaDosageParm.getInt("DOSAGE_QTY", 0);
				}
                //零售价
                table.setItem(row, "RETAIL_PRICE", StringTool.round(retail_price * dosageQty, 2));
                // 订购单号
                table.setItem(row, "PURORDER_NO", addParm.getData("PURORDER_NO", i));
                // 订购单号序号
                table.setItem(row, "PURSEQ_NO",addParm.getData("PURSEQ_NO", i));
