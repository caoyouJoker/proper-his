 ...
 * @version 1.0
 */
public class INDDispenseOUTAccountControl
    extends TControl {
    ...
    public INDDispenseOUTAccountControl() {
        super();
    }
    ...
    public void onPrint() {
		...
		if (table_d.getRowCount() > 0) {
			// modify by wangbin 2015/2/2 内部需求 #786 泰心物联网系统调整方案 : 药库出库单打印相关修改(出库单打印按照供应商分单打印) START
			// 查询该出库单号下有多少供应商
			...
			String sql = "SELECT A.ORDER_CODE, CASE WHEN B.GOODS_DESC IS NULL "
					...
					+ " A.QTY AS OUT_QTY, A.BATCH_NO, A.VALID_DATE,A.VERIFYIN_PRICE,A.SUP_CODE,D.TYPE_CODE "// ADD VERIFYIN_PRICE
					+ " FROM IND_DISPENSED A, SYS_FEE B, SYS_UNIT C,PHA_BASE D "
					+ " WHERE A.ORDER_CODE = B.ORDER_CODE "
					+ " AND A.UNIT_CODE = C.UNIT_CODE "
					+ " AND A.ORDER_CODE = D.ORDER_CODE "
					+ " AND A.DISPENSE_NO = '"
					+ this.getValueString("DISPENSE_NO") + "' AND 1=1"
					+ " ORDER BY A.SEQ_NO";
			...
			// modify by wangbin 2015/2/2 内部需求 #786 泰心物联网系统调整方案 : 药库出库单打印相关修改(出库单打印按照供应商分单打印) END
...
//                "BATCH_SEQ"));
	...
	private void setPrintData(TParm supCountData, TParm printData) {
		...
		DecimalFormat df = new DecimalFormat("##########0.00");// wanglong add 20140729
		...
		for (int i = 0; i < printData.getCount("ORDER_CODE"); i++) {
			if (StringUtils.equals(supCountData.getValue("SUP_CODE"), printData
				...
				parm.addData("AMT_VERIFYIN", df.format(StringTool.round(
						printData.getDouble("VERIFYIN_PRICE", i) * qty, 2)));// 显示两位小数 wanglong modify 20140729
		...
		data.setData("ATM", "TEXT", df.format(atm));// 显示两位小数 wanglong modify 20140729
		data.setData("VERIFYIN_ATM", "TEXT", df.format(verifyinAtm));// 显示两位小数 wanglong modify 20140729
