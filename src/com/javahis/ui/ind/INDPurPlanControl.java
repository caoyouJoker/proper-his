...
import jdo.ind.IndPurPlanDTool;
...
public class INDPurPlanControl
    ...
    public void onSave() {
        ...
        if ("insertM".equals(action)) {
            ...
            result = TIOM_AppServer.executeAction(
                "action.ind.INDPurPlanAction", "onInsertM", parm);
            ...
            // 执行数据更新
            result = TIOM_AppServer.executeAction(
                "action.ind.INDPurPlanAction", "onUpdateM", parm);
        }
        else {
            TTable table_D = getTable("TABLE_D");
            table_D.acceptText();
    ...
    public void onDelete() {
        if (getTable("TABLE_M").getSelectedRow() > -1) {
            if (this.messageBox("删除", "确定是否删除采购计划单", 2) == 0) {
                ...
                result = TIOM_AppServer.executeAction(
                    "action.ind.INDPurPlanAction", "onDeleteM", parm);
                // 删除判断
                if (result == null || result.getErrCode() < 0) {
                    this.messageBox("删除失败");
                    return;
    ...
    public void onExport() {
        ...
        this.openWindow("%ROOT%\\config\\ind\\INDPurplanExcerpt.x",
                        parm);
    }
    ...
    public void onMake() {
        ...
        parm.setData("PLAN_MONTH", getValueString("PLAN_MONTH"));
        parm.setData("PLAN_NO", getValueString("PLAN_NO"));
        Object result = openDialog("%ROOT%\\config\\ind\\INDOrderForm.x",
                                   parm);
    }
