...
//			result.addData("UNIT_CHN_DESC", tableParm.getValue("BILL_UNIT", i)); 
		...
		String pDate = SystemTool.getInstance().getDate().toString().substring(0,10).replaceAll("-", "/");//制表时间
		...
		printParm.setData("DATE", "TEXT","入库日期: "+this.getValueString("VERIFYIN_DATE").substring(0, 10).replaceAll("-", "/")) ;
		printParm.setData("P_DATE", "TEXT", "制表时间: " + pDate);
		printParm.setData("P_USER", "TEXT", "申请科室: " + getTextFormat("CON_ORG").getText());
		printParm.setData("ORG_DESC", "TEXT", "部门: " + orgDesc);
		printParm.setData("SUP_CODE", "TEXT", "供应商：" + this.getTextFormat("SUP_CODE").getText());
		printParm.setData("REQUEST_TYPE", "TEXT", "入库单号: " + this.getValueString("VERIFYIN_NO"));
...
//			result.addData("UNIT_CHN_DESC", tableParm.getValue("BILL_UNIT", i)); 
		...
		printParm.setData("DATE", "TEXT","出库日期: "+this.getValueString("VERIFYIN_DATE").substring(0, 10).replaceAll("-", "/")) ;
		printParm.setData("P_DATE", "TEXT", "制表时间: " + pDate);
		printParm.setData("P_USER", "TEXT", "申请科室: " + getTextFormat("CON_ORG").getText());
		printParm.setData("ORG_DESC", "TEXT", "部门: " + orgDesc);
		printParm.setData("SUP_CODE", "TEXT", "供应商：" + this.getTextFormat("SUP_CODE").getText());
    	...
    	for (int j = 0; j < parm.getCount(); j++) {
    		 if ("Y".equals(table_d.getItemString(j, "SELECT_FLG"))) {
    				for (int i = 0; i < parm.getRow(j).getInt("QTY"); i++) {
    			        ...
    			        reportParm.setData("PACK_DESC","TEXT","名称："+parm.getData("INV_CHN_DESC", i));
    			        reportParm.setData("PDATE_WORD","TEXT","规格："+parm.getData("DESCRIPTION", i));
    			        reportParm.setData("PACK_DATE","TEXT","效期："+parm.getData("VALID_DATE", i).toString().substring(0,10));
