...
public class INVVerifyinControl extends TControl {
	...
	public void onExport() {   
		...
		if (result != null) {
			UnaddParm = (TParm) result;
			getTextFormat("CON_ORG").setValue(UnaddParm.getData("CON_ORG"));
			// addParm 看看其中的FROM_TYPE 决定 直接入库按钮和入库科室(0,自动拨补 1,手工)
//			System.out.println("addParm.getData('CON_ORG')==="
//					+ UnaddParm.getData("CON_ORG")); 
			if (UnaddParm == null) {
				return;                         
			}  
			// 供应厂商
			this.setValue("SUP_CODE", UnaddParm.getValue("SUP_CODE", 0));
			// 计划单号
			this.setValue("STATIO_NO", UnaddParm.getValue("STATIO_NO", 0));
			double purorder_qty = 0;
			double actual_qty = 0;
			double puroder_price = 0;
			for (int i = 0; i < UnaddParm.getCount("INV_CODE"); i++) {
				// 验收量
				purorder_qty = UnaddParm.getDouble("PURORDER_QTY", i);
				actual_qty = UnaddParm.getDouble("STOCKIN_SUM_QTY", i);
				int row = table_d.addRow();
				// 选择
				table_d.setItem(row, "SELECT_FLG", "N");
				// 物资名称
				table_d.setItem(row, "INV_CHN_DESC", UnaddParm.getValue(
						"INV_CHN_DESC", i));
				// 规格
				table_d.setItem(row, "DESCRIPTION", UnaddParm.getValue(
						"DESCRIPTION", i));
				// 验收数量
				table_d.setItem(row, "VERIFIN_QTY", purorder_qty - actual_qty);
				// 赠与数
				table_d.setItem(row, "GIFT_QTY", UnaddParm.getData("GIFT_QTY",
						i));
				// 进货单位
				table_d.setItem(row, "BILL_UNIT", UnaddParm.getValue(
						"BILL_UNIT", i));
				// 验收单价
				puroder_price = UnaddParm.getDouble("PURORDER_PRICE", i);
				...
				// 入库数量
				table_d.setItem(row, "IN_QTY", StringTool.round(UnaddParm
						.getDouble("STOCK_QTY", i)
						* UnaddParm.getDouble("DISPENSE_QTY", i)
						/ UnaddParm.getDouble("PURCH_QTY", i)
						* (purorder_qty - actual_qty), 1));   
				// 入库单位
				table_d.setItem(row, "STOCK_UNIT", UnaddParm.getValue(
						"DISPENSE_UNIT", i));
				// 批号 ----fux modify 手工填写
				...
				table_d.setItem(row, "BATCH_NO", "");
				// 生产厂商
				table_d.setItem(row, "MAN_CODE", UnaddParm.getValue("MAN_CODE",
						i));
				// 序号管理
				table_d.setItem(row, "SEQMAN_FLG", UnaddParm.getValue(
						"SEQMAN_FLG", i));
				// 效期管理
				table_d.setItem(row, "VALIDATE_FLG", UnaddParm.getValue(
						"VALIDATE_FLG", i));
				// 物资代码
				table_d.getParmValue().setData("INV_CODE", row,
						UnaddParm.getValue("INV_CODE", i));
				// 订购单序号
				table_d.getParmValue().setData("STESEQ_NO", row,
						UnaddParm.getInt("SEQ_NO", i));
				// 订购单号
				table_d.getParmValue().setData("PURORDER_NO", row,
						UnaddParm.getValue("PURORDER_NO", i));
				// 订购数量
				table_d.getParmValue().setData("PURORDER_QTY", row,
						purorder_qty);
				// 库存转换量
				table_d.getParmValue().setData("STOCK_QTY", row,
						UnaddParm.getDouble("STOCK_QTY", i));
				// 出库转换量
...
//	}
	...
	public void onSave() {
		...
		if (table_m.getSelectedRow() < 0) {
			...
			//更新只能修改数量不能新加物品
			//STOCKIN_SUM_QTY = PURORDER_QTY  全部验收完成   再将FINAL_FLG = 'Y'
			//如果不等于则是部分入库
			// 1.取得验收主表数据(TABLE_M)
	...
	private TParm getDispenseDData(TParm parm) {
		...
		for (int i = 0; i < table_d.getRowCount() - 1; i++) {
...
//		}
//		result = TIOM_AppServer.executeAction("action.inv.INVVerifyinAction",
//				"onUpdateValData", parm);
//
...
//	}
	...
	private boolean checkData() {
		...
		// 加校验 同类物品验收总数 等于 订购数
		double purorder_qty = 0;
		for (int j = 0; j < UnaddParm.getCount("INV_CODE"); j++) {
			double ver_qty = 0; 
			double gift_qty = 0;   
		for (int i = 0; i < table_d.getRowCount() - 1; i++) {
			...
			if ("Y".equals(table_d.getItemString(i, "SELECT_FLG"))) {
				...
				if (table_d.getItemDouble(i, "UNIT_PRICE") <= 0) {
					...
					if (new BigDecimal(parmM.getData("CONTRACT_PRICE", 0)
							.toString()).doubleValue() != table_d
							.getItemDouble(i, "UNIT_PRICE")) {
				...
				if (UnaddParm.getValue("INV_CODE", j).equals(table_d.getParmValue().getValue("INV_CODE",i).toString())) {
					ver_qty = ver_qty + table_d.getItemDouble(i, "VERIFIN_QTY");  
					gift_qty = gift_qty + table_d.getItemDouble(i, "GIFT_QTY");
				 }     
				}     
				}          
	            purorder_qty = UnaddParm.getDouble("PURORDER_QTY", j); 
			    if(ver_qty+gift_qty<purorder_qty){           
				   this.messageBox("物资代码: " +
						   UnaddParm.getValue("INV_CODE", j)+"验收物资部分入库,请注意。");     
				return true;   
			    }           
			    if(ver_qty+gift_qty>purorder_qty){            
				   this.messageBox("物资代码: " +
						   UnaddParm.getValue("INV_CODE", j)+"验收物资超过本次订购限额,请确认。");
				return false; 
				} 
		}       
	...
	private TParm getUpdateInvStockMData(TParm parm) {
		...
		for (int i = 0; i < parmD.getCount("INV_CODE"); i++) {
			...
			// }  
			// IN_QTY->QTY
			//fux  modify 20140703 加入赠送数 +parmD.getDouble("GIFT_QTY", i)
	...
	private TParm getInsertInvStockDData(TParm parm) {
		...
		int seq = 0;
		int num = 0;
		Map<String, Integer> mapNum = new TreeMap<String, Integer>();
		Map<String, Integer> map = new TreeMap<String, Integer>();
		ArrayList list = new ArrayList();
        //可以treeMap放进去先排序  然后再判断进行数据保存   
		int count = parmD.getCount("INV_CODE");
		...
		for (int i = 0; i < parmD.getCount("INV_CODE"); i++) {
			...
			// 还要考虑到现在界面上会有相同的物品代码导致批号重复 
			String valid_dateString = valid_date.substring(0, 10).replace('-',
					'/');
...
//				int[] addRow = new int[count < 0 ? 0 : count];
//				//orderCode = 1,2,2,1,1,2,3  
//				for (int j = i; j < count; j++) {
//					inv_codeOld = parmD.getValue("INV_CODE", j);
//				if (map.containsKey(inv_code)) { 
//					seq++;  
//					map.remove(inv_code);  
//					map.put(inv_code, batch_seq);    
//					batch_seq = batch_seq + seq;         
//					addRow[addRowCount] = j;
//					addRowCount++;
//				}  else{
//					map.put(inv_code, batch_seq);
//				   }
...
//				for (int k = addRowCount - 1; k >= 0; k--) {
//					parm.removeRow(addRow[k]);
//				}
...
//					 qtyIn = (Integer)map.get(s[j]);  
//					 System.out.println("键值："+map.get(s[j]));
//					 }    
				} else {  
					map.put(inv_code, batch_seq);
				}
	...
	private void initPage() {
		...
		table_d.addEventListener(TTableEvent.CREATE_EDIT_COMPONENT, this,
	...
	public void popReturn(String tag, Object obj) {
		...
		if (!StringUtil.isNullString(inv_desc)) {
			...
			TParm stockMParm = new TParm(TJDODBTool.getInstance().select(
					INVSQL.getSupInv(sup_code, inv_code)));
			// TParm stockMParm = new
			// TParm(TJDODBTool.getInstance().select(INVSQL.
			...
			double actual_qty = 0;
			double puroder_price = 0;
			TParm purParm = new TParm();
			for (int i = 0; i < stockMParm.getCount("INV_CODE"); i++) {
				for (int j = 0; j < UnaddParm.getCount("INV_CODE"); j++) {
					if (stockMParm.getValue("INV_CODE", i).equals(
							UnaddParm.getValue("INV_CODE", j))) {
						purParm = UnaddParm.getRow(j);
					}
				}
				// purParm：{Data={GIFT_QTY=0.0, INV_CODE=21.03.01.0700101,
				// SUP_CODE=19, STOCKIN_SUM_QTY=0.0,
				// PURORDER_NO=140630000004, PURORDER_PRICE=4.7,
				// PURORDER_QTY=10.0, SEQ_NO=0, INV_CHN_DESC=注射器,
				// CON_ORG=null, BILL_UNIT=154, DISPENSE_QTY=1, STOCK_UNIT=154,
				// SELECT_FLG=Y, PURCH_QTY=1,
				// VALIDATE_FLG=Y, DISPENSE_UNIT=158, EXPENSIVE_FLG=Y,
				// DESCRIPTION=60ML, STOCK_QTY=1,
				// PURORDER_DATE=2014-06-30 14:00:43.0, SEQMAN_FLG=N,
				// STATIO_NO=null, FROM_TYPE=1, MAN_CODE=154}}
				// 验收量
				purorder_qty = purParm.getDouble("PURORDER_QTY");
				actual_qty = purParm.getDouble("STOCKIN_SUM_QTY");
				int row = table_d.getSelectedRow();
				// 选择
				table_d.setItem(row, "SELECT_FLG", "N");
				// 物资名称
				table_d.setItem(row, "INV_CHN_DESC", purParm
						.getValue("INV_CHN_DESC"));
				// 规格
				table_d.setItem(row, "DESCRIPTION", purParm
						.getValue("DESCRIPTION"));
				// 验收数量
				table_d.setItem(row, "VERIFIN_QTY", purorder_qty - actual_qty);
				// 赠与数
				table_d.setItem(row, "GIFT_QTY", purParm.getData("GIFT_QTY"));
				// 进货单位
				table_d
						.setItem(row, "BILL_UNIT", purParm
								.getValue("BILL_UNIT"));
				// 验收单价
				puroder_price = purParm.getDouble("PURORDER_PRICE");
				table_d.setItem(row, "UNIT_PRICE", puroder_price);
				// 小计
				table_d.setItem(row, "VERIFYIN_AMT", StringTool.round(
						puroder_price * (purorder_qty - actual_qty), 2));
				// 入库数量
				table_d.setItem(row, "IN_QTY", StringTool.round(purParm
						.getDouble("STOCK_QTY")
						* purParm.getDouble("DISPENSE_QTY")
						/ purParm.getDouble("PURCH_QTY")
						* (purorder_qty - actual_qty), 1));
				// 入库单位
				table_d.setItem(row, "STOCK_UNIT", purParm
				...
				table_d.setItem(row, "BATCH_NO", "");
				// 生产厂商
				table_d.setItem(row, "MAN_CODE", stockMParm
						.getValue("MAN_CODE"));
				// 序号管理
				table_d.setItem(row, "SEQMAN_FLG", purParm
						.getValue("SEQMAN_FLG"));
				// 效期管理
				table_d.setItem(row, "VALIDATE_FLG", purParm
						.getValue("VALIDATE_FLG"));
				// 物资代码
				table_d.getParmValue().setData("INV_CODE", row,
						purParm.getValue("INV_CODE"));
				// 订购单序号
				table_d.getParmValue().setData("STESEQ_NO", row,
						purParm.getInt("SEQ_NO"));
				// 订购单号
				table_d.getParmValue().setData("PURORDER_NO", row,
						purParm.getValue("PURORDER_NO"));
				// 订购数量
				table_d.getParmValue().setData("PURORDER_QTY", row,
						purorder_qty);
				// 库存转换量
				table_d.getParmValue().setData("STOCK_QTY", row,
						purParm.getDouble("STOCK_QTY"));
				// 出库转换量
				table_d.getParmValue().setData("DISPENSE_QTY", row,
						purParm.getDouble("DISPENSE_QTY"));
			}
	...
	private int createNewRow() {
		int row = table_d.addRow();
		// table_d.release();
		// table_d.setLockRows(row+"");
		table_d.setLockColumns("2,4,5,7,12,13,14,15,16");
		return row;
	}
	...
	/**
	 * 得到点击的行数,+1 由点击的行取数据 然后在下一行进行录入不同批号效期 注意校验 修改批号效期
	 */
	public void onBatchNo() {
		int row = table_d.getSelectedRow();
		table_d.addRow();
		int row1 = table_d.addRow();
		table_d.setItem(row1, row, "");
	}
