 ...
 * @version 1.0
 */
public class ODIInfecPackDiagControl extends TControl {
	TTable tablem;// 主表
  ...
  */
	...
	private void insertPackOrder(int row, TParm parm,int maxSeq) {
		...
		//LINKMAIN_FLG;LINK_NO;SEQ_NO;ORDER_DESC;MEDI_QTY;MEDI_UNIT;ROUTE_CODE;FREQ_CODE;SPECIFICATION;DESCRIPTION;ORDER_CODE;CAT1_TYPE;ORDER_CAT1_CODE;TRADE_ENG_DESC
		...
		if (row > 0) {
			...
			tableorder.setItem(row, "ROUTE_CODE", resultDs.getData(
					"ROUTE_CODE", 0));
//			System.out.println("用法是：：："+resultDs.getData(
//					"ROUTE_CODE", 0));
//			tableorder.setItem(row, "FREQ_CODE", "");
			tableorder.setItem(row, "FREQ_CODE",resultDs.getData(
					"FREQ_CODE", 0));
//			System.out.println("频次是：：："+resultDs.getData(
//					"FREQ_CODE", 0));
		}
		//组号相同则单位、频次、用法和组号相同的连一致
		if(tableorder.getItemString(row,"LINK_NO").length()>0){
			String liNo=tableorder.getItemString(row,"LINK_NO");
			TParm treslut=this.getTresult(liNo);
			//tableorder.setItem(row, "MEDI_QTY", treslut.getData("MEDI_QTY"));//用量
			//tableorder.setItem(row, "MEDI_UNIT", treslut.getData("MEDI_UNIT"));//单位
			tableorder.setItem(row, "ROUTE_CODE", treslut.getData("ROUTE_CODE" ));//用法
			tableorder.setItem(row, "FREQ_CODE", treslut.getData("FREQ_CODE")); //频次
			//tableorder.setItem(row, "SPECIFICATION", treslut.getData("SPECIFICATION"));//规格
			//tableorder.setItem(row, "DESCRIPTION", treslut.getData("DESCRIPTION"));//备注
		}
	}
 ...
 */
	...
	public void onSave() {
		tableicd.acceptText();
		if(this.getValue("PACK_DESC").equals(null)||"".equals (this.getValue("PACK_DESC"))){
			this.messageBox("方案名称不可以为空！");
			return;
		}
		tableorder.acceptText();
	 ...
	 */
	public void onQuery() {
		String where = "WHERE SMEALTYPE = 'W'";
		if (!this.getValueString("PACK_CODE").equals(""))
	 ...
	 * @return
	 */
	public TParm getTresult(String liNo){
