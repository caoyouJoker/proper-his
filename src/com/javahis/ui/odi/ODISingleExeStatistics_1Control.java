 ...
 * @version 1.0
 */
public class ODISingleExeStatistics_1Control extends TControl {
    ...
    public void onQuery() {
    	...
    	if(this.getValue("ORDER_TYPE").equals("1")){
    		sql=" SELECT TO_CHAR(M.ORDER_DATE,'YYYYMM') AS ORDER_DATE," +
    			 ...
    			 " THEN 1"+ 
    			 " ELSE 0"+
    			 " END ) AS EXEC_ACCOUNT_NUM,'检验' AS OREDER_TYPE,M.DEPT_CODE"+
    			 ...
    			 " GROUP BY TO_CHAR(M.ORDER_DATE,'YYYYMM'),DEPT.DEPT_CHN_DESC,'检验',M.DEPT_CODE"+
    	...
    	//药品
    		...
    		sql=" SELECT SUBSTR(D.ORDER_DATE,0,6) AS ORDER_DATE," +
    			 ...
    			 " THEN 1" + 
    			 " ELSE 0" +
    			 " END ) AS EXEC_ACCOUNT_NUM,'药品' AS OREDER_TYPE,M.DEPT_CODE" +
    			 ...
    			 " AND DEPT.DEPT_CODE=M.DEPT_CODE" +
    			 " GROUP BY SUBSTR(D.ORDER_DATE,0,6),DEPT.DEPT_CHN_DESC,'药品',M.DEPT_CODE" +
    			 " ORDER BY ORDER_DATE,M.DEPT_CODE,OREDER_TYPE";
    	}
    	else {
    		...
    		" AND DEPT.DEPT_CODE=M.DEPT_CODE"+
    		" GROUP BY SUBSTR(D.ORDER_DATE,0,6),DEPT.DEPT_CHN_DESC,'药品',M.DEPT_CODE) A" +
...
//    	 System.out.println("data===========" + data);
    	...
    	this.openDialog("%ROOT%\\config\\odi\\ODISingleDetail_1.x",parm);
