...
//            return;
        ...
        String text = parm.getValue("TEXT");
        setEditText(text);
        if(parm.getValue("MTN_KIND").equals("0")){//保养
        	this.sql = "SELECT A.DEV_CODE,A.MTN_TYPE_CODE,'保养' AS MTN_KIND,A.MTN_TYPE_DESC,B.DEV_CHN_DESC "
        			+ " FROM DEV_MAINTENANCEM A,DEV_BASE B "
        			+ " WHERE A.DEV_CODE = B.DEV_CODE "
        			+ " AND A.MTN_KIND='0' "
        			+ " AND A.ACTIVE_FLG='Y' ";
        }else if(parm.getValue("MTN_KIND").equals("1")){//质控
        	this.sql = "SELECT A.DEV_CODE,A.MTN_TYPE_CODE,'质控' AS MTN_KIND,A.MTN_TYPE_DESC,B.DEV_CHN_DESC "
        			+ " FROM DEV_MAINTENANCEM A,DEV_BASE B "
        			+ " WHERE A.DEV_CODE = B.DEV_CODE "
        			+ " AND A.MTN_KIND='1' "
        			+ " AND A.ACTIVE_FLG='Y' ";
        }else if(parm.getValue("MTN_KIND").equals("2")){//计量
        	this.sql = "SELECT A.DEV_CODE,A.MEASUREM_CODE AS MTN_TYPE_CODE,'计量' AS MTN_KIND,A.MEASUREM_DESC AS MTN_TYPE_DESC,B.DEV_CHN_DESC "
