 ...
 * @version 1.0
 */
public class DEVFixedDepreciationControl extends TControl{
	
	private TTable table;
	
	public DEVFixedDepreciationControl(){
		super();
	}
	/**
	 * 初始化方法
...
//		} 
		...
		System.out.println("固定资产折旧计算 sql="+sql.toString()); 
		TParm returnParm = new TParm(TJDODBTool.getInstance().select(
				 sql.toString()));
		
		//添加合计
		double all_dep_price_m = 0.0;		//本月折旧额
		double all_dep_price_all = 0.0;		//累计折旧额
		double all_dep_price_m_jz = 0.0;	//本月折旧额(净值)
	...
	public void onPrint(){
		...
		if(table.getRowCount() > 0){
			...
			for(int i = 0; i < table.getRowCount(); i++){
				...
				parm.addData("DEP_PRICE_M", tableParm.getValue("DEP_PRICE_M", i));			//本月折旧额
				parm.addData("DEP_PRICE_ALL", tableParm.getValue("DEP_PRICE_ALL", i));		//累计折旧额
				parm.addData("DEP_PRICE_M_JZ", tableParm.getValue("DEP_PRICE_M_JZ", i));	//本月折旧额(净值)
			...
			//调用打印方法
			this.openPrintWindow("%ROOT%\\config\\prt\\DEV\\DEVFixedDepreciation.jhw", data);
			}
