 ...
 * @version 1.0
 */
public class DEVFixedAssetListControl extends TControl{
	
	private TTable table;
	
	public DEVFixedAssetListControl(){ 
		super();
	}
	...
	private void initPage(){ 
		...
		"yyyyMMdd");
		this.setValue("Q_DATE", StringTool.getTimestamp(now,
				"yyyyMMdd"));// 查询日期 
	}
	 ...
	 */
	public void onQuery(){ 
		String devKind = getValueString("DEV_KIND");	//固定资产类别-资金来源
...
//		}
		...
		sql.append("SELECT T.DEV_CODE,T.DEV_CHN_DESC AS DEV_NAME,T.SPECIFICATION AS DEV_SPEC,")
				...
				.append(" 		B.DEP_PRICE AS ACC_DEP,B.CURR_PRICE AS PRE_ASSET,D.COST_CENTER_ABS_DESC AS COST_CENTER,")
		...
		for (int i = 0; i < returnParm.getCount(); i++) {
			...
			double unitPrice = StringTool.round(returnParm.getDouble("UNIT_PRICE",i),2);
			...
			double acc_dep = StringTool.round(returnParm.getDouble("ACC_DEP",i),2);
			double pre_price =StringTool.round(returnParm.getDouble("PRE_ASSET",i),2);
		...
		returnParm.setData("ACC_DEP", j, all_dep_price_all);
	...
	public void onTableClicked(int row) {
		...
		// 状态条显示       
			...
			//DEV_CODE;DEV_NAME;DEV_SPEC;STOCK_UNIT;DEV_NUM;UNIT_PRICE;
			//ALL_PRICE;BUY_DATE;ACC_DEP;PRE_ASSET;COST_CENTER;SOURCE_FUND 
		callFunction(  
		        "UI|setSysStatus", 
	...
	public void onPrint(){
		...
		if(table.getRowCount() > 0){
			...
			for(int i = 0; i < table.getRowCount(); i++){
				...
				parm.addData("ACC_DEP", tableParm.getValue("ACC_DEP", i));			//累计折旧
			...
			parm.addData("SYSTEM", "COLUMNS", "ACC_DEP");
			...
			// 调用打印方法
			this.openPrintWindow("%ROOT%\\config\\prt\\DEV\\DEVFixedAssetList.jhw", data);
			}
    ...
    public void onExport() {
    	...
    	if(table_e.getRowCount() > 0){
    		ExportExcelUtil.getInstance().exportExcel(table_e, "固定资产明细表统计");
    	}else {
