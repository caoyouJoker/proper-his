...
//                         "onTableChangeValue");
    ...
    private void initComponent(){
    	...
    	typeSpecial = (TCheckBox) this.getComponent("SPECIAL_FLG");//获取特殊授权
...
//        }
        ...
        detailTable.setItem(row, "UNIT_PRICE", resultPha.getValue("OWN_PRICE",0) ); 
		...
		if (result.getCount("ORDER_CODE") > 0) {
			for (int i = 0; i < result.getCount("ORDER_CODE"); i++) {
				...
				detailTable.setItem(row + i + 1, "ORDER_DESC", parm
						.getValue("ORDER_DESC"));
				detailTable.setItem(row + i + 1, "ORDER_CODE", parm
						.getValue("ORDER_CODE"));
				detailTable.setItem(row + i + 1, "ORDER_DESC_DETAIL", result
						.getValue("ORDER_DESC", i));
				detailTable.setItem(row + i + 1, "ORDER_CODE_DETAIL", result
						.getValue("ORDER_CODE", i));
				detailTable.setItem(row + i + 1, "UNIT_PRICE", result.getValue(
						"OWN_AMT", i));
			}
    ...
    public void onCtzClick() {
    	...
    	String nhiNo=tableparm.getValue("NHI_NO",selectedIndx);//医保身份 add by huangjw 20141011
    	if("Y".equals(useFlg)){
    	...
    	//查询是显示特殊权限标记
    	String specialFlg = tableparm.getValue("SPECIAL_FLG",selectedIndx);
    	if("Y".equals(specialFlg)){
    		typeSpecial.setSelected(true);
    	}else{
    		typeSpecial.setSelected(false);
    	}
    ...
    public void queryDetailData(String ctzCode) {
    	...
    	String sql2 = "(SELECT 'Y' AS EXEC, B.DEPT_CODE,B.CTZ_DESC,A.CTZ_CODE,A.ORDER_CODE," +
    			...
    			" A.OPT_DATE,A.OPT_TERM,A.ORDERSET_CODE,A.HIDE_FLG,A.UNIT_PRICE " +
    		...
    		if(detailParm.getCount()>0){//2:再判断细项表是否有数据
        		for (int i = 0; i < detailParm.getCount(); i++) {
    				...
    				if("Y".equals(exec)){//老数据
    					...
    					updateParm.addData("ORDER_DESC", detailParm.getValue("ORDER_DESC", i));  
    					}
    					//ORDER_DESC_DETAIL;ORDER_CODE_DETAIL;UNIT_PRICE
    					else{
        					updateParm.addData("ORDER_CODE", detailParm.getValue("ORDER_CODE_DETAIL", i));
        					updateParm.addData("ORDER_DESC", detailParm.getValue("ORDER_DESC_DETAIL", i));
    					}    
    					//数据库还得加入单价字段....
    					//updateParm.addData("UNIT_PRICE", detailTable.getItemString(i, "UNIT_PRICE"));  
    					updateParm.addData("DISCOUNT_RATE", detailParm.getValue("DISCOUNT_RATE", i));
...
//    					//SYS_CTZ_ORDER_DETAIL      
    					//if(detailParm.getValue("ORDER_DESC", i).length()>0){  
            					if(detailParm.getValue("ORDER_CODE_DETAIL", i).length()>0){  
            						boolean flgDetail = onCheckOrder(this.getValueString("CTZ_CODE3"), detailParm.getValue("ORDER_CODE_DETAIL", i));    
            						if(flgDetail == true){                            
...
//            						insertOrderParm.addData("ORDER_DESC", detailTable.getItemString(i, "ORDER_DESC"));  
            						insertOrderParm.addData("ORDER_CODE", detailParm.getValue("ORDER_CODE_DETAIL", i));  
            						insertOrderParm.addData("ORDER_DESC", detailParm.getValue("ORDER_DESC_DETAIL", i));  
                					insertOrderParm.addData("UNIT_PRICE", detailParm.getValue("UNIT_PRICE",i));
            						...
            						if(detailParm.getValue("ORDER_CODE", i).length()>0)  
            						{                    
            						boolean flgMain = onCheckOrder(this.getValueString("CTZ_CODE3"),detailParm.getValue("ORDER_CODE", i));
            						if(flgMain == true){         
            							insertOrderParm.addData("CTZ_CODE", this.getValue("CTZ_CODE3"));     
                						//insertOrderParm.addData("ORDER_CODE", detailParm.getValue("ORDER_CODE", i));    
            							insertOrderParm.addData("ORDER_CODE", detailParm.getValue("ORDER_CODE", i));  
                						insertOrderParm.addData("ORDER_DESC", detailParm.getValue("ORDER_DESC", i));  
                 						insertOrderParm.addData("UNIT_PRICE", detailParm.getValue("UNIT_PRICE",i));      
                						insertOrderParm.addData("DISCOUNT_RATE", detailTable.getItemString(i, "DISCOUNT_RATE"));
                						insertOrderParm.addData("OPT_DATE", detailTable.getItemString(i, "OPT_DATE"));
                						insertOrderParm.addData("OPT_USER", detailTable.getItemString(i, "OPT_USER"));    
...
//        	//初始化          
    ...
    private boolean onCheckOrder(String value, String value2) {
					...
					" FROM SYS_CTZ_ORDER_DETAIL " +
					" WHERE  CTZ_CODE = '"+value+"'" +      
					" AND ORDER_CODE = '"+value2+"' ";   
			//System.out.println("sql===----"+sql);
			TParm result = new TParm(TJDODBTool.getInstance().select(sql));
			if (result.getCount("ORDER_CODE")>0) {
				//查询到数据
    ...
    public void onDelete(){
    	...
    	if(detailIndx>=0){ 
    		...
    		String orderCodedetail = detailTable.getItemString(detailIndx, "ORDER_CODE_DETAIL");
    ...
    public TParm getData(){
    	...
    	String nhiNoQ=this.getValueString("NHI_NO_Q");//医保身份代码 add by huangjw 20141011
    	...
    	String typeSpecial1 ="";//特殊授权
    	if(typeSpecial.isSelected()){
    		typeSpecial1 = "Y";
    	}else{
    		typeSpecial1 ="N";
    	}
    	...
    	parm.setData("SPECIAL_FLG", typeSpecial1);//特殊授权
	...
	public void insertRow(String opertable, boolean focusFlg) {
		...
		if (focusFlg) {
			...
			/*try {
				 ...
				 * robot.mouseMove(operTable.getTable().getcol);
				 * robot.delay(2000);
