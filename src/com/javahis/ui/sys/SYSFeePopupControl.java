...
extends TControl {
	 ...
	 * 急诊抢救之口头医嘱
	 */
	private String onwPackType;// add by wangqing 20170905
	...
	public void initParamenter() {
		...
		// add by wangqing 20170905
		onwPackType = parm.getValue("ONW_PACK_ORDER");
	}
	...
	public boolean filter(TParm parm, int row) {
		// 过滤条件 1、包含关键字 2、ACTIVE_FLG为Y 3、REGION_CODE校验 4、ORDER_CAT1_CODE（小分类） 5、CAT1_TYPE（大分类）
		...
		// add by wangqing 20170905 start
		else if(!StringUtil.isNullString(onwPackType)){
			//全部
			if ("A".equals(onwPackType)) {
				result = result;
			}
		}
		// add by wangqing 20170905 end
		if(result)
		{
			index++;
	...
	public void onSelected() {
		...
		TParm parm = new TParm(TJDODBTool.getInstance().select(
				...
				"' AND ACTIVE_FLG = 'Y' "));
		System.out.println( "SELECT * FROM SYS_FEE WHERE ORDER_CODE='" + orderCode +
				"' AND ACTIVE_FLG = 'Y' ");
		//System.out.println("====parm======"+parm);
		System.out.println("pop parm : "+parm);
