...
public class REGOrderDetailControl extends TControl{
	...
	public void onQuery(){
		...
		String sql="SELECT A.REGION_CODE, A.ADM_TYPE, B.MR_NO, B.PAT_NAME, B.TEL_HOME, A.ADM_DATE, A.REG_DATE," +
				...
				" AND A.REG_DATE BETWEEN TO_DATE ('" + date_s + "', 'YYYYMMDDHH24MISS') " +
...
//		System.out.println(sql);
		...
		for(int i=0;i<tableParm.getCount("MR_NO");i++){
			...
			parm.addData("Name", tableParm.getValue("PAT_NAME", i));
			 String content = "您已预约成功"+
				...
				tableParm.getValue("DR_CODE", i) +"医生的门诊，仅限"+tableParm.getValue("PAT_NAME", i)+"本人，如需取消，请提前一天拨打服务电话4001568568，为了保证您准时就诊，您需提前办理挂号手续";
	...
	public void onExport() {
		...
		ExportExcelUtil.getInstance().exportExcel(table, "今日预约挂号明细");
	}
	...
	public void onPrint(){
        ...
        data.setData("TITLE", "TEXT", "今日预约明细报表");
...
//REGION_CODE;ADM_TYPE;;;TEL_HOME;;;;;;;REALDEPT_CODE;REALDR_CODE;;
    	...
    	this.openPrintWindow("%ROOT%\\config\\prt\\reg\\REGOrderDetailPrint.jhw", data);
	}
