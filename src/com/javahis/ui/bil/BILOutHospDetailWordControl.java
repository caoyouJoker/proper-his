...
/**
 * <p>Title: 出院患者医疗费用结转明细表</p>
 *
 * <p>Description: 出院患者医疗费用结转明细表</p>
 *
 ...
 * @version 1.0
 */
public class BILOutHospDetailWordControl extends TControl {
...
//        this.callFunction("UI|E_DATE|setEnabled", false);
    ...
    private TParm getHeard() {
        ...
        //modify by lim 2012/05/04 begin
        heardParm.addData("DIAG", "出院主诊断");//wanglong restore 20140512
        heardParm.addData("ICD", "手术主ICD");
        heardParm.addData("IN_DATE", "入院时间");
        heardParm.addData("OUT_DATE", "出院时间");
        heardParm.addData("DAYS", "住院天数");
        //modify by lim 2012/05/04 end
    ...
    private void print() {
        ...
        String sysDate = StringTool.getString(SystemTool.getInstance().getDate(),
                                              "yyyy/MM/dd HH:mm:ss");//20140109 wangjingchun modify 683
        ...
        if ("Y".equalsIgnoreCase(this.getValueString("tRadioButton_4"))) {
        	parm.setData("TITLE", "TEXT","出院病患医疗费用结转明细表(已结转)");
        }else if ("Y".equalsIgnoreCase(this.getValueString("tRadioButton_3"))) {
        	parm.setData("TITLE", "TEXT","出院病患医疗费用结转明细表(全部)");
        }else{
        	parm.setData("TITLE", "TEXT","出院病患医疗费用结转明细表(未结转)");
        }
        ...
        this.openPrintWindow(
                "%ROOT%\\config\\prt\\IBS\\BILOutHospPatDetailFee.jhw",
                parm);
    ...
    private TParm getPrintDate(String startTime, String endTime,
        ...
        if (!"".equals(Operator.getRegion()))
            regionWhere = " AND E.REGION_CODE = '" + Operator.getRegion() +
                          "' ";
        String sql =
                " SELECT B.DEPT_CODE AS EXE_DEPT_CODE, SUM (C.AR_AMT)  AS TOT_AMT, C.REXP_CODE, E.DEPT_ABS_DESC ,F.STATION_DESC, " +
                " H.PAT_NAME,A.MR_NO,A.CASE_NO,A.IPD_NO,G.CTZ_DESC " +//wanglong 20140512增加CASE_NO
                " FROM BIL_IBS_RECPM A,IBS_BILLM B,IBS_BILLD C,ADM_INP D,SYS_DEPT E,SYS_STATION F,SYS_CTZ G,SYS_PATINFO H " +
...
//              " AND D.DS_DATE BETWEEN TO_DATE ('" + startTime +
                ...
                deptWhere +
                regionWhere +
                " GROUP BY A.MR_NO,A.CASE_NO,A.IPD_NO,H.PAT_NAME,B.DEPT_CODE,B.STATION_CODE,C.REXP_CODE, E.DEPT_ABS_DESC ,F.STATION_DESC,G.CTZ_DESC ORDER BY IPD_NO";
        ...
        resultParm.addData("SYSTEM", "COLUMNS", "DIAG");//wanglong restore 20140512
        resultParm.addData("SYSTEM", "COLUMNS", "ICD");
        resultParm.addData("SYSTEM", "COLUMNS", "IN_DATE");
        resultParm.addData("SYSTEM", "COLUMNS", "OUT_DATE");
        resultParm.addData("SYSTEM", "COLUMNS", "DAYS");
     ...
     */
    public void onQuery() {
        String startTime = StringTool.getString(TypeTool.getTimestamp(getValue("S_DATE")), "yyyyMMdd");
        String endTime = StringTool.getString(TypeTool.getTimestamp(getValue("E_DATE")), "yyyyMMdd");
//        if (!startTime.substring(0, 6).equalsIgnoreCase(endTime.substring(0, 6))) {
//            this.messageBox_("起讫日期要在同一个月内");
//            return;
//        }
        String billTime = StringTool.getString(TypeTool.getTimestamp(getValue("BILL_DATE")), "yyyyMMdd");
        TParm printData = this.getPrintDate(startTime, endTime, billTime);
    }
    ...
    public void onExport() {
        ...
        ExportExcelUtil.getInstance().exportExcel(table, "出院患者医疗费用结转明细表");
    }
