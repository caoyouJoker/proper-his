...
public class SPCAccountDispense extends TControl {

	/* Table控件 */
	private TTable table_count;

	public SPCAccountDispense() {
		super();
	}
	...
	public void onQuery() {
		...
		TParm result = SPCAccountVerifyinTool.getInstance().getAccountForDis(
				parm);
		...
		for (int i = 0; i < result.getCount(); i++) {
			...
			// searchParm.setData("DISPENSE_DATE","2013/11/25 23:59:59");
			searchParm.setData("DISPENSE_DATE", dateNowStr.replaceAll("-", "/")
					+ " 23:59:59");
			String orderCode = result.getValue("ORDER_CODE", i);
			searchParm.setData("ORDER_CODE", orderCode);
			searchParm.setData("ORG_CODE", orgCode);
			TParm searchresult = SPCAccountVerifyinTool.getInstance()
					.getDispense(searchParm);
			Double diff = result.getDouble("ACCOUNT_AMT", i)
					- searchresult.getDouble("DISPENSE_AMT", 0);
			verifyinAmt = verifyinAmt
					+ searchresult.getDouble("DISPENSE_AMT", 0);
			...
			if (searchresult.getCount() <= 0) {
				result.addData("DISPENSE_QTY", 0);
				result.addData("VERIFYIN_PRICE", 0);
				result.addData("DISPENSE_AMT", 0);
				result.addData("DIFF", diff);
			} else {
				result.addData("DISPENSE_QTY", searchresult.getDouble(
						"DISPENSE_QTY", 0));
				result.addData("VERIFYIN_PRICE", searchresult.getDouble(
						"VERIFYIN_PRICE", 0));
				result.addData("DISPENSE_AMT", searchresult.getDouble(
						"DISPENSE_AMT", 0));
				result.addData("DIFF", diff);
			}
			if (Math.abs(diff) > 1) {
