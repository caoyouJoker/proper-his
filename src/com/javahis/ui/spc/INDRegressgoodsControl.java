...
public class INDRegressgoodsControl
    ...
    public void onQuery() {
        ...
        result = TIOM_AppServer.executeAction(
            "action.spc.INDRegressgoodsAction", "onQueryM", parm);
    ...
    public void onSave() {
		...
		double sQty = 0.00;
		double qtyCheck = 0.00;
		boolean flg = true;

		//fux modify 20150128 退货量不能大于验收量
		for (int i = 0; i < table.getRowCount(); i++) {
			double qtySum = 0.00;   
			double verQty = 0.00;
			String sql1 = " SELECT  VERIFYIN_QTY  FROM IND_VERIFYIND  WHERE VERIFYIN_NO = '"
					+ table.getItemString(i, "VERIFYIN_NO")
					+ "'"
					+ " AND SEQ_NO = '"
					+ table.getItemString(i, "VERSEQ_NO")
					+ "' ";
			String sql2 = " SELECT VERIFYIN_NO, VERSEQ_NO,SUM (QTY) AS QTY  FROM IND_REGRESSGOODSD  "
					+ " WHERE VERIFYIN_NO = '"
					+ table.getItemString(i, "VERIFYIN_NO")
					+ "'"
					+ " AND VERSEQ_NO = '"
					...
					+ "' "
					+
					// 完成状态(如果判断非完成状态则无法正常退货 )
					" AND UPDATE_FLG ='3' "
					+ " GROUP BY VERIFYIN_NO, VERSEQ_NO ";
			TParm parm1 = new TParm(TJDODBTool.getInstance().select(sql1));
			TParm parm2 = new TParm(TJDODBTool.getInstance().select(sql2));
			String orderCode = table.getItemString(i, "ORDER_CODE");
			String batchNo = table.getItemString(i, "BATCH_NO");
			String validDate = table.getItemString(i, "VALID_DATE").substring(
					0, 10);
			stockQty = INDTool.getInstance().getStockQTYAll(orgCode, orderCode,    
					batchNo, validDate, "");
			//System.out.println("sql3:"+INDSQL.getINDStockAll(orgCode, orderCode, batchNo, validDate));
			TParm getTRA = INDTool.getInstance().getTransunitByCode(orderCode);
			sQty = getTRA.getDouble("DOSAGE_QTY", 0);
			qtyCheck = table.getItemInt(i, "QTY");
			if (qtyCheck * sQty > stockQty) {
				flg = false;  
				break;    
			}
			verQty = parm1.getDouble("VERIFYIN_QTY", 0);
			qtySum = parm2.getDouble("QTY", 0);
			
			if ((qtyCheck * sQty + qtySum * sQty) > (verQty * sQty)) {
				flg = false;                     
				break;                           
			}
		...
		if (flg == false) {
			this.messageBox("退货数量大于验收数量不能退货!");
			return;
		} 
...
//                parm.setData("BATCH_SEQ", count, batch_seq);
                ...
                //
                String supOrderCode = dataStore.getItemString(newrows[i], "SUP_ORDER_CODE");
                dataStore.setItem(newrows[i], "SUP_ORDER_CODE", supOrderCode);
            ...
            result = TIOM_AppServer.executeAction(
                "action.spc.INDRegressgoodsAction", "onInsert", parm);
            //dataStore.showDebug();
            // 保存判断
...
//                    System.out.println("======"+batchSeqStr);
            ...
            // 执行数据更新
            result = TIOM_AppServer.executeAction(
                "action.spc.INDRegressgoodsAction", "onUpdate", parm);
            // 保存判断
            if (result == null || result.getErrCode() < 0) {
                this.messageBox("E0001");
                return;
        ...
        if (getRadioButton("UPDATE_FLG_A").isSelected()) {
            ...
            if (getTable("TABLE_M").getSelectedRow() > -1) {
                if (this.messageBox("删除", "确定是否删除退货单", 2) == 0) {
                    ...
                    TParm result = new TParm();
                    result = TIOM_AppServer.executeAction(
                        "action.spc.INDRegressgoodsAction", "onDeleteM",
                        parm);
                    // 删除判断
                    if (result == null || result.getErrCode() < 0) {
    ...
    public void onExport() {
        ...
        if (result != null) {
            ...
            for (int i = 0; i < addParm.getCount("ORDER_CODE"); i++) {
                ...
                String verifyin_no = addParm.getValue("VERIFYIN_NO", i)
				.toString();
                int verseq_no = addParm.getInt("SEQ_NO", i);
		TParm parmReg = new TParm(TJDODBTool.getInstance().select(
				getRegSql(verifyin_no, verseq_no)));
		// System.out.println("Regsql:"+getRegSql(verifyin_no,verseq_no));
		// 累计退货数
		        actual_qty = parmReg.getDouble("QTY", 0);
...
//                table.setItem(row, "SUM_QTY", sum_qty);
                ...
                //供应商编码 
                String supOrderCode = addParm.getValue("SUP_ORDER_CODE",i) ;
                table.setItem(row,"SUP_ORDER_CODE",supOrderCode);
...
//            parm.addData("SYSTEM","COLUMNS","MAN_CHN_DESC");
            ...
            // 调用打印方法
            this.openPrintWindow("%ROOT%\\config\\prt\\spc\\RegressGoods.jhw",
                                 date);
        }
        else {
            this.messageBox("没有打印数据");
...
//                return true;
//            }
        	//fux modify 20140128 退货数量大于库存量不能退货转移到保存校验
			String orgCode = getValueString("ORG_CODE");
			String orderCode = table.getItemString(row, "ORDER_CODE");
			String batchNo = table.getItemString(row, "BATCH_NO");  
			String validDate = table.getItemString(row, "VALID_DATE")
					.substring(0, 10);
			String sql = "SELECT * FROM IND_STOCK WHERE ORG_CODE = '" + orgCode
					+ "' AND ORDER_CODE = '" + orderCode + "' AND BATCH_NO = '"
					+ batchNo + "' AND VALID_DATE = TO_DATE('" + validDate
					+ "','YYYY-MM-DD')  AND STOCK_QTY>0 ";
			TParm parmSql = new TParm(TJDODBTool.getInstance().select(sql));
			double stockQty = parmSql.getDouble("STOCK_QTY", 0);
			if (stockQty <= 0) {
				this.messageBox("库存不够不能退货!");
				return true;
			}
...
*/                 if ("insert".equals(action)) {
	 ...
	 * @return QTY 库存量
	 */
	public String getRegSql(String verifyin_no, int verseq_no) {
		String sql = " SELECT SUM (QTY) AS QTY  FROM IND_REGRESSGOODSD "
				+ " WHERE VERIFYIN_NO = '" + verifyin_no + "'"
				+ " AND VERSEQ_NO = '" + verseq_no + "'"
				+ " GROUP BY VERIFYIN_NO, VERSEQ_NO";
		return sql;
	}
