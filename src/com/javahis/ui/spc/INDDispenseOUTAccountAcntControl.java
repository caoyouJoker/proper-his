 ...
 * @version 1.0
 */
public class INDDispenseOUTAccountAcntControl extends TControl {
	...
	public void onQuery() {
		...
		result = TIOM_AppServer.executeAction("action.spc.INDDispenseAction",
				"onQueryOutMAcnt", onQueryParm());// wanglong add 20150202
	...
	public void onClear() {
		...
		setValue("DISPENSE_DATE", dateNowStr.replace('-', '/'));
		// 初始化页面状态及数据
	...
	public void onSave() {
		if (getRadioButton("UPDATE_FLG_B").isSelected()) {
			...
			// modify by wangb 2015/06/16 允许用户修改出库时间
			if (getRadioButton("UPDATE_FLG_A").isSelected()) {
				if (StringUtils.isEmpty(this.getValueString("REQUEST_NO"))
						|| StringUtils.isEmpty(this
								.getValueString("DISPENSE_NO"))) {
					this.messageBox("申请单号以及出库单号不能为空");
					return;
				} else {
					TParm updateParm = new TParm();
					updateParm.setData("DISPENSE_DATE", this.getValueString(
							"DISPENSE_DATE").substring(0, 19));
					updateParm.setData("DISPENSE_NO", this
							.getValue("DISPENSE_NO"));
					updateParm.setData("OPT_USER", Operator.getID());
					updateParm.setData("OPT_TERM", Operator.getIP());

					// 更新出库时间
					updateParm = IndDispenseMTool.getInstance()
							.onUpdateMAcntOutDate(updateParm);
	...
	public void onCancle() {
		...
		if (this.messageBox("提示", "是否取消出库", 2) == 0) {
			...
			TParm result = TIOM_AppServer.executeAction(
					"action.spc.INDDispenseAction",
					"onUpdateDipenseCancelAcnt", parm);// wanglong modify
			// 20150202
	...
	public void onStop() {
		...
		if (this.messageBox("提示", "是否终止单据", 2) == 0) {
			...
			parm.setData("OPT_TERM", Operator.getIP());
			TParm result = TIOM_AppServer.executeAction(
					"action.spc.INDRequestAction", "onUpdateFlgAcnt", parm);// wanglong
			// modify
			// 20150202
			if (result == null || result.getErrCode() < 0) {
	...
	public void onPrint() {
		...
		DecimalFormat df = new DecimalFormat("##########0.00");// wanglong add
		// 20140729
		...
		if (table_d.getRowCount() > 0) {
			...
			String sql = "SELECT A.ORDER_CODE, CASE WHEN B.GOODS_DESC IS NULL "
					...
					// 零售价取盒装的?
					// +
					// " B.SPECIFICATION, C.UNIT_CHN_DESC, (A.RETAIL_PRICE*D.DOSAGE_QTY/D.STOCK_QTY) AS RETAIL_PRICE, "
					// fux modify 20150701
					+ " B.SPECIFICATION, C.UNIT_CHN_DESC,A.RETAILBOX_PRICE  AS RETAIL_PRICE, "
					// +
					// " A.QTY AS OUT_QTY, A.BATCH_NO, A.VALID_DATE,A.VERIFYIN_PRICE "//ADD
					// VERIFYIN_PRICE
					+ " A.QTY AS OUT_QTY, A.BATCH_NO, A.VALID_DATE,A.INVENT_PRICE AS VERIFYIN_PRICE,"
					+ " E.PHA_TYPE "// ADD
					// INVENT_PRICE
					// +
					// " FROM SPC_DISPENSED A, SYS_FEE B, SYS_UNIT C "//wanglong
					// 20150202
					+ " FROM SPC_DISPENSED A, SYS_FEE B, SYS_UNIT C, PHA_TRANSUNIT D ,PHA_BASE E "// fux
					// 20150706
					+ " WHERE A.ORDER_CODE = B.ORDER_CODE "
					+ " AND A.UNIT_CODE = C.UNIT_CODE "
			...
			for (int i = 0; i < printData.getCount("ORDER_CODE"); i++) {
				String pha_type = printData.getValue("PHA_TYPE", i);
				...
				qty = printData.getDouble("OUT_QTY", i);
				parm.addData("QTY", qty);
				parm.addData("AMT", df.format(StringTool.round(printData
						.getDouble("RETAIL_PRICE", i)
						* qty, 2)));// 显示两位小数 wanglong modify 20140729
				parm.addData("AMT_VERIFYIN", df.format(StringTool.round(
						printData.getDouble("VERIFYIN_PRICE", i) * qty, 2)));// 显示两位小数
				// wanglong
				// modify
				// 20140729
				sum_retail_price += printData.getDouble("RETAIL_PRICE", i)
			...
			double atm = StringTool.round(sum_retail_price, 2);
			double verifyinAtm = StringTool.round(sum_verifyin_price, 2);
			// date.setData("TOT_CHN", "TEXT",
			// "合计(大写): " + StringUtil.getInstance().numberToWord(atm));
			date.setData("ATM", "TEXT", df.format(atm));// 显示两位小数 wanglong
			// modify 20140729
			date.setData("VERIFYIN_ATM", "TEXT", df.format(verifyinAtm));// 显示两位小数
			// wanglong
			// modify
			// 20140729
			date.setData("VERIFYIN_ATM_DESC", "TEXT", StringUtil.getInstance()
					.numberToWord(verifyinAtm));
	...
	private void initPage() {
		...
		((TMenuItem) getComponent("cancle")).setEnabled(false);
		resultParm = new TParm();
		// getComboBox("TEST").setEnabled(true);
		// getText
	}
	...
	private void getTableDInfo(String req_no) {
		...
		TParm result = new TParm(TJDODBTool.getInstance().select(
				INDSQL.getOutRequestDInfoAcnt(req_no, "3")));// wanglong modify
		// 20150202
	...
	private void getTableDInfo2(String dispense_no) {
		...
		String sqlGetIndDispenseDByNo = "SELECT DISTINCT UNIT_TYPE FROM SPC_DISPENSEM WHERE DISPENSE_NO = '"
				+ dispense_no + "'";// wanglong modify 20150202
		TParm result = new TParm(TJDODBTool.getInstance().select(
		...
		if (result.getValue("UNIT_TYPE", 0).equals("1")) {
			sqlGetIndDispenseDByNo = " SELECT CASE"
					...
					+ " FROM SPC_DISPENSED A, PHA_BASE B, PHA_TRANSUNIT C"
					+ // wanglong modify 20150202
					" WHERE A.ORDER_CODE = B.ORDER_CODE"
			...
			sqlGetIndDispenseDByNo = "SELECT CASE WHEN B.GOODS_DESC IS NULL THEN B.ORDER_DESC ELSE "
					...
					+ "B.SPECIFICATION, A.QTY, A.ACTUAL_QTY, A.UNIT_CODE, "
					// fux modify 20150630
					// VERIFYIN_PRICE -> INVENT_PRICE
					// + "A.INVENT_PRICE/C.DOSAGE_QTY  AS STOCK_PRICE, "
					// + "A.RETAIL_PRICE/C.DOSAGE_QTY  "
					+ "A.INVENT_PRICE/C.DOSAGE_QTY/C.STOCK_QTY  AS STOCK_PRICE, "
					+ "A.RETAIL_PRICE  "
	...
	private TParm setTableDValue(TParm result) {
		...
		for (int i = 0; i < result.getCount(); i++) {
			// fuxin modify 20170420 加入藥品屬性判斷
			String pha_type = result.getValue("PHA_TYPE", i);
			if (!"ALL".equals(phaType)) {
				if (!phaType.equals(pha_type)) {
					continue;
				}
				...
				if ("0".equals(u_type)) {// 0代表库存单位是盒 所以stock_qty（片）
					...
					if ("RET".equals(request_type)) {// 如果是退药，查询库存量
						...
						parm.setData("STOCK_QTY", i, INDTool.getInstance()
								.getSPCStockQTY(out_org_code, order_code)
								/ result.getDouble("DOSAGE_QTY", i)
								/ result.getDouble("STOCK_QTY", i));
					} else {
						parm.setData("STOCK_QTY", i, INDTool.getInstance()
								.getStockQTYAcnt(
										out_org_code,
	 ...
	 * @return
	 */
	private TTextFormat getTestformat(String tagName) {
	...
	private boolean getDispenseOutOn(String org_code) {
			...
			for (int i = 0; i < table_d.getRowCount(); i++) {
				if ("Y".equals(table_d.getItemString(i, "SELECT_FLG"))) {
					...
					String sql = " SELECT A.STOCK_QTY / B.STOCK_QTY / B.DOSAGE_QTY AS "
							...
							+ " FROM SPC_STOCK A, PHA_TRANSUNIT B, "// wanglong
							// modify
							// 20150202
		...
		parm = TIOM_AppServer.executeAction("action.spc.INDDispenseAction",
				"onInsertOutOnAcnt", parm);// wanglong modify 20150202
	...
	private void getDispenseOutIn(String out_org_code, String in_org_code,
		...
		// 执行数据新增
		parm = TIOM_AppServer.executeAction("action.spc.INDDispenseAction",
				"onInsertOutInAcnt", parm);// wanglong modify 20150202
		// 保存判断
		if (parm == null || parm.getErrCode() < 0) {
			this.messageBox("E0001");
			return;
	...
	private void getUpdateDispenseMOutOn() {
		...
		// 执行数据更新
		parm = TIOM_AppServer.executeAction("action.spc.INDDispenseAction",
				"onUpdateMOutOnAcnt", parm);// wanglong modify 20150202
		// 保存判断
		if (parm == null || parm.getErrCode() < 0) {
			this.messageBox("E0001");
			return;
	...
	private boolean checkSameOrderQty(Map map) {
		...
		for (int i = 0; i < table_d.getRowCount(); i++) {
			...
			stock_qty = INDTool.getInstance().getSPCStockQTY(out_org_code,
					order_code);
