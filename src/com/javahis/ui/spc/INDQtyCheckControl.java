...
public class INDQtyCheckControl
    ...
    public void onSave() {
        ...
        Timestamp date = SystemTool.getInstance().getDate();//StringTool.getTimestamp(new Date());
        ...
        TParm result = TIOM_AppServer.executeAction(
            "action.spc.INDQtyCheckAction", "onUpdate", parm);
        // 保存判断
    ...
    public void onPrint() {
        ...
        Object result = openWindow("%ROOT%\\config\\spc\\INDQtyCheckPrt.x",
                                   parm);
    }
...
/*            //查询部门是否有智能柜
           System.out.println("-----------SPCSQL.getCabinetByOrgCode(org_code): "+SPCSQL.getCheckQtyInContainerD(org_code));
            TParm orgCodeParm = new TParm(TJDODBTool.getInstance().select(SPCSQL.getCheckQtyInContainerD(org_code)));
            if(null != orgCodeParm && orgCodeParm.getCount() > 0){
            	 ((TButton)getComponent("ATO_CHECK")).setEnabled(true);
            }else{
            	((TButton)getComponent("ATO_CHECK")).setEnabled(false);
            }*/
        }
    }
     ...
     * 自动盘点
     */
    public void onAtoCheckQty(){
          //查询部门是否有智能柜
//          System.out.println("-----------getCheckQtyInContainerD(org_code): "+SPCSQL.getCheckQtyInContainerD(org_code));
          TParm orgCodeParm = new TParm(TJDODBTool.getInstance().select(SPCSQL.getCheckQtyInContainerD(org_code)));
          if(null == orgCodeParm || orgCodeParm.getCount() <0){
        	  return ;
          }
	  	  TParm  parmD = table_d.getParmValue();
		  int countM = orgCodeParm.getCount();
		  int countD = parmD.getCount();
		  //在智能柜中是否有实物，有1 没有 0
		  String isCheckFlg = "0";
		  for (int i = 0; i < countD; i++) {
			  isCheckFlg = "0";
			  String orderCodeD = parmD.getValue("ORDER_CODE", i);
			  String batchSeqD = parmD.getValue("BATCH_SEQ", i);
			  for (int j = 0; j < countM; j++) {
				  String orderCodeM = orgCodeParm.getValue("ORDER_CODE", j);
				  String batchSeqM = orgCodeParm.getValue("BATCH_SEQ", j);
				  if(orderCodeD.equals(orderCodeM) && batchSeqD.equals(batchSeqM)){//在智能柜中 找到对应的实物，实际盘点量改为 智能柜中的量
					  String actualQtyF = orgCodeParm.getValue("STOCK_QTY", j);
					  String actualQtyM = orgCodeParm.getValue("STOCK_QTY_M", j);
					  //实际盘点量改为 智能柜中的量
					  table_d.setItem(i, "ACTUAL_QTY_F", StringTool.getDouble(actualQtyF));
					  table_d.setItem(i, "ACTUAL_QTY_M", StringTool.getDouble(actualQtyM));
					  isCheckFlg = "1";
					  break;
				  }
			}
			if(isCheckFlg.equals("0")){//表示在智能柜中没有对应的实物，实际数量置为0
				  table_d.setItem(i, "ACTUAL_QTY_F", "0");
				  table_d.setItem(i, "ACTUAL_QTY_M", "0");
			}  
		}
    }
    ...
    public boolean onTableDChangeValue(Object obj) {
        ...
        int row = node.getRow();
        if(StringUtils.isEmpty(frozen_date) && frozen_date.length()<5){
        	frozen_date = getComboBox("FROZEN_DATE").getSelectedID();
        }
        int column_stock = 10;
    ...
    private void getStockModiQty(int row, double qty) {
        ...
        if (qty_check_flg) {
            ...
            SPCTool.getInstance().updateQtyCheck(qty * rate + actual_qty_m - stock_qty,qty * rate + actual_qty_m,org_code,
            		frozen_date,table_d.getItemString(row,"ORDER_CODE"),table_d.getItemString(row,"BATCH_SEQ"),Operator.getID(),Operator.getIP());              
        }
        else {
    ...
    private void getDosageModiQty(int row, double qty) {
        ...
        if (qty_check_flg) {
            ...
            SPCTool.getInstance().updateQtyCheck(qty + actual_qty_f * rate - stock_qty ,qty + actual_qty_f * rate,org_code,
            		frozen_date,table_d.getItemString(row,"ORDER_CODE"),table_d.getItemString(row,"BATCH_SEQ"),Operator.getID(),Operator.getIP());
        }
        else {
            double stock_qty = table_d.getParmValue().getDouble(
