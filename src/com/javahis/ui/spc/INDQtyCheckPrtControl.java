...
public class INDQtyCheckPrtControl
    ...
    private void onPrintQtyCheckM() {
        ...
        String stockFlg = "N";
        if("Y".equals(getCheckBox("STOCL_FLG").getValue())){
        	stockFlg = "Y";
        };
        //fux modify INDSQL(ind->spc)
        String sql = INDSQL.getIndQtyCheckM(org_code, drozen_date,stockFlg);
...
//        parmData.addData("SYSTEM", "COLUMNS", "MAN_CODE");
    ...
    private void onPrintQtyCheckD() {
        ...
        String sql = INDSQL.getIndQtyCheckD(org_code, drozen_date,stockFlg);
...
//        parmEnd.addData("COL_7", StringTool.round(own_amt_g, 2));
    ...
    private void onPrintProfitLoss() {
        ...
        String sql = INDSQL.getIndQtyProfitLoss(org_code, drozen_date,stockFlg);
//        System.out.println("sql=== "+sql);  
        TParm parm = new TParm(TJDODBTool.getInstance().select(sql));
        if (parm == null || parm.getCount() <= 0) {
