...
public class SPCVerifyinControl extends TControl {
	...
	public void onSave() {
		...
		if ("insert".equals(action)) {
			...
			for (int i = 0; i < newrows.length; i++) {
				...
				if (supParm.getValue("SUP_ORDER_CODE", 0).equals("")) {
					// parm.setData("SUP_ORDER_CODE", count, "");
					// // 验收单号
					// dataStore.setItem(newrows[i], "SUP_ORDER_CODE", "");
					// 20150720 wangjc add start
					parm.setData("SUP_ORDER_CODE", count, order_code);
					// 验收单号
					dataStore.setItem(newrows[i], "SUP_ORDER_CODE", order_code);
					// 20150720 wangjc add end

				}
				// if(supParm.getCount()>0) {
				else if (!supParm.getValue("SUP_ORDER_CODE", 0).equals("")) {
					dataStore.setItem(newrows[i], "SUP_ORDER_CODE", supParm
							.getData("SUP_ORDER_CODE", 0));
					parm.setData("SUP_ORDER_CODE", count, supParm.getData(
							"SUP_ORDER_CODE", 0));
				}
				// 进货转换率集合
				...
				// 出入库效期批号者
				// double verifyinPrice = dataStore.getItemDouble(newrows[i],
				// "VERIFYIN_PRICE");
				...
				getSEQ = IndStockDTool.getInstance().onQueryStockBatchSeqBy(
						// org_code, order_code, batch_no,
						// valid_date,verpriceDStr,getValueString("SUP_CODE"),searchParm.getValue("SUP_ORDER_CODE",0));
						org_code, order_code, batch_no, valid_date,
						verpriceDStr, getValueString("SUP_CODE"),
						dataStore.getItemString(newrows[i], "SUP_ORDER_CODE"));// 20150717
				// wangjc
				// add
			...
			if ("Y".equals(check_flg)) {
				...
				for (int i = 0; i < table_D.getRowCount(); i++) {
					...
					searchParm.setData("SUP_CODE", getValueString("SUP_CODE"));
					searchParm.setData("ORDER_CODE", order_code);
					TParm supParm = INDTool.getInstance()
							.getSupCode(searchParm);
					String supOrderCode = "";
					if (supParm.getValue("SUP_ORDER_CODE", 0).equals("")) {
						supOrderCode = order_code;
						// 验收单号
					} else if (!supParm.getValue("SUP_ORDER_CODE", 0)
							.equals("")) {
						supOrderCode = supParm.getValue("SUP_ORDER_CODE", 0);
					}
					parm.setData("SUP_ORDER_CODE", i, supOrderCode);
					// 进货转换率集合
					s_qty = getTRA.getDouble("STOCK_QTY", 0);
					...
					double verifyinPrice = dataStore.getItemDouble(i,
							"VERIFYIN_PRICE");
					...
					// 出入库效期批号者
					getSEQ = IndStockDTool.getInstance()
							.onQueryStockBatchSeqBy(org_code, order_code,
	...
	public void onImpXML() {
		...
		TParm parm = new TParm();
		parm.setData("ORG_CODE", getValueString("ORG_CODE"));
		Object result = openDialog("%ROOT%\\config\\spc\\INDVerifyinImpXML.x",
				parm);
		...
		if (result != null) {
			...
			for (int i = 0; i < addParm.getCount("ORDER_CODE"); i++) {
				...
				TParm phaParm = new TParm(TJDODBTool.getInstance().select(
						INDSQL
								.getPHABaseInfo(addParm.getValue("ORDER_CODE",
										i))));
	...
	public void onPrint() {
		...
		if (table_d.getRowCount() > 0) {
			...
			for (int i = 0; i < table_d.getDataStore().rowCount(); i++) {
				...
				// fux modify 20170620 如果为整数 则显示整数 如果为小数 则格式化后保留2位小数显示
				if (table_d.getItemDouble(i, "VERIFYIN_QTY") % 1.0 == 0) {
					parm.addData("VERIFYIN_QTY", df5.format(table_d
							.getItemDouble(i, "VERIFYIN_QTY")));
				} else {
					parm.addData("VERIFYIN_QTY", df2.format(table_d
							.getItemDouble(i, "VERIFYIN_QTY")));
				}
	...
	public void onPrintcos() {
		...
		if (table_d.getRowCount() > 0) {
			...
			// 调用打印方法
			this.openPrintWindow("%ROOT%\\config\\prt\\IND\\VerifyIncos.jhw",
					date);
		} else {
			this.messageBox("没有打印数据");
			return;
	...
	public void onPrintnss() {
		...
		if (table_d.getRowCount() > 0) {
			...
			// 调用打印方法
			this.openPrintWindow("%ROOT%\\config\\prt\\IND\\VerifyInnss.jhw",
					date);
		} else {
			this.messageBox("没有打印数据");
			return;
	...
	private boolean CheckDataD() {
		...
		for (int i = 0; i < table.getRowCount(); i++) {
			if (table.getDataStore().isActive(i)) {
				...
				// 发票日期
				if (table.getItemTimestamp(i, "INVOICE_DATE") == null
						|| table.getItemTimestamp(i, "INVOICE_DATE").equals(
	...
	public void onExport2() {
		if (getTable("TABLE_D").getShowParmValue() == null
				|| getTable("TABLE_D").getShowParmValue().getCount() < 1) {
			this.messageBox("没有需要导出的数据");
			return;
		} else {
			// 获取医院名称
			String title = Manager.getOrganization().getHospitalCHNFullName(
					Operator.getRegion())
					+ "验收入库细项";

			TParm exportparm = getTable("TABLE_D").getShowParmValue();
			// 设置Excel名称
			exportparm.setData("TITLE", title);
			// 导出EXCEL并且去掉第一列
			exportparm.setData("HEAD", getTable("TABLE_D").getHeader()
					.replaceAll("执,30,boolean;", ""));

			String[] header = getTable("TABLE_D").getParmMap().split(";");
			int i;
			for (i = 1; i < header.length; i++) {
				exportparm.addData("SYSTEM", "COLUMNS", header[i]);
			}

			TParm[] exportexcel = new TParm[] { exportparm };
			ExportExcelUtil.getInstance().exeSaveExcel(exportexcel, title);
		}
	}
}
