 ...
 * @version 1.0
 */
public class INDDispenseOUTControl extends TControl {
	...
	// 页面上翻数据列表
	private String[] pageItem = { "REQTYPE_CODE", "REQUEST_NO", "APP_ORG_CODE",
			...
			for (int i = result.getCount("REQTYPE_CODE") - 1; i >= 0; i--) {
				...
				for (int j = 0; j < parm.getCount("DEPT_CODE"); j++) {
					...
					if ("DEP".equals(result.getValue("REQTYPE_CODE", i))
						...
						if (dept_code
								.equals(result.getValue("APP_ORG_CODE", i))) {
							flg = false;
							break;
						} else {
							flg = true;
	...
	public void onClear() {
		...
		// 清空画面内容
		String clearString = "REQUEST_NO;APP_ORG_CODE;REQTYPE_CODE;DISPENSE_NO;"
	 ...
	 */
	public void onSave() {
		if (!queryExistRequestNo()) {
			this.messageBox("该笔申请单已被删除,无法出库");
			return;
		}
	 ...
	 * @return
	 */
	private boolean queryExistRequestNo() {
		String request_no = this.getValueString("REQUEST_NO");
		String sql = "SELECT REQUEST_DATE FROM IND_REQUESTM WHERE REQUEST_NO = '"
				+ request_no + "'";
	...
	public void onCancle() {
		...
		if (this.messageBox("提示", "是否取消出库", 2) == 0) {
			...
			TParm result = TIOM_AppServer.executeAction(
					"action.spc.INDDispenseAction", "onUpdateDipenseCancel",
					parm);
	...
	public void onPrint() {
		...
		if (table_d.getRowCount() > 0) {
			// modify by wangbin 2015/2/2 内部需求 #786 泰心物联网系统调整方案 :
			// 药库出库单打印相关修改(出库单打印按照供应商分单打印) START
			// 查询该出库单号下有多少供应商
			String supCountSql = "SELECT  A.SUP_CODE, B.SUP_ABS_DESC FROM IND_DISPENSED A, SYS_SUPPLIER B WHERE A.SUP_CODE = B.SUP_CODE (+) "
					+ " AND A.DISPENSE_NO = '"
					+ this.getValueString("DISPENSE_NO")
					+ "' GROUP BY A.SUP_CODE, B.SUP_ABS_DESC ORDER BY A.SUP_CODE";
			TParm supCountData = new TParm(TJDODBTool.getInstance().select(
					supCountSql));

			if (supCountData.getErrCode() < 0 || supCountData.getCount() <= 0) {
				this.messageBox("查询药品供应商信息错误");
				err("ERR:" + supCountData.getErrText());
				return;
			}

			for (int m = 0; m < supCountData.getCount(); m++) {
				if (StringUtils.isEmpty(supCountData.getValue("SUP_CODE", m))
						|| StringUtils.isEmpty(supCountData.getValue(
								"SUP_ABS_DESC", m))) {
					this.messageBox("该出库单中存在无供应商的数据");
				}
			}

			// 出库明细数据
			String sql = "SELECT A.ORDER_CODE, CASE WHEN B.GOODS_DESC IS NULL "
					...
					+ "  CASE WHEN A.UNIT_CODE=D.STOCK_UNIT "
					+ "       THEN A.INVENT_PRICE "
					+ "       WHEN A.UNIT_CODE=D.DOSAGE_UNIT THEN A.VERIFYIN_PRICE ELSE A.VERIFYIN_PRICE  END  VERIFYIN_PRICE, "
					+ " A.SUP_CODE,E.TYPE_CODE "
					+ " FROM IND_DISPENSED A, SYS_FEE B, SYS_UNIT C ,PHA_TRANSUNIT D,PHA_BASE E "
					...
					+ this.getValueString("DISPENSE_NO")
					+ "' AND 1=1 "
					// === add by wukai 自动备药将未出库的药品筛选调
					+ " AND A.BATCH_NO IS NOT NULL AND A.BATCH_SEQ IS NOT NULL "
					+ " ORDER BY A.SEQ_NO";

			// 卫耗材
			sql = sql.replace("AND 1=1", "AND TYPE_CODE='4'");
			TParm healthSuppliesData = new TParm(TJDODBTool.getInstance()
					.select(sql));
			// System.out.println("printd sql1 : " + sql);
			if (healthSuppliesData.getErrCode() < 0) {
				this.messageBox("查询出库明细错误");
				err("ERR:" + healthSuppliesData.getErrText());
				return;
			}

			// 非卫耗材
			sql = sql.replace("AND TYPE_CODE='4'", "AND TYPE_CODE<>'4'");
			TParm notHealthSuppliesData = new TParm(TJDODBTool.getInstance()
					.select(sql));
			// System.out.println("printd sql2 : " + sql);
			if (notHealthSuppliesData.getErrCode() < 0) {
				this.messageBox("查询出库明细无误");
				err("ERR:" + notHealthSuppliesData.getErrText());
				return;
			}

			// 卫耗材与非卫耗材都查不到数据时
			if (healthSuppliesData.getCount() <= 0
					&& notHealthSuppliesData.getCount() <= 0) {
				this.messageBox("查无出库明细数据");
				return;
			}
			...
			if (healthSuppliesData.getCount() > 0) {
				// 按供应商分单打印卫耗材出库单
				for (int m = 0; m < supCountData.getCount(); m++) {
					this.setPrintData(supCountData.getRow(m),
							healthSuppliesData);
				}
			}

			if (notHealthSuppliesData.getCount() > 0) {
				// 按供应商分单打印非卫耗材出库单
				for (int m = 0; m < supCountData.getCount(); m++) {
					this.setPrintData(supCountData.getRow(m),
							notHealthSuppliesData);
				}
			}
			// modify by wangbin 2015/2/2 内部需求 #786 泰心物联网系统调整方案 :
			// 药库出库单打印相关修改(出库单打印按照供应商分单打印) END
		} else {
	...
	public void onTableDClicked() {
		...
		if (row_d != -1) {
			 ...
			 * table_d.getSelectedRow()); if (!"".equals(order_code)) {
	...
	public boolean onTableDChangeValue(Object obj) {
		...
		if (column == 6) {
			...
			double amt1 = StringTool.round(qty
					* table_d.getItemDouble(row_d, "STOCK_PRICE"), 2);
			double amt2 = StringTool.round(qty
					* table_d.getItemDouble(row_d, "RETAIL_PRICE"), 2);
	...
	private void getTableDInfo(String req_no) {
		...
		if ("TEC".equals(request_type) || "EXM".equals(request_type)
				|| "COS".equals(request_type) || "SRD".equals(request_type)
		...
		if ("DEP".equals(request_type) || "TEC".equals(request_type)) {
			...
			in_flg = false;
				...
				|| "SRD".equals(request_type) || "NMA".equals(request_type)) {
			out_org_code = this.getValueString("TO_ORG_CODE");
			out_flg = true;
			in_org_code = this.getValueString("APP_ORG_CODE");
			in_flg = false;
	...
	private void getTableDInfo2(String dispense_no) {
		...
		String sqlGetIndDispenseDByNo = "SELECT DISTINCT UNIT_TYPE FROM IND_DISPENSEM WHERE DISPENSE_NO = '"
				+ dispense_no + "'";
		/*
		 * TParm result = new
		 * TParm(TJDODBTool.getInstance().select(sqlGetIndDispenseDByNo)); if
		 * (result.getCount("UNIT_TYPE") == 0) { this.messageBox("没有申请明细");
		 * return; } if(result.getValue("UNIT_TYPE", 0).equals("1")){
		 * sqlGetIndDispenseDByNo = " SELECT CASE" +
		 * " WHEN B.GOODS_DESC IS NULL" + " THEN B.ORDER_DESC" +
		 ...
		 * " WHERE A.ORDER_CODE = B.ORDER_CODE" +
		 * " AND A.ORDER_CODE = C.ORDER_CODE" + " AND A.DISPENSE_NO = '" +
		 * dispense_no + "'"; }else{ sqlGetIndDispenseDByNo =
	...
	private TParm setTableDValue(TParm result) {
		...
		for (int i = 0; i < result.getCount(); i++) {
			...
			if (!"".equals(result.getValue("BATCH_NO", i))) {
				if ("0".equals(u_type)) {// 0代表库存单位是盒 所以stock_qty（片）
					// 要除以规格=盒，1代表发药单位by liyh 20120910
					if ("RET".equals(request_type)) {// 如果是退药，查询库存量
						// 不用根据batch_no,valid_date来查询，只需根据order_code就Ok
						// by liyh 20120910
						parm.setData("STOCK_QTY", i, INDTool.getInstance()
								.getStockQTY(out_org_code, order_code)
					...
					parm.setData("STOCK_QTY", i, INDTool.getInstance()
							.getStockQTY(
									...
									order_code,
									result.getValue("BATCH_NO", i),
									result.getValue("VALID_DATE", i).substring(
											0, 10), Operator.getRegion()));
				}
			} else {
				if ("0".equals(u_type)) {
					parm.setData("STOCK_QTY", i, INDTool.getInstance()
	...
	private String getBatchValid(String type) {
		if ("DEP".equals(type) || "TEC".equals(type) || "EXM".equals(type)
				...
				|| "SRD".equals(type) || "NMA".equals(type)) {
			return "1";
		}
		return "2";
	}
		...
		if ("DEP".equals(request_type)) {
			...
			for (int i = 0; i < table_d.getRowCount(); i++) {
				if ("Y".equals(table_d.getItemString(i, "SELECT_FLG"))) {
					...
					String sql = " SELECT A.STOCK_QTY / B.STOCK_QTY / B.DOSAGE_QTY AS "
					...
					if (checkParm.getCount("ORDER_CODE") > 0) {
						if (out_qty > checkParm.getDouble("STOCK_QTY", 0)) {
							...
							this.messageBox(order_desc
									+ " "
									+ order_code
									+ " 请领出库 "
									+ out_qty
									+ " "
									+ checkParm.getValue("UNIT_CHN_DESC", 0)
									+ ", 批号:"
									+ checkParm.getValue("BATCH_NO", 0)
									+ ",效期:"
									+ checkParm.getValue("VALID_DATE", 0)
											.substring(0, 10) + ",当前库存为"
	...
	private void getDispenseOutIn(String out_org_code, String in_org_code,
			String reuprice_flg, boolean out_flg, boolean in_flg) {
		if (!checkSelectRow()) {
			return;
		}
		TParm parm = new TParm();
	...
	private TParm getDispenseMParm(TParm parm, String update_flg) {
		...
		if ("WAS".equals(getValue("REQTYPE_CODE"))
				...
				|| "SRD".equals(getValue("REQTYPE_CODE"))
				|| "EXM".equals(getValue("REQTYPE_CODE"))
				|| "NMA".equals(getValue("REQTYPE_CODE"))) {
			update_flg = "3";
		}
	...
	private TParm getDispenseDParm(TParm parm) {
		...
		for (int i = 0; i < table_d.getRowCount(); i++) {
			...
			parmD.setData("PHA_TYPE", count, table_d.getItemString(i,
					"PHA_TYPE"));
			// luahi modify 2012-1-16 batch_seq 的值需要从Tparm中取得，因为页面中并不体现batch_seq
			// begin
			// parmD.setData("BATCH_SEQ", count, table_d.getItemData(i,
			// "BATCH_SEQ"));
			parmD.setData("BATCH_SEQ", count, table_d.getParmValue().getInt(
					"BATCH_SEQ", i));
			// luahi modify 2012-1-16 batch_seq 的值需要从Tparm中取得，因为页面中并不体现batch_seq
			// end
			...
			if ("".equals(valid_date)) {
				...
				parmD.setData("VALID_DATE", count, table_d.getItemTimestamp(i,
						"VALID_DATE"));
			}
			...
			parmD.setData("ELETAG_CODE", count, resultParm.getValue(
					"ELETAG_CODE", i));
			parmD.setData("ORDER_DESC", count, resultParm.getValue(
					"ORDER_DESC", i));
	 ...
	 * @param printData
	 *            出库明细
	 * @author wangbin 2015/2/2
	 */
	private void setPrintData(TParm supCountData, TParm printData) {
		// 表格数据
		TParm parm = new TParm();
		double qty = 0;
		...
		for (int i = 0; i < printData.getCount("ORDER_CODE"); i++) {
			if (StringUtils.equals(supCountData.getValue("SUP_CODE"), printData
					.getValue("SUP_CODE", i))) {
				parm.addData("ORDER_DESC", printData.getValue("ORDER_DESC", i));
				parm.addData("SPECIFICATION", printData.getValue(
						"SPECIFICATION", i));
				parm.addData("UNIT", printData.getValue("UNIT_CHN_DESC", i));
				parm.addData("UNIT_PRICE", df4.format(printData.getDouble(
						"RETAIL_PRICE", i)));
				parm.addData("VERIFYIN_PRICE", df4.format(printData.getDouble(
						"VERIFYIN_PRICE", i)));
				...
				parm.addData("AMT", StringTool.round(printData.getDouble(
						"RETAIL_PRICE", i)
						* qty, 2));
				parm.addData("AMT_VERIFYIN", StringTool.round(printData
		...
		if (null != getValue("URGENT_FLG")
				&& getValue("URGENT_FLG").equals("Y"))
			data.setData("URGENT", "TEXT", "急");
		else
			data.setData("URGENT", "TEXT", "");
		// ===============pangben modify 20110607 stop
		data.setData("DISP_NO", "TEXT", "出库单号: "
				+ this.getValueString("DISPENSE_NO"));
		data.setData("REQ_NO", "TEXT", "申请单号: "
				+ this.getValueString("REQUEST_NO"));
		data.setData("OUT_DATE", "TEXT", "出库日期: "
				+ this.getValueString("DISPENSE_DATE").substring(0, 10)
						.replace('-', '/'));
		data.setData("REQ_TYPE", "TEXT", "申请类别: "
				+ this.getComboBox("REQTYPE_CODE").getSelectedName());
		data.setData("ORG_CODE_APP", "TEXT", "申请部门: "
				+ this.getTextFormat("APP_ORG_CODE").getText());
		data.setData("ORG_CODE_FROM", "TEXT", "接受部门: "
				+ this.getComboBox("TO_ORG_CODE").getSelectedName());
		data.setData("DATE", "TEXT", "制表日期: "
				+ datetime.toString().substring(0, 10).replace('-', '/'));
		// 供应商
		data.setData("SUP_ABS_DESC", "TEXT", "供应商: "
				+ supCountData.getValue("SUP_ABS_DESC"));

		data.setData("TABLE", parm.getData());

		// 表尾数据
		// luhai 2012-1-22 加入合计atm
		String atm = String.valueOf(StringTool.round(sum_retail_price, 2));
		String verifyinAtm = String.valueOf(StringTool.round(
				sum_verifyin_price, 2));
		// date.setData("TOT_CHN", "TEXT",
		// "合计(大写): " + StringUtil.getInstance().numberToWord(atm));
		data.setData("BAR_CODE", "TEXT", this.getValueString("DISPENSE_NO"));
		data.setData("ATM", "TEXT", atm);
		data.setData("VERIFYIN_ATM", "TEXT", verifyinAtm);
		data.setData("VERIFYIN_ATM_DESC", "TEXT", StringUtil.getInstance()
				.numberToWord(Double.parseDouble(verifyinAtm)));
		data.setData("TOT", "TEXT", "合计: ");
		data.setData("USER", "TEXT", "制表人: " + Operator.getName());
		data.setData("BAR_CODE", "TEXT", this.getValueString("DISPENSE_NO"));
		// 调用打印方法
		this.openPrintWindow("%ROOT%\\config\\prt\\ind\\DispenseOut.jhw", data);
	}
}
