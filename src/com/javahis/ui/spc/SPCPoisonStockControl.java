 ...
 * @version 1.0
 */
public class SPCPoisonStockControl extends TControl {
	...
	public void onClick() {
		...
		if ("".equals(code)) {
			this.messageBox("验收入库单号 不能为空");
			return;
		}
		TParm result = SPCPoisonTool.getInstance().queryInfoVerifyin(t);
		if (result.getCount() <= 0) {
			this.messageBox("验收入库单号错误");			
		}
	...
	public void onTableClicked() {
		...
		String verNo = (String) rowParm.getData("VERIFYIN_NO", row);
		int seqNo = rowParm.getInt("SEQNO", row);
		TParm parm = new TParm();
		parm.setData("DISPENSE_NO", verNo);
	...
	public boolean onOrderValueChange(TTableNode tNode) {
		...
		if (row != -1) {
			...
			String dispenseNo = (String) rowParm.getData("VERIFYIN_NO", rowNum);
