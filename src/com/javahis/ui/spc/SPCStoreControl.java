 ...
 * @version 1.0
 */
public class SPCStoreControl extends TControl implements MessageListener{
	...
	public void initPage() {
		...
		// add by wangbin 2014/11/19 非智能柜设备访问该页面时IP控件可编辑 START
		if (result.getCount() > 0) {
			...
			getTextField("CABINET_IP").setEnabled(true);
		}
		// add by wangbin 2014/11/19 非智能柜设备访问该页面时IP控件可编辑 END
	...
	public void onInstore() {
		...
		for (int i = 0; i < count; i++) {
			...
			if (set.contains(containerId)) {
				...
				TParm mParm = SPCInStoreTool.getInstance().onQueryDispenseM(
						inParmSec);
				TParm mRowParm = mParm.getRow(0);
				setValueAll(mRowParm);
				int seqNo = talbleparm.getInt("DISPENSE_SEQ_NO", i);
				inParmSec.setData("SEQ_NO", seqNo);
				inParmSec.setData("CONTAINER_ID", containerId);
				TParm dispenseDParm = SPCInStoreTool.getInstance()
						.onQueryDispenseD(inParmSec);
				TParm dParm = new TParm();
				dParm.setData("DISPENSE_NO", dispenseDParm.getData(
						"DISPENSE_NO", 0));

				dParm.setData("SEQ_NO", dispenseDParm.getData("SEQ_NO", 0));

				dParm.setData("REQUEST_SEQ", dispenseDParm.getData(
						"REQUEST_SEQ", 0));
				dParm.setData("ORDER_CODE", dispenseDParm.getData("ORDER_CODE",
						0));
				dParm.setData("QTY", dispenseDParm.getData("QTY", 0));
				dParm.setData("UNIT_CODE", dispenseDParm
						.getData("UNIT_CODE", 0));
				dParm.setData("RETAIL_PRICE", dispenseDParm.getDouble(
						"RETAIL_PRICE", 0));
				dParm.setData("STOCK_PRICE", dispenseDParm.getDouble(
						"STOCK_PRICE", 0));
				dParm.setData("ACTUAL_QTY", dispenseDParm.getDouble(
						"ACTUAL_QTY", 0));
				dParm.setData("PHA_TYPE", dispenseDParm.getData("PHA_TYPE", 0));

				dParm
						.setData("BATCH_SEQ", dispenseDParm.getInt("BATCH_SEQ",
								0));

				dParm.setData("BATCH_NO", dispenseDParm.getData("BATCH_NO", 0));

				dParm.setData("VALID_DATE", dispenseDParm.getTimestamp(
						"VALID_DATE", 0));

				dParm.setData("DOSAGE_QTY", dispenseDParm.getDouble(
						"DOSAGE_QTY", 0));

				dParm.setData("OUT_QTY", dispenseDParm.getDouble("OUT_QTY", 0));
				
				dParm.setData("INSTORE_QTY", dispenseDParm.getDouble(
						"OUT_QTY", 0));						

				dParm.setData("STOCK_QTY", dispenseDParm.getDouble("STOCK_QTY",
						0));	
				dParm.setData("SUP_CODE", dispenseDParm.getValue("SUP_CODE",
						0));  
				dParm.setData("INVENT_PRICE", dispenseDParm.getValue("INVENT_PRICE",
						0));  
				dParm.setData("RETAIL_PRICE", dispenseDParm.getDouble("RETAIL_PRICE",
						0)); 
				dParm.setData("SUP_ORDER_CODE", dispenseDParm.getValue("SUP_ORDER_CODE",
						0));  
				dParm.setData("VERIFYIN_PRICE", dispenseDParm.getDouble("VERIFYIN_PRICE",
						0)/dispenseDParm.getDouble("DOSAGE_QTY",
								0));  							
				this.onSave(mRowParm, dParm, inserParm);  				
	...
	public void onClear() {
		...
		// add by wangbin 2014/11/19 清空时保持对应控件可编辑
	 ...
	 */
	public void onSave() {	
    	// add by wangbin 2141124 取不到智能柜编号时提示 START
    	if (StringUtils.isEmpty(getValueString("CABINET_ID"))) {
    		this.messageBox("智能柜编号不能为空");
    		return;
    	}
    	// add by wangbin 2141124 取不到智能柜编号时提示 END
//		onEnd();
/*		TABLE_M_N = getTable("TABLE_M_N");
...
*/		
		...
		for (int i = 0; i < count; i++) {
			String containerId = (String) talbleparm.getData("CONTAINER_ID", 0); 
			inserParm.setData("CONTAINER_ID", containerId);
		//	if (set.contains(containerId)) {  
				TParm inParmSec = new TParm();
				inParmSec.setData("DISPENSE_NO", dispenseNo);
				...
				TParm mRowParm = mParm.getRow(0);
				setValueAll(mRowParm);
				int seqNo = talbleparm.getInt("DISPENSE_SEQ_NO", 0);
				inParmSec.setData("SEQ_NO", seqNo);
				inParmSec.setData("CONTAINER_ID", containerId);
				TParm dispenseDParm = SPCInStoreTool.getInstance()
						.onQueryDispenseD(inParmSec);
				...
				dParm.setData("SUP_CODE", dispenseDParm.getValue("SUP_CODE",
						0));  
				/*this.messageBox(dispenseDParm.getValue("SUP_ORDER_CODE",
						0));
				if(1==1) {
					return;
				}*/
	 ...
	 * 保存方法
	 */
	public void onSave(TParm mParm, TParm dParm, TParm insertParm) {
		...
		if (!"THI".equals(request_type)) {
			// 入库作业
			getDispenseOutIn(out_org_code, in_org_code, batchvalid, out_flg,
					in_flg, mParm, dParm, insertParm);
		} else {
			// 其它入库作业
	...
	//	parmD.setData("SUP_CODE", count, rowParm.getValue("SUP_CODE"));
		...
		parm.setData("SUP_CODE",rowParm.getValue("SUP_CODE"));
		parm.setData("INVENT_PRICE",rowParm.getDouble("INVENT_PRICE"));		
		parm.setData("RETAIL_PRICE",rowParm.getDouble("RETAIL_PRICE"));	
		parmD.setData("VERIFYIN_PRICE",count,rowParm.getDouble("VERIFYIN_PRICE"));	
		//parm.setData("SUP_ORDER_CODE",rowParm.getDouble("SUP_ORDER_CODE"));			
		count++;				
		if (parmD != null) {
	 ...
	 * 通过手动输入智能柜IP地址完成入库
	 * 
	 * @author wangbin 2014/11/19
	 */
	public void onQueryCabinet() {
		ip =  this.getValueString("CABINET_IP").trim();
		if (ip.length() < 1) {
			return;
		}
		TParm parm = new TParm();
		...
		if (result.getCount() > 0) {
			...
			String mqIp = WS_URL.substring(WS_URL.indexOf("http://") + 7,
					WS_URL.indexOf(":8080"));
			MQ_IP = mqIp;
			tparm.setData("MQ_IP", mqIp);
			tparm.setData("READER_ID", CABINET_ID);
			...
			this.messageBox("无此IP地址的智能柜信息");
			this.setValue("CABINET_IP", "");
			this.setValue("CABINET_ID", "");
			this.setValue("CABINET_DESC", "");
			this.setValue("ORG_CODE", "");
			this.grabFocus("CABINET_IP");
			return;
		}
	}
