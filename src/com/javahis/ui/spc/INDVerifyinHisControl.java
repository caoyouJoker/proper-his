...
import com.javahis.ui.spc.FileUtils;
...
public class INDVerifyinHisControl
    extends TControl {
...
/*                    result = IndPurorderMTool.getInstance().onQueryDone(inparm);
            ...
            // 执行数据更新
            result = TIOM_AppServer.executeAction(
                "action.spc.INDVerifyinAction", "onUpdate", parm);
            // 保存判断
            if (result == null || result.getErrCode() < 0) {
                this.messageBox("E0001");
                return;
...
/*                // 发票号码
	...
	public void onPrint() {
		...
		if (table_d.getRowCount() > 0) {
			...
			date.setData("DATE", "TEXT", "验收日期: " + datetime.toString().substring(0, 10).replace('-', '/'));
			// 表格数据
			TParm parm = new TParm();
			String order_code = "";
			double ver_sum = 0;
