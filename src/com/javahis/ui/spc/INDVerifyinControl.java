...
//			parm.setData("SPC_BOX_BARCODE", getValueString("SPC_BOX_BARCODE"));
			...
			for (int i = 0; i < newrows.length; i++) {
                ...
                getSEQ = IndStockDTool.getInstance().onQueryStockBatchSeqBy(
                        org_code, order_code, batch_no, valid_date,verpriceDStr,getValueString("SUP_CODE"),sup_order_code);
				...
				//System.out.println("getSEQ.getCount::::"+getSEQ.getCount("BATCH_SEQ"));
...
//				}
				...
				for (int i = 0; i < table_D.getRowCount(); i++) {
					...
					getSEQ = IndStockDTool.getInstance().onQueryStockBatchSeqBy(
		                		org_code, order_code, batch_no, valid_date,verpriceDStr,getValueString("SUP_CODE"),supOrderCode);
...
//					dataStore.setItem(i, "ACTUAL_QTY", result.getDouble("ACTUAL_QTY", i));//20150723 wangjc add
					// 状态
					String update_flg = "0";
					...
					//dataStore.setItem(i, "PUTAWAY_DATE", date);    
...
//		System.out.println("-------------flag: "+flag);
	...
	public void onPrint() {
		...
		//	parm.addData("SYSTEM", "COLUMNS", "INVOICE_NO");
			...
			date.setData("BAR_CODE", "TEXT", "1311270000001");  
			// 表尾数据
		//	date.setData("USER", "TEXT", "核对人: " + Operator.getName());
			...
			//fux modify 20150811   
			String datePrint  = datetime.toString().substring(0, 10).replace('-', '/')+"";;
			date.setData("PRINT_DATE", "TEXT", "制表日期: " +datePrint);  
...
//		System.out.println("-----dispenseMParm: "+dispenseMParm);
		if (dispenseMParm.getCount() > 0) {
			...
			String sql = "SELECT A.ORDER_CODE, CASE WHEN B.GOODS_DESC IS NULL "
					...
					+ " A.QTY AS OUT_QTY, A.BATCH_NO, A.VALID_DATE,A.VERIFYIN_PRICE,A.SUP_CODE,D.SUP_ABS_DESC,E.TYPE_CODE "// ADD
					+ " FROM IND_DISPENSED A, SYS_FEE B, SYS_UNIT C, SYS_SUPPLIER D, PHA_BASE E "
					+ " WHERE A.ORDER_CODE = B.ORDER_CODE "
					+ " AND A.UNIT_CODE = C.UNIT_CODE "
					+ " AND A.SUP_CODE = D.SUP_CODE "
					+ " AND A.ORDER_CODE = E.ORDER_CODE "
					+ " AND A.DISPENSE_NO = '" + dispsenseNo + "' "
					+ " ORDER BY A.SUP_CODE,A.SEQ_NO";
			...
			for (int m = 0; m < supCountData.getCount(); m++) {
				...
				qty = 0;
				sum_retail_price = 0;
				sum_verifyin_price = 0;
				for (int i = 0; i < printData.getCount("ORDER_CODE"); i++) {
					if (StringUtils.equals(
							supCountData.getValue("SUP_CODE", m), printData
									.getValue("SUP_CODE", i))) {
						parm.addData("ORDER_DESC", printData.getValue(
								"ORDER_DESC", i));
						parm.addData("SPECIFICATION", printData.getValue(
								"SPECIFICATION", i));
						parm.addData("UNIT", printData.getValue(
								"UNIT_CHN_DESC", i));
						parm.addData("UNIT_PRICE", df4.format(printData
								.getDouble("RETAIL_PRICE", i)));
						parm.addData("VERIFYIN_PRICE", df4.format(printData
								.getDouble("VERIFYIN_PRICE", i)));
						qty = printData.getDouble("OUT_QTY", i);
						parm.addData("QTY", qty);
						parm.addData("AMT", StringTool.round(printData
								.getDouble("RETAIL_PRICE", i)
								* qty, 2));
						parm.addData("AMT_VERIFYIN", StringTool.round(printData
								.getDouble("VERIFYIN_PRICE", i)
								* qty, 2));
						sum_retail_price += printData.getDouble("RETAIL_PRICE",
								i)
								* qty;
						sum_verifyin_price += printData.getDouble(
								"VERIFYIN_PRICE", i)
								* qty;
						parm.addData("BATCH_NO", printData.getValue("BATCH_NO",
								i));
						parm.addData("VALID_DATE", StringTool.getString(
								printData.getTimestamp("VALID_DATE", i),
								"yyyy/MM/dd"));
					}
				}
				...
				// 表头数据
				data.setData("TITLE", "TEXT", Manager.getOrganization().getHospitalCHNFullName(Operator.getRegion()) + "出库单");
				...
				data.setData("DISP_NO", "TEXT", "出库单号: " + dispsenseNo);
				data.setData("REQ_NO", "TEXT", "申请单号: " + dispenseMParm.getValue("REQUEST_NO",0));
				...
				data.setData("OUT_DATE", "TEXT", "出库日期: " + dispenseDate);
				data.setData("REQ_TYPE", "TEXT", "申请类别: " + "麻精过度");
				data.setData("ORG_CODE_APP", "TEXT", "申请部门: " +appOrgCodeParm.getValue("ORG_CHN_DESC",0));
				data.setData("ORG_CODE_FROM", "TEXT", "接受部门: " +toOrgCodeParm.getValue("ORG_CHN_DESC",0));
				data.setData("DATE", "TEXT", "制表日期: " + datetime.toString().substring(0, 10).replace('-', '/'));
				data.setData("TABLE", parm.getData());
				
				// 供应商
				data.setData("SUP_ABS_DESC", "TEXT", "供应商: "
						+ supCountData.getValue("SUP_ABS_DESC", m));
				// 表尾数据
				...
				data.setData("VERIFYIN_ATM_DESC", "TEXT", StringUtil
						.getInstance().numberToWord(
								Double.parseDouble(verifyinAtm)));
				data.setData("TOT", "TEXT", "合计: ");
				data.setData("USER", "TEXT", "制表人: " + Operator.getName());
				data.setData("BAR_CODE", "TEXT", dispsenseNo);
				// 调用打印方法
				this.openPrintWindow(
						"%ROOT%\\config\\prt\\spc\\DispenseOut.jhw", data);
			}
