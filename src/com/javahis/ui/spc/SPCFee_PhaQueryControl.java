...
import jdo.spc.SPCFee_PhaQueryTool;
 ...
 * @version 1.0
 */
public class SPCFee_PhaQueryControl extends TControl {
	 ...
	 * 所有控件的名字
	 */
	private String controlNameTab1 = "LET_KEYIN_FLG;OPD_FIT_FLG;EMG_FIT_FLG;IPD_FIT_FLG;"
			...
			+ "OWN_PRICE3;ADDPAY_RATE;ADDPAY_AMT;ORD_SUPERVISION";// shibl
																	// 20121127
																	// add

	private String controlNameTab2 = "MEDI_QTY1;DOSAGE_QTY1;DOSAGE_QTY;STOCK_QTY;STOCK_QTY1;PURCH_QTY;"
			...
			// 添加SKINTEST_FLG（皮试）字段PRESCRIPTION_FLG（处方）字段
			...
			+
			// =========pangben modify 20110817 添加SYS_GRUG_CLASS(药物种类)字段
			// NOADDTION_FLG (零差价)字段
			"MEDI_QTY;DEFAULT_TOTQTY;ATC_FLG;IS_REMARK,ATC_FLG_I"
			+ ";SYS_GRUG_CLASS;NOADDTION_FLG;OPD_ATC_ORG;ODI_ATC_ORG;" +
			// 加DRUG_NOTES_PATIENT（病人用药注意事项） DRUG_NOTES_DR（医师用药注意事项）
			// DRUG_NOTE（药袋说明） DRUG_FORM（外形说明）
			// DDD
			"DRUG_NOTES_PATIENT;DRUG_NOTES_DR;DRUG_NOTE;DRUG_FORM;DDD";// shibl
																		// 20121127
																		// add
	...
	/*
	 * 药品设置时,默认总量只读,当选中总量给药时.打开默认总量 初始化时,当选中药品是总量给药时,打开默认总量,否则设置默认总量只读 Modify
	 * ZhenQin 2011-06-01
	 */
...
//		onInitNode();
	...
	public void onTreeClicked(Object parm) {
		...
		if (node.getType().equals("Root")) {
			...
			String sql = getSQL(id, Operator.getRegion());// ======pangben
															// modify 20110426
															// 添加参数
	 ...
	 * 
	 * @return String =========pangben modify 20110426 添加区域过滤
	 */
	private String getSQL(String orderCode, String regionCode) {
		...
		if (orderCode != null && orderCode.length() > 0)
			if (orderCode.length() <= 5) {
				sql = " SELECT * " + " FROM SYS_FEE_HISTORY A,SYS_FEE_SPC B"
						+ " WHERE " + " B.HIS_ORDER_CODE like '" + orderCode
						+ "%'" + active + region + " ORDER BY A.ORDER_CODE";
			} else {
				sql = " SELECT * " + " FROM SYS_FEE_HISTORY" + " WHERE "
						+ " ORDER_CODE like '" + orderCode + "%'" + active
	...
	public void myInitControler() {
		...
		CTRLDRUGCLASS_CODE = (TComboBox) this
				.getComponent("CTRLDRUGCLASS_CODE");
		...
		DRUG_NOTES_PATIENT = (TTextField) this
				.getComponent("DRUG_NOTES_PATIENT	");
	...
	public void onTabPanel2() {
		...
		this.DRUG_NOTES_PATIENT.setValue(phaBase.getValue("DRUG_NOTES_PATIENT",
				0));
	...
	public void getTabPanel2Vaule() {
		...
		phaTransUnitDS.setItem(phaTransUnitRow, "ORDER_CODE", ORDER_CODE
				.getValue());
		phaTransUnitDS
				.setItem(phaTransUnitRow, "MEDI_QTY", MEDI_QTY1.getText());
		phaTransUnitDS.setItem(phaTransUnitRow, "DOSAGE_QTY", DOSAGE_QTY
				.getText());
		phaTransUnitDS.setItem(phaTransUnitRow, "STOCK_QTY", STOCK_QTY1
				.getText());
		phaTransUnitDS.setItem(phaTransUnitRow, "PURCH_QTY", PURCH_QTY
				.getText());
		phaTransUnitDS.setItem(phaTransUnitRow, "MEDI_UNIT", MEDI_UNIT
				.getValue());
		phaTransUnitDS.setItem(phaTransUnitRow, "DOSAGE_UNIT", DOSAGE_UNIT
				.getValue());
		phaTransUnitDS.setItem(phaTransUnitRow, "STOCK_UNIT", STOCK_UNIT
				.getValue());
		phaTransUnitDS.setItem(phaTransUnitRow, "PURCH_UNIT", PURCH_UNIT
				.getValue());
		// ============pangben modify 20110427 start
		phaTransUnitDS.setItem(phaTransUnitRow, "REGION_CODE", this
		...
		phaBaseDS.setItem(phaBaseRow, "DEFAULT_TOTQTY", DEFAULT_TOTQTY
				.getText());
		...
		phaBaseDS.setItem(phaBaseRow, "DSPNSTOTDOSE_FLG", DSPNSTOTDOSE_FLG
				.getValue());
		...
		// 处方combo
		phaBaseDS.setItem(phaBaseRow, "PRESCRIPTION_FLG", PRESCRIPTION_FLG
				.getValue());
		// =======pangben modify 20110428 stop

		phaBaseDS.setItem(phaBaseRow, "CTRLDRUGCLASS_CODE", CTRLDRUGCLASS_CODE
				.getValue());
		phaBaseDS.setItem(phaBaseRow, "ANTIBIOTIC_CODE", ANTIBIOTIC_CODE
				.getValue());
		...
		// ===========pangben modify 20110426 start
		phaBaseDS.setItem(phaBaseRow, "REGION_CODE", this
				.getValueString("REGION_CODE"));
		// ===========pangben modify 20110426 stop
		phaBaseDS.setItem(phaBaseRow, "DRUG_NOTES_PATIENT", DRUG_NOTES_PATIENT
				.getValue());
		phaBaseDS
				.setItem(phaBaseRow, "DRUG_NOTES_DR", DRUG_NOTES_DR.getValue());
	...
	public void clearCtl() {
		...
		String cbo = cbos.toString().substring(0,
				cbos.toString().lastIndexOf(";"));
		this.clearValue(cbo);
		// =========pangben modify 20110426 start
	 ...
	 */
	public void onSave() {
		String order_code = ORDER_CODE.getValue();
		String pack_unit = PACK_UNIT.getValue();
		if(this.pack_unitOld.equals(pack_unit)){
			messageBox("只可以修改中包装!");
			onQuery();
			return;
		}
		...
		parm.setData("PACK_UNIT",pack_unit);
		TParm result = SPCFee_PhaQueryTool.getInstance().updatePhaBase(parm);
	...
	private boolean setDataInSysFeeTds() {
		...
		sysFeeDS
				.setItem(0, "DRUG_NOTES_PATIENT", DRUG_NOTES_PATIENT.getValue());
	...
	public void setDataInTDS(TDataStore dataStore, int row, boolean dateFlg) {
		...
		dataStore.setItem(row, "DRUG_NOTES_PATIENT",
				getValue("DRUG_NOTES_PATIENT"));
		...
		// dataStore.setItem(row, "ORD_SUPERVISION",
		// ORD_SUPERVISION.getValue());
		StringBuffer cbos=new StringBuffer();
        for (int i = 1; i <= 20; i++) {
            if (this.getValue("cbo" + i).equals("Y")) {
                cbos.append(i + ";");
	...
	public void cerateNewDate() {
		...
		if (classify > ruleTool.getClassifyCurrent()) {
			...
			// ===pangben modify 20110427 start
			// ============查找此编号规则中编号最大值
			String sql = "SELECT MAX(ORDER_CODE) AS ORDER_CODE FROM PHA_BASE WHERE ORDER_CODE LIKE '"
					+ nowID + "%'";
			TParm parm = new TParm(getDBTool().select(sql));
...
//		}
        String[] cbos = date.getValue("SYS_PHA_CLASS").split(";");
        for (int i = 0; i < cbos.length; i++) {
      	    String cTag = "cbo" + cbos[i];
      	    String cText = this.getText(cTag)==null?"":this.getText(cTag).trim();
            if (!"".equals(cText)){
                this.setValue(cTag, "Y");
            }
        }
		this
				.setValue("DRUG_NOTES_PATIENT", date
						.getValue("DRUG_NOTES_PATIENT"));
	...
	public void onQuery() {
		...
		// 拿到查询code的SQL语句
		String sql = getSQL(selCode, this.getValueString("REGION_CODE"));// ======pangben
																			// modify
																			// 20110426
																			// 添加参数
		// add by lx 2013/01/06
		...
		this.pack_unitOld = this.getValueString("PACK_UNIT");
	}
	...
	public void onTableDoubleCleck() {
		...
		// 调用查询事件
		// ======pangben modify 20110426 start 添加参数
		onCleckClassifyNode(partentID, upTable, this
				.getValueString("REGION_CODE"));
		// ======pangben modify 20110426 stop
		// table中设置选中行
	...
	public String[] getSql() {
		...
		return StringTool.copyArray(StringTool.copyArray(bigObject.getDS(
				"SYS_FEE").getUpdateSQL(), bigObject.getDS("PHA_TRANSUNIT")
				.getUpdateSQL()), bigObject.getDS("PHA_BASE").getUpdateSQL());
	}
