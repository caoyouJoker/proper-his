 ...
 * @version 1.0
 */
public class OPBRECTPrintReportToolForCustom_puhua implements IReport {
    ...
    public TParm getReportParm(TParm parm) {
        if (parm.getValue("ADM_TYPE").equals("H")) {//add by wanglong 20130408
            parm.setData("CIRCLE", "TEXT", "○");
            parm.setData("HRM", "TEXT", "健");
        }
    ...
    private TParm setUncertainItems(TParm oneReceiptParm) {//add by wanglong 20121217
        ...
        if (oneReceiptParm.existGroup("CHARGE11")&&(oneReceiptParm.getDouble("CHARGE11", "TEXT")!=0)) {// 体检费(自定义费用) add by wanglong 20130123
            if(!oneReceiptParm.getValue("CUSTOM_TEXT1", "TEXT").trim().equals("")){
                oneReceiptParm.setData("ITEM"+num, "TEXT", oneReceiptParm.getValue("CUSTOM_TEXT1", "TEXT"));
                oneReceiptParm.setData("FEE"+(num++), "TEXT", df.format(oneReceiptParm.getDouble("CHARGE11", "TEXT")));
            }
        }
        if (oneReceiptParm.existGroup("CHARGE19")&&(oneReceiptParm.getDouble("CHARGE19", "TEXT")!=0)) {// 诊察费
        ...
        if (num > 9) {
            ...
            partAMt += oneReceiptParm.getDouble("FEE17", "TEXT");//add by wanglong 20130123
            // 警告：上面总共有17个项目，目前票据上只留了16个项目的打印位置。
            // 极端情况下，如果病患把项目都做全了，发票会少打一个项目，引发错误！
