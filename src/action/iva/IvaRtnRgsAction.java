 ...
 * @version 1.0
 */
public class IvaRtnRgsAction extends TAction {
	...
	public TParm onInsert(TParm parm) {
		...
		for (int i = 0; i < parm.getCount("CASE_NO"); i++) {
			...
			result = IvaRtnRgsTool.getInstance().onInsertM(parmRow, connection);
            ...
            if (parmRow.getValue("RT_TYPE").equals("DC")) {// add by wanglong 20130628
                ...
                result = IvaRtnRgsTool.getInstance().onUpdateDispenseReturnQty(parmRow, connection);
                if (result.getErrCode() != 0) {
                    connection.close();
                    return result;
                }
			...
			result = IvaRtnRgsTool.getInstance().onInsertD(parmRow, connection);
			if (result.getErrCode() != 0) {
				connection.close();
				return result;
			}
	...
	public TParm onUpdate(TParm parm) {
		...
		for (int i = 0; i < parm.getCount("CASE_NO"); i++) {
			TParm parmRow = parm.getRow(i);
			result = IvaRtnRgsTool.getInstance().onUpdateM(parmRow, connection);
			if (result.getErrCode() < 0) {
				connection.close();
				return result;
			}
			result = IvaRtnRgsTool.getInstance().onUpdateD(parmRow, connection);
			if (result.getErrCode() < 0) {
				connection.close();
				return result;
			}
	...
	public TParm onDelete(TParm parm) {
		...
		for (int i = 0; i < parm.getCount("CASE_NO"); i++) {
			TParm parmRow = parm.getRow(i);
			result = IvaRtnRgsTool.getInstance().onDeleteM(parmRow, connection);
			if (result.getErrCode() < 0) {
				connection.close();
				return result;
			}
			...
			result = IvaRtnRgsTool.getInstance().onDeleteD(parmRow, connection);
			if (result.getErrCode() < 0) {
				connection.close();
				return result;
			}
	...
	public TParm onDeleteSingle(TParm parm) {
		...
		TParm result = new TParm();
		// System.out.println(parm.getCount("CASE_NO"));
		result = IvaRtnRgsTool.getInstance().onDeleteM(parm, connection);
		if (result.getErrCode() < 0) {
			connection.close();
			return result;
		}
		result = IvaRtnRgsTool.getInstance().onDeleteD(parm, connection);
		if (result.getErrCode() < 0) {
			connection.close();
			return result;
		}
	...
	public TParm onUpdateRtnCfm(TParm parm) {
		...
		if (spcflg.equals("N")) {
			for (int i = 0; i < parm.getCount("CASE_NO"); i++) {
				...
				result = IvaRtnRgsTool.getInstance().onUpdateRtnCfmM(parmRow,
						connection);
				if (result.getErrCode() < 0) {
				...
				result = IvaRtnRgsTool.getInstance().onUpdateRtnCfmD(parmRow,
						connection);
				if (result.getErrCode() < 0) {
					// luhai add 2012-04-15 add rollback
					System.out.println(result.getErrText());
	...
	public TParm uddRtnMed(TParm uddRegParm, String serviceLevel,
		...
		TParm uddrtnData = IvaRtnRgsTool.getInstance().getUDDRTNData(
				uddRegParm, conn);
